...
- The transaction that creates `dispatch_fenced` uses steps 2–8 of the shared lock plan for connection/current health generation, configuration, discovery observation, capability status, input-scan policy, global/connection policy, and attempt/job state. While holding them, require connection status `active`, circuit breaker `closed`, and a dispatch-eligible current health generation; revalidate the clean input-scan decision/current version, live tool name, invocable capability projection, exact current connection-configuration version and its required minimum output classification, current present discovery observation/snapshot, `last_complete_bypass_at` younger than the 15-minute dispatch-staleness bound, and current input/output policy compatibility. Half-open probes use a separate non-run path. Write the fence only if every check passes. If breaker/degradation, control change, or staleness wins first, fencing fails and no call occurs; if fencing commits first, the attempt is already in at-most-once uncertain-execution semantics. Health mutations take the same connection lock, so there is no unlocked check-to-fence gap.
...
- Every run attempt persists its pre-send dispatch fence, exact per-run-keyed clean-input fingerprint reference/state and scan-policy version/decision, nullable credential-binding version, optional provider receipt, reconciliation state, output classification, output-scan status, and result-fingerprint kind plus nullable digest/HMAC key version. A rejected sensitive input is never a run attempt, a reclassified request's fingerprint becomes cryptographically non-comparable, and a discarded sensitive result has no content-derived fingerprint.
...
- `GET /v1/artifacts/{id}/content` — require normal authentication plus the bound grant in the redacted `X-Modall-Artifact-Grant` header, never a URL. On every call, obtain a qualified current authorization snapshot under the Section 8 freshness rule, resolve the verifier to the exact grant, require its current event-backed status projection to be `active`, and recheck subject, workspace membership/role, token epoch against the current epoch, artifact visibility, classification policy, retention, scan status, exact immutable version/integrity, and grant expiry. Any stale refresh, mismatch, revoked/expired projection, or post-mint authorization change returns no bytes; possession never bypasses current authorization. Clean text and JSON are parsed, escaped, and rendered in the sandboxed viewer; only explicitly allowlisted non-text types that completed all content-aware scans are attachment-only. Active, encrypted/uninspectable, unknown, unsupported, type-mismatched, coverage-incomplete, or quarantined artifacts return no content bytes. Ingress, proxy, application, and trace configuration must redact the grant header. Responses use `Cache-Control: no-store`, `X-Content-Type-Options: nosniff`, restrictive sandbox CSP, and same-origin/cross-origin isolation headers.
...
| E5-T2 | Implement run, request-key hierarchy/erasure, attempt, event, receipt/reconciliation, output-classification, and immutable artifact plus event-backed access-grant models | L | E1, E2-T5 | Effective output class is independently derived; retained requests and fingerprints use purpose-separated externally erasable subkeys; every terminal, erasure/non-comparable, quarantine, grant active/revoked/expired, and fingerprint-key state has complete replayable lineage |
...
| E5-T7 | Quarantine; independent output-data classification; malware/type/archive plus per-MIME extraction/render/OCR classification; validate, redact, and store results | L | E5-T2, E0-T4 | Detector-clean proprietary output from a restricted connection remains restricted; effective class is the maximum of input/connection/detector; images/PDFs/archives with visible, metadata, or embedded secrets/PII are restricted/discarded; incomplete/invalid/scanner-failed content cannot publish |
...
| E6-T5 | Implement secret-scanning run preflight, no-store run/event/cancel, artifact-grant mint/revoke, and currently reauthorized artifact-read endpoints | M | E5, E6-T1 | Event-backed individual revocation, grant replay/expiry, and membership/role/epoch/visibility/policy/retention/integrity changes deny token replay and bytes; invocation/cache tests pass |
...
| 05 | `feat(registry): add entries connections and immutable snapshots` | `SafeUrl`, inert schema-URI references, explicit connection output classification, core registry migrations, repositories, lifecycle, canonical hashing | 03, 04 | Missing/changed output class, navigable-URL, local-ref, no-external-fetch, and versioning golden tests |
...
| 07 | `feat(mcp): connect and discover remote Streamable HTTP servers` | Shared `SafeUrl` enforcement, SDK wrapper, negotiation, address-pinned transport, paginated tools, recursive whole-schema classification, TTL-clamped forced reconciliation, health | 05, 06 | Both protocol eras plus description/default/example/custom-annotation secret/PII rejection, zero schema-derived persistence, cache-staleness, URL-secret, DNS-rebinding, and SSRF suites |
...
| 10 | `feat(jobs): add durable jobs encrypted run ledger and erasure keys` | Leases, events, attempts, idempotency, per-run external encryption/fingerprint key handles, ciphertext/keyed-only request state, shared lock plan, output classification, artifact and grant-event/projection metadata | 03, 04, 08 | Crash recovery, no-compare erasure, classification/grant replay, key-lifecycle, and lock-order-helper tests |
| 11 | `feat(invocation): execute MCP tools with bounded inputs and results` | Input scanning, ephemeral decryption/rescanning, attested content/fingerprint erasure, health-eligible lock-serialized dispatch, independent output classification, cancellation, and content-aware artifacts | 07, 10 | Breaker races and low-entropy post-erasure enumeration fail safely; proprietary output, input secrecy, fault, invalid/binary-sensitive output, malware/archive, and integrity suites pass |
| 12 | `feat(api): expose registry capability and run APIs` | REST resources, stable Run/event contracts, scan-before-hash idempotency, lock-serialized admission, run preflight, prepared/committed access-grant envelopes, individual grant revocation, auth epochs, generated client | 08–11 | OpenAPI/client fixtures, admission/control races, envelope prepare/commit/revoke crash matrix, revoked-mint-replay denial, grant-token/input secrecy, API E2E |
...
- effective result-class lattice derivation from independent input, immutable connection minimum, and detector/profile values; quarantine, redaction, per-MIME required-coverage accounting, and fail-closed extraction/OCR/scanner behavior;
...
- A grant minted immediately before individual grant, membership/role, or artifact-policy revocation cannot retrieve content after the authoritative grant/epoch/state projection changes; replaying the original mint key after individual revocation never returns the token.
