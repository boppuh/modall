...
- Every ordinary URL-valued field—including connection endpoints, redirects, imported source locations, upstream search results, and retained resource links—must cross the one typed `SafeUrl` parser/sanitizer before persistence, audit, error interpolation, logging, tracing, or API output. It rejects userinfo, fragments, ambiguous/double encoding, credential/token/signature/secret/high-entropy-shaped decoded path segments, and non-allowlisted or credential-shaped query names/values. Raw URL secrets never leave the bounded parsing boundary. Registry Alpha does not issue delegated upload URLs; Phase 1's separately typed, short-lived `EphemeralUploadTarget` is a sealed outbound secret capability and is never accepted or represented as `SafeUrl`.
...
| E5-T4 | Implement bounded pre-persistence input secret/PII scanning and JSON Schema validation | M | E2-T5, E0-T4 | Invalid/pathological schemas and scanner failures fail safely; low-entropy credentials produce no request, digest, fingerprint, log, or provider send |
...
| 12 | `feat(api): expose registry capability and run APIs` | REST resources, scan-before-hash durable idempotency, lock-serialized run admission, single-use run preflight, sealed access-grant replay envelopes, auth epochs, generated client | 08–11 | OpenAPI, admission/control races, grant-token non-persistence, input-secret non-persistence, and API E2E suites |
