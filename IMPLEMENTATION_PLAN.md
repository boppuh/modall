...
| P0-04A Budget pilot | Separate 20-PR candidate matrix, exact usage/rate attribution, and frozen limits | Pilot cases excluded from official splits; sub-minor costs, settlement rounding/residuals, and cost/latency/resource ceilings verified before official matrix | Platform/product |
...
| P0-08 Analysis | Baseline comparison, exact-cost joint power, and stratum-scoped sequential confidence bounds | Report preserves fractional-minor costs; sizing simulates the complete gate with a >=90% lower bound; activation uses the same paired bootstrap/alpha registry; no component/evidence is reused across attempts/strata | Data/full-stack |
...
- `POST /v1/artifact-uploads` — authorize metadata, bind an immutable server-derived source-classification floor and provenance-attestation version whose signed content identity/manifest is expected for this exact upload, and return one dedicated short-lived `EphemeralUploadTarget` scoped to a unique create-only object key or storage version; a client declaration may raise but never lower the floor
- `POST /v1/artifact-uploads/{id}/complete` — under the shared provenance/artifact/policy lock order, require server time strictly before `expires_at`, close the upload, verify the exact storage version's ephemeral client integrity proof, size, content type, and canonical content identity against the signed provenance attestation, plus scan attestation against the current output/artifact policy and applicable MIME-profile versions, then derive the effective artifact classification as the maximum of the immutable source floor, client declaration, and detector/profile result, persist kind-appropriate integrity metadata, and mint the authoritative `artifact://` URI; provenance mismatch stays quarantined and cannot inherit the claimed floor, while an expired `open` row atomically moves to `cleanup_pending` and returns no artifact even if the sweeper is late
- `GET /v1/artifacts/{id}` — return authorized metadata, current trusted source-floor and policy/profile scan eligibility/readiness, exact immutable storage-version identity, and a clean-content digest only when policy permits; stale provenance/scan reports safe reclassification/`rescan_required` metadata and no viewer mode, never a restricted plaintext fingerprint or ambient object-store credential
...
Every API money, provider-rate, reservation, and usage-cost field is an exact decimal string or explicit rational—not a JSON floating-point number. The schema carries ISO currency, billing unit, immutable price-version ID, and provider settlement scope/rounding rule so SDKs cannot silently round fractional minor units.
...
Routing and run requests accept only finalized, unexpired artifacts owned by the same workspace, allowed by request/provider data policy, and carrying an append-only scan attestation over the exact immutable content identity that matches the locked current output/artifact-scan-policy version and applicable MIME-profile version. Each upload uses a unique non-overwritable object key or versioned-bucket write. Completion closes the upload, verifies an ephemeral client integrity proof and length against the exact storage version, detects archive expansion/path traversal, and applies the Registry Alpha malware/type plus per-MIME metadata/embedded-content extraction, all-page/frame render/OCR, secret/PII classification, and complete-coverage policy; a type without a complete tested profile remains quarantined. Clean artifacts persist a content digest; encrypted restricted artifacts persist a ciphertext integrity checksum plus a purpose-separated versioned HMAC plaintext fingerprint whose key remains in the secret manager, never an ordinary plaintext digest. Activating a changed policy or applicable profile immediately makes every older attestation logically `rescan_required` without waiting for fan-out: grant mint/replay/redemption, reads, routing, and initial/fallback dispatch all return no token/bytes/send. A durable job may rescan retained quarantined bytes and append a superseding attestation only when commit-time locks prove the same content identity and still-current policy/profile; unavailable bytes, scan failure, newly disallowed content, or another intervening change remains quarantined or is discarded. Consumption reauthorizes the artifact and reads only that pinned version, verifying its kind-appropriate integrity value and current attestation; a still-valid upload credential cannot replace finalized content. HTTP/SDK clients upload through the `EphemeralUploadTarget`; the MCP facade exposes both `create_artifact_upload` and `complete_artifact_upload`, with target delivery allowed only through the non-recording transfer channel above and completion returning the authoritative `artifact://` URI so an eligible MCP control-plane client needs no REST credential. Large or retained non-text results use the same immutable artifact and authorized-access contract, so the console never reads object storage directly.
...
For result consumption without REST credentials, `read_artifact(artifact_uri, offset, max_bytes)` uses the authenticated MCP session to obtain a qualified current authorization snapshot and, under the shared provenance/artifact/policy lock order, recheck subject, workspace membership/role, authorization epoch, artifact visibility, current trusted source floor/effective classification, retention, exact immutable version/kind-appropriate integrity state, and an attestation matching the current output/artifact-scan-policy plus applicable MIME-profile versions on every call. It returns at most 256 KiB per chunk with total length, detected content type, a clean-content digest only when policy permits, and the next offset; restricted plaintext fingerprints remain internal. Safe text/JSON uses typed content, and only explicitly allowlisted bytes that completed their full current content-aware profile use base64. Repeated bounded calls can consume a large result, while stale/incomplete authorization or provenance, `rescan_required`, active, encrypted/uninspectable, unknown, unsupported, type-mismatched, coverage-incomplete, quarantined, expired, cross-workspace, integrity-failed, or policy-forbidden content fails closed. The tool never returns an ambient object-store URL or credential.
...
Phase 1 extends the Alpha shared lock-plan helper while preserving the relative order of every common class. A fence performs its qualified-source network lookup before taking database locks. After an API idempotency lock when applicable, coordinated transactions acquire: (1) actor user, workspace membership, and current authorization-epoch projection; (2) provider current-price selectors; (3) stable server-connection or deployment rows; (4) exact/current target configuration and MCP discovery pointers/snapshots; (5) capability-status projections; (6) source-provenance/repository-classification and workspace unverified-upload-floor pointers; (7) artifact rows; (8) current input-scan-policy and output/artifact-scan-policy plus MIME-profile pointers/versions, ordered by policy kind then MIME; (9) workspace/provider/data/destination policy rows; (10) quota/budget accounts ordered by scope, currency, and unit; and (11) run, attempt, job, and reservation rows in that order. IDs within a class are sorted. Fence workers release lease/job locks before starting this transaction; identity, price, disable, provenance/classification, artifact publication/access/rescan, scan-policy/profile activation, other policy, and quota mutations use only the same relevant subsequence. Advancing a provenance/classification or scan-policy/profile pointer atomically invalidates older eligibility/attestations, so activation never requires locking every artifact. Reverse-order input and opposite worker/control start tests fail on any timeout, deadlock, raw coordinated lock outside the helper, or order inversion.

The immutable route quote and execution ceilings determine an approved worst-case reservation vector across every enforced dimension, including exact decimal/rational currency, calls, tokens/tool usage, and workspace/provider limits. Immediately before the transaction that creates the initial dispatch fence—and again for every fallback child fence—the worker obtains a qualified current authorization result for the originating actor without database locks. The extended helper then locks actor membership/current epoch, quoted price selector, execution target/configuration, capability, source provenance/classification, artifacts, scan policies/profiles, applicable policies, quota/budget accounts, and attempt/reservation state in the order above. Reconcile the fresh authorization result and require current membership/execution role plus exact equality with the admitted epoch; recheck every input artifact's effective classification against its current trusted source floor and every scan attestation against the exact current output/artifact policy/profile, server time strictly before quote expiry, and the provider's applicable current immutable price-version ID still equal to the quoted version. Actor changes or a stale/underclassified artifact terminate unfenced work with `actor_authorization_changed` or `dispatch_eligibility_revoked` and zero sends. An expired quote or changed price terminates the undispatched attempt with stable `quote_expired` or `price_version_changed`, sends nothing, and requires a new route/quote plus user confirmation; the worker may not silently reprice or extend expiry. Only a fresh quote proceeds to check each limit dimension net of outstanding reservations, append a unique `usage_reservation` for the exact attempt/fence, atomically move the whole worst-case vector from available to reserved, and write the dispatch fence. No complete reservation means no fence and no provider send; concurrent attempts therefore cannot all pass a check against the same unreserved balance. If any other eligibility or reservation check fails, terminate the still-undispatched invocation with stable `dispatch_eligibility_revoked` or `quota_budget_unavailable` evidence.

Provider reconciliation is the only path from reserved liability to a terminal accounting disposition. Definitive actual usage commits that amount and releases the proven remainder; definitive non-execution releases the full reservation. An uncertain fenced attempt retains its worst-case reservation while `indeterminate` until provider evidence or manual reconciliation resolves it; an age policy may escalate or block new work but cannot release an amount that might still be billed. A fallback child gets a new reservation only under the remaining immutable run ceiling and only after the prior attempt is proven not executed and its reservation is released, unless a tested end-to-end provider idempotency contract gives one shared charge identity and reservation lineage. Any provider usage above the reserved ceiling is a fail-closed billing incident that blocks further dispatch for the affected account until reconciled. Record execution disposition separately from transport status. Fallback is allowed only when durable evidence proves the preceding attempt did not execute, or when all candidates share a tested end-to-end idempotency contract for the external operation. A fenced attempt with uncertain provider acceptance becomes `indeterminate` and can neither retry nor fall back. Every result passes the Registry Alpha quarantine, classification, redaction, and artifact policy and stores its exact policy/profile scan attestation before ordinary persistence or display; scanner failure fails closed. A later policy/profile change immediately suppresses persisted inline and artifact result reads until a current-version rescan succeeds or policy discards the bytes.
...
- every inline result and artifact is releasable only under an attestation matching the current output/artifact scan policy and MIME profile; activation of a new version immediately yields `rescan_required`, denies grant replay/redemption and reads, and only a same-content current-version rescan can restore readiness;
...
| P1-01A Artifacts | Non-overwritable ingestion, trusted source-provenance/classification floors, quiescent cleanup, policy/profile scan attestations/rescan, immutable URI, revocable grant | Identity, object storage | Client declarations cannot lower source floor; private/unverified source cannot reach universal providers; expiry/overwrite pass; scan/provenance drift blocks grant/read/dispatch until current reclassification/rescan; revocation suppresses replay |
...
| P1-04 Runtime | Input scan/erasure, attested output scanning, pools, quote/exact-price freshness, dispatch/authorization eligibility, precise reservation, deadlines, fallback, breakers, no redirects | P0 adapters | Rejected input erases both subkeys; stale/underclassified artifacts release/send nothing; fences reauthorize actor and recheck health/artifacts/exact price plus reserve atomically; 3xx never resends; fractional costs reconcile without per-call rounding |
...
| P1-08 Telemetry | Traces, logs, metrics, exact-rate usage reservation and cost reconciliation | All request paths | Trace/span-link chain covers route through outcome; exact decimal/rational usage preserves sub-minor costs and provider settlement residuals; stale liabilities alert |
...
- `artifact_source_attestations` with server-verified repository identity/commit or data-owner-approved source record, expected canonical bundle/manifest identity, immutable classification/version, authority, and audit lineage; `run_artifacts` as a visibility-domain- and role-qualified link to existing immutable `artifacts` and their source plus append-only exact-content scan attestations/current status projection; `artifact_uploads` retains only opaque upload ID, safe configured-origin ID, immutable key/version scope, client declaration, trusted source-attestation/floor version and expected content identity, constraints, `open|completed|cleanup_pending|deleted` state, expiry, qualified `W`/`C`, write-fence/active-lease evidence, cleanup lease/attempts, quiescence deadline, and deletion attestation—never an `EphemeralUploadTarget` credential; upload-target and canonical `artifact_access_grants` rows store only non-secret metadata/versioned HMAC verifiers, inherited append-only grant events/current projections provide `active|revoked|expired`, and inherited idempotency/outbox rows reference prepared/committed exact-lifetime encrypted envelopes and retain only non-secret tombstones after logical expiry/erasure
- append-only `usage_reservations` linked one-to-one with a dispatch fence/attempt, plus exact decimal/rational reservation/commit/release/reconciliation events, raw provider usage dimensions, immutable decimal/rational price-version references, settlement rules/residuals, and workspace/provider quota-budget account balances
...
- Every upload/final artifact stores an immutable trusted source-attestation ID/version, expected/computed canonical content identity, and server-derived source-classification floor. A claimed attestation is usable only when its signed identity matches the exact completed bytes. Effective artifact classification is constrained to the maximum of that floor, the client declaration, and detector/profile classification. Missing/unverifiable provenance uses the current workspace `unverified_upload_floor` (default `private_internal`); client-controlled fields cannot lower either value, and current floor/attestation changes invalidate prior routing/dispatch eligibility.
...
- Every retained result/artifact stores an append-only attestation over its immutable content identity, exact output/artifact-scan-policy version, applicable MIME-profile version, classification, and coverage. Release checks compare it with the locked current pointers; any mismatch is immediately `rescan_required` and denies inline reads, grants, artifact bytes, routing, and dispatch until a same-content current-version rescan commits. Policy/profile activation advances only its pointer under the shared lock order, so it cannot race into partial artifact invalidation.
...
- A monetary value uses exact fixed-scale `NUMERIC(38,18)` major-currency storage when exactly representable at that scale; otherwise it uses a normalized arbitrary-precision numerator/denominator representation. Both carry ISO currency and a representation discriminator and serialize as decimal strings or explicit rationals, never binary floating point. Conversion between the forms may not round. Immutable provider price versions retain their exact decimal or rational rate plus billing unit, documented settlement scope, and rounding rule. Usage and charges accumulate without per-invocation minor-unit or fixed-scale rounding; rounding occurs only at that provider's settlement boundary, and reconciliation records/distributes any residual deterministically. Reservations, attributed attempt costs, currency normalization, bootstrap inputs, and cost-per-success ratios preserve the same exact value.
...
- output/artifact scan-attestation state, current policy/profile matching, and `rescan_required` invalidation;
...
- generated Python, TypeScript, and MCP interfaces match OpenAPI semantics, including decimal-string/rational money fields, sealed upload-target/access-grant handling, upload completion, unsupported-host failure, and bounded `read_artifact` parity;
- upload contracts expose client declaration plus opaque authorized provenance reference but never client-writable source floor/effective classification; absent, cross-workspace, stale, or unauthorized attestations fail closed;
...
- fractional-minor provider rates and invocations remain nonzero through reservation, usage attribution, settlement reconciliation, currency normalization, bootstrap resampling, and cost-per-success ratios; exact provider-boundary rounding and deterministic residual allocation reconcile to the billed total without per-invocation bias;
...
- artifact upload with trusted source floor, current-policy/profile-attested completion, authoritative URI issuance, authorized viewer/download grants, and rejection of underclassified, unsafe, mutable/overwritten, wrong-version, integrity-mismatched, stale-scan, unfinalized, expired, or cross-workspace artifacts;
