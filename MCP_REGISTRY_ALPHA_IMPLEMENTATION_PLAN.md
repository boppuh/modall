...
- A tool content digest is an opaque composite over normalized non-sensitive name/title/description/annotations, the input/output schema HMAC identities, exact `server_connection_version_id`, negotiated protocol revision, `implementation_identity_assurance`, normalized identity source, and implementation revision when available; raw schema bytes or an ordinary schema-derived digest never enter it. The complete discovery-snapshot ID/digest remains separate observation provenance, so unrelated tool drift cannot churn a version. Credential rotation, protocol change, identity-source change, or assurance change creates a new digest even with byte-identical advertised content. Destruction of either schema fingerprint subkey marks the tool digest non-comparable, suppresses it from public detail, and leaves only an opaque generic tombstone projection; retained HMAC/composite bits in immutable rows, WAL, replicas, and backups cannot test candidate schema values without the destroyed subkey.
- Capability-version identity is the logical capability plus opaque content digest plus a monotonically increasing occurrence generation. To reuse the latest non-superseded version, discovery must hold current clean attestations, decrypt its schemas in bounded memory, and compare the candidate's canonical schema bytes plus safe fields exactly; it then reuses the existing schema identities/digest. A changed or non-comparable current version creates a new immutable `pending_review` generation with fresh schema hierarchies. If content A is superseded by B and A later reappears, create a fresh A generation in `pending_review`; never resurrect the terminal earlier A row.
...
- Disabling a connection atomically increments its monotonic `connection_lifecycle_epoch`, cancels every discovery/recovery job and in-process discovery cancellation token, transitions every currently `enabled` capability version on that connection to `disabled` with reason `connection_disabled`, and cancels queued runs before setting the connection `disabled`. Re-enable increments the epoch again and enters `verifying`; connection reverification never clears prior capability states, and each intended version requires a later explicit enable action.
...
- Ordinary reads may honor an entry within that effective TTL. Independently, every active connection must complete a scheduled full no-cache reconciliation at least every 10 minutes with bounded jitter that never pushes the deadline later, regardless of the server hint or notification support. Before any scheduled, notification-triggered, or operator discovery performs network I/O, a short transaction locks the connection, requires lifecycle state `verifying|active|degraded`, monotonically increments `latest_started_discovery_generation`, and captures that generation plus the exact current connection-configuration version and `connection_lifecycle_epoch`; disabled connections allocate nothing and perform no request. Before credential retrieval and every later pagination request, the client rechecks that the epoch is unchanged and state is not `disabled`, aborting and wiping buffers on mismatch in addition to honoring the cancellation token. After complete pagination and schema scanning, publication reacquires the shared connection/configuration/observation/capability/schema-policy locks and succeeds only through compare-and-set when the captured generation, configuration, schema-policy version, and lifecycle epoch still equal their current values and the locked lifecycle remains publication-eligible. A superseded or disabled completion is discarded with safe `discovery_superseded|connection_disabled` evidence and cannot write a cache/snapshot/observation/capability/schema-attestation transition, update reconciliation timestamps, schedule recovery, or move the connection to `active`; raw pages and provisional schema keys are destroyed. Thus a request started later always wins even if it finishes first, while disable/re-enable invalidates every older fetch. A failed latest eligible generation is recovered by a new generation rather than an older stale success. When a missed deadline or current-generation failure degrades a non-disabled connection, the scheduler preserves one durable recovery job and keeps attempting complete bypass with bounded exponential backoff/jitter capped at five minutes until a winning successful pagination atomically restores current observations and `active`, or an operator explicitly disables the connection; disable terminates retries and lease recovery cannot recreate them until re-enable. Operator refresh also bypasses the SDK/list cache. A bypass path performs a fresh upstream listing and replaces the cache only after the winning complete publication; if the SDK lacks a bypass API, use a fresh no-cache client instance/path.
...
| `capability_versions` | Immutable whole-schema-scan-clean encrypted schema references, opaque composite keyed content digest/comparability lineage, occurrence generation, and binding identity; excludes lifecycle status | Append-only |
...
- A comparable discovery snapshot digest is unique within a server connection and is constructed only from safe normalized metadata plus active keyed schema identities, never raw schema bytes or ordinary schema digests.
- Every discovery network attempt first requires an eligible non-disabled lifecycle, allocates a monotonically increasing per-connection generation, and captures the current configuration and lifecycle epoch under lock. Publication compare-and-sets all three values and requires an eligible locked state; only the latest-started matching generation/epoch may append a `discovery_publication` plus observations/version transitions, point to a new or deduplicated snapshot, replace cache state, advance reconciliation time, schedule recovery, or restore `active`. Disable and re-enable each advance the epoch; superseded/disabled completions are non-publishable and their raw pages/provisional keys are discarded.
...
- `POST /v1/server-connections/{id}/disable` — atomically increment the connection lifecycle epoch, invalidate/cancel every in-flight or queued discovery/recovery attempt, prevent new discovery execution/runs, disable currently enabled capability versions with reason `connection_disabled`, and cancel undispatched runs.
...
- `GET /v1/capabilities/{id}/versions/{version_id}` — exact schemas, opaque content identity, and binding only when every schema has an active hierarchy and attestation matching the locked current schema-scan policy; otherwise return safe `schema_rescan_required|schema_quarantined|schema_erased` plus non-comparable tombstone metadata and no schema bytes or content digest.
...
| E3-T6 | Implement clamped cache hints, per-connection discovery generations/lifecycle epochs/CAS publication, durable forced reconciliation, observations, and change handling | M | E3-T5, E2 | Only the latest-started matching configuration, policy, and enabled lifecycle epoch can publish/restore active; older or disable-invalidated completion is discarded; durable <=5-minute recovery, worker loss, A→B→A, removal, and one-of-100 drift pass |
...
| 05 | `feat(registry): add entries connections and immutable snapshots` | Context/scheme-qualified `SafeUrl`, inert schema-URI references, encrypted schema payload/key and scan-attestation models, explicit output classification, migrations, lifecycle, canonical normalization and keyed identities | 03, 04 | HTTPS-only schemes, no ordinary schema digest, current schema-policy gates, key lifecycle/non-comparability, missing/changed output class, local-ref, no-external-fetch, and versioning tests pass |
...
| 07 | `feat(mcp): connect and discover remote Streamable HTTP servers` | Scheme-qualified `SafeUrl`, method-aware redirects, SDK/negotiation, address-pinned transport, paginated tools, policy-versioned schema scan/rescan, discovery generations/lifecycle epochs/CAS publication, reconciliation, health | 05, 06 | Dual-era, schema-policy invalidation, overlapping-refresh last-start-wins, disable-vs-refresh invalidation, invocation-no-redirect, active-scheme, metadata-secret, staleness, rebinding, and SSRF suites pass |
...
| Schema bombs, low-entropy fingerprint oracles, or detector-policy drift | CPU/memory spikes, schema guesses can be tested, stale schema bytes remain visible, or a newly recognized credential survives retention | Schema bounds; per-hierarchy keyed identities with no ordinary schema digest; policy-versioned attestations; immediate read/invocation invalidation; encrypted schema quarantine and attested encryption/fingerprint-key erasure |
...
4. **ADR-004:** Immutable discovery snapshots, erasable keyed schema identities, non-comparable dependent content identities, and occurrence-generated version identity.
