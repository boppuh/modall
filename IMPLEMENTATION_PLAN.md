...
- artifact-upload qualification proves targets use only the sealed encryption gateway, bind subject/workspace/authorization epoch, reauthorize current upload permission at first byte/each bounded interval/precommit/completion within the inherited 60-second budget, fence leases on authorization change, never place plaintext in durable storage, give each upload its own externally erasable hierarchy, fit late-write cleanup within 15 minutes, and require key-destruction evidence for abandoned/revoked/retention-forbidden terminal states; unsupported backends or MCP hosts fail closed;
...
- quote/estimate expiry plus every candidate's immutable provider price-version ID, provider-native exact amount, workspace/evaluation base currency, immutable FX-rate-set/version and exact conversion factor when currencies differ, normalized exact amount, and worst-case reservation vector;
...
- `evaluation_stratum_id`, G1 attempt ID, above-threshold planning alternative, simulated complete-gate joint pass probability and Monte Carlo bound, stratum-specific maximum attempts, success/cost alpha allocations and cumulative spend, frozen policy/candidate versions, base currency and immutable FX rate-set/source/revision/timestamp/factor lineage, retired `g1_revealed` component IDs, paired bootstrap success interval, and cost-ratio upper bound grouped by frozen `correlation_cluster_id` using exactly one primary outcome and attributed native plus normalized cost per policy/task, plus separately labeled nested-repeat variability intervals that cannot affect activation;
...
| P0-08 Analysis | Baseline comparison, exact-cost/FX joint power, and stratum-scoped sequential bounds | Report preserves native/sub-minor costs and uses one frozen immutable FX set for every observed/bootstrap value; sizing has >=90% joint lower bound; no component/evidence reuse | Data/full-stack |
...
Phase 1 inherits the Registry Alpha authorization-freshness contract without resetting any timer: the conservative provider propagation bound `P`, qualified-source lookup allowance `R`, server snapshot TTL/poll interval `L`, and safety margin `M` must satisfy `P + R + L + M <= 60 seconds`. Privileged mutations, every initial/fallback dispatch fence, artifact grant mint/redemption, and every `read_artifact` chunk synchronously bypass the shared authorization cache and require `P + R + M <= 60 seconds`; timeout or incomplete source data fails closed. Upload-target mint, the encryption-gateway pre-first-byte and pre-ciphertext-commit checks, and completion do the same; a stream lasting beyond a configured authorization interval `U` pauses for another qualified check, with `P + R + U + M <= 60 seconds`. Each run/upload binds its originating actor and admitted authorization epoch. A dispatch fence requires a fresh qualified lookup plus locked current membership/execution-role/unchanged-epoch check, and an upload-write check requires current membership/upload permission, unchanged epoch, active upload/key, and an unfenced lease. Any change cancels unfenced execution and atomically enqueues fencing/abort of every open upload lease for that actor/workspace; gateway mismatch aborts the multipart/ciphertext commit, advances cleanup, destroys both upload subkeys, and returns no artifact. Server-authored snapshot/stream authorization expiry propagates to clients and gateways so caching cannot add another freshness interval. Phase 1 also preserves Registry Alpha's immutable MCP binding identity: negotiated protocol revision participates in every MCP capability-version digest, and a protocol switch creates a fresh `pending_review` version before routing or execution can use the changed wire contract.

Endpoints:

- `POST /v1/artifact-uploads` — synchronously reauthorize current membership/upload permission, bind originating subject/workspace and current authorization epoch plus an immutable server-derived source-classification floor/provenance-attestation version whose signed content identity/manifest is expected for this exact upload, provision/activate a unique external per-upload hierarchy with purpose-separated encryption/fingerprint subkeys, and return one dedicated short-lived `EphemeralUploadTarget` for the service-controlled streaming encryption gateway scoped to a unique create-only ciphertext object/version; a client declaration may raise but never lower the floor, and no target is minted before the hierarchy is active
- `POST /v1/artifact-uploads/{id}/complete` — synchronously reauthorize the bound subject/upload permission/unchanged epoch, then under the shared identity/provenance/artifact/policy lock order require server time strictly before `expires_at`, an active unfenced gateway lease whose last qualified check is within `U`, close the upload, verify the gateway-attested exact ciphertext version plus ephemeral client integrity proof and plaintext size/type, decrypt only into bounded scan quarantine, transiently canonicalize the plaintext and compare it with the decrypted signed expected provenance identity, then persist only the upload-hierarchy-keyed identity and comparison result. Require an attestation against the current output/artifact policy and MIME profile. Derive effective classification as the maximum of the immutable source floor, client declaration, and detector/profile result; if retention is allowed, the finalized artifact inherits the upload hierarchy and mints the authoritative `artifact://` URI without a plaintext rewrite. Authorization/provenance mismatch stays quarantined, returns no artifact, fences/aborts the upload, and enters key-destroying cleanup. Sensitive/failed content whose policy forbids retention enters `upload_erasure_pending`, returns no artifact, and becomes `upload_erased` only after both subkeys are destroyed and ciphertext cleanup is attested; an expired `open` row atomically moves to `cleanup_pending` with the same key-destruction requirement even if the sweeper is late
...
- `POST /v1/artifacts/{id}/access-grants` — authorize the current subject, workspace, trusted source floor/effective classification, retention, integrity, and exact current output/artifact scan attestation before mint or replay, then mint the sealed Alpha `ArtifactAccessGrantToken` bound to that subject and exact artifact version; stale provenance/scans return no token, ordinary replay storage retains only a non-secret reference/HMAC verifier, and the encrypted token envelope remains replayable through exact grant expiry and becomes logically unreadable then
...
- `GET /v1/artifacts/{id}/content` — require ordinary authentication plus the subject/workspace/version/authorization-epoch-bound grant in a redacted header and, on every redemption, reauthorize current membership/role, epoch, visibility, trusted source floor/effective classification, retention, integrity, and exact current output/artifact-scan-policy plus MIME-profile attestation before streaming through the isolated no-store path
...
`EphemeralUploadTarget` is a sealed outbound secret-capability type, not a `SafeUrl` or an ordinary URL-valued API field. It contains an upload ID, method, credential-bearing target, required headers, expiry, maximum bytes, plaintext checksum constraints, and no read/list or encryption-key authority; its verifier is bound to the originating subject, workspace, admitted authorization epoch, and exact upload. The service provisions a unique external per-upload hierarchy first, then mints the target only from the configured validated service-controlled streaming encryption-gateway origin for an exact downstream non-overwritable ciphertext key/version, with a maximum five-minute TTL and no redirects. Before first byte, at every server-authored `upload_authorization_expires_at` no more than `U` later during a long stream, and immediately before ciphertext commit/materialization, the gateway calls the qualified cache-bypassing authorization path and under the identity/upload locks requires current membership, upload permission, matching epoch, active hierarchy/upload, and unfenced lease. It durably records only safe check time/epoch/result, pauses while checking, and on timeout/mismatch fences and aborts the lease, transitions cleanup, destroys both subkeys, and cannot commit or complete an artifact. Identity/permission mutations also enqueue lease fencing under the same lock order. While authorized, the gateway streams through bounded non-swappable memory, computes the purpose-separated keyed plaintext fingerprint, emits chunk-authenticated ciphertext directly to an uncommitted multipart/version, and wipes buffers; it never spools plaintext to disk, queues, logs, traces, crash dumps, or caches. Direct browser/object-store presigned plaintext upload is not allowed. The target appears only in the authorized creation response or a same-key/same-fingerprint idempotency replay with `Cache-Control: no-store`; replay reauthorizes the same subject/epoch and its encrypted exact-lifetime envelope follows the inherited prepare/commit/promotion protocol without a content key. A backend qualifies only if its gateway/storage path supplies a conservative maximum post-expiry or post-authorization-fence in-flight materialization window `W` plus read-after-write/list visibility bound `C`, supports lease cancellation/hard deadline and precommit authorization, and guarantees `W + C` plus cleanup retry fits the 15-minute deletion SLO.

The precommit authorization transaction is the write linearization point: while holding the identity/upload locks it atomically consumes the open write lease and records one `ciphertext_commit_fenced` token bound to the current epoch, exact object/version, integrity, byte count, and authorization check. The gateway may perform the external ciphertext commit only with that token and accepts no later bytes. An authorization mutation locking first prevents the token; one locking afterward observes that ciphertext commit was already authorized but still marks the upload non-finalizable, so completion returns no artifact and key-destroying cleanup removes any materialized ciphertext. Lease recovery can never create or replay a second commit token.

Ordinary durable records retain only the opaque upload ID, bound originating subject/workspace/admitted authorization epoch, last qualified authorization check/expiry and safe decision, safe gateway/origin identifier, never-reused ciphertext key/version scope, opaque hierarchy handle/state, keyed plaintext fingerprint, ciphertext integrity, lifecycle state `open|completed|cleanup_pending|upload_erasure_pending|upload_erased|deleted`, expiry, qualified `W`/`C`, authorization/write-fence/active-lease evidence, cleanup attempts, quiescence deadline, and key/deletion attestations—never plaintext, a content key, raw identity response, or ordinary plaintext digest. A gateway/provider event or ciphertext HEAD restricted to that key records the actual version even when the client never calls completion; create-only policy permits at most one version. Authorization mutation, gateway check failure, completion, and expiry cleanup lock the same identity/upload subsequence and compare database server time. Completion may finalize only an `open` row with `now < expires_at`, unchanged current authorization epoch, current upload permission, and a fresh successful gateway check; expiry or authorization loss atomically enters `cleanup_pending`, fences/cancels writes, and returns no artifact. The sweeper destroys both upload subkeys, aborts the session, and repeatedly deletes the exact ciphertext version through all leases and at least the later applicable expiry/fence time plus `W + C`; an object appearing during the interval restarts verification. `cleanup_pending -> deleted` requires attested key destruction, a closed/hard-expired lease set, elapsed quiescence, and provider-consistent absence. Completion-time retention rejection follows `open -> upload_erasure_pending -> upload_erased`, with immediate read/finalization suppression and terminal state only after both subkeys are destroyed; ciphertext deletion continues independently and failure remains alerted. A late object after terminal attestation triggers delete plus incident but is already undecryptable. A completed artifact can enter only its separate policy-reclassification erasure path, never abandoned cleanup. Application/proxy/CDN/browser/MCP-host capture is disabled or redacted. Browser and MCP clients stream directly to the non-navigating gateway primitive with no ambient cookies/referrer and immediate buffer disposal; unsupported MCP hosts receive no target. The target credential never enters generic URL/DOM/model/resource/database/audit/error/history/analytics/log/trace surfaces.
...
Artifact source sensitivity is independent of malware/secret/PII scanning and uses the ordered policy lattice `public < non_confidential < private_internal < restricted_sensitive`. Upload creation resolves a trusted `source_classification_floor` from either (a) a server-side repository connector attesting exact repository identity, visibility, commit, and canonical bundle/manifest digest, or (b) an approved immutable repository-source attestation signed by the designated data owner over the expected canonical artifact identity and classification version. Completion independently computes the uploaded content identity and requires an exact match; a caller cannot attach a public attestation to different bytes. An arbitrary, mismatched, or unverifiable upload has no trusted public provenance and inherits the workspace's `unverified_upload_floor`, which defaults to `private_internal`; a claimed attestation mismatch fails/quarantines completion rather than silently granting its lower floor. Only a service identity or subject holding the dedicated `artifact_provenance:attest` permission may create/change a source attestation, and the action is audited. The uploading client can declare a more restrictive class but cannot write the floor, provenance authority, or effective class. Completion derives `effective_artifact_classification = max(source_classification_floor, client_declared_classification, detector_or_content_profile_classification)`. A clean detector never proves public ownership or lowers proprietary source. The upload target, final artifact, route snapshot, and every attempt retain the exact provenance/content/classification versions. If a source or workspace floor is later raised, current-pointer comparison immediately blocks grant replay/redemption, read, routing, and unfenced dispatch until the artifact is reclassified under the new floor; no earlier universal/non-confidential eligibility survives.
...
- an incomplete upload becomes permanently non-completable at expiry or authorization loss, both upload subkeys are attested destroyed, and its exact ciphertext object version or multipart state is verified deleted within 15 minutes; create/replay, first-byte, periodic, precommit, and completion checks require current upload permission plus the bound authorization epoch, while permission/epoch mutation fences outstanding leases. Completion checks database server time under the same row lock, so an expired/revoked `open` row enters/stays on cleanup even when completion beats a delayed sweeper; authorization/completion/cleanup and late-materialization races have one safe winner, and cleanup failures remain retryable, visible, and alerted without touching finalized artifacts;
- a currently authorized subject can view safe text/JSON or download other content only through an active, unexpired subject/workspace/version/epoch-bound grant, exact current scan attestation, and required isolation headers; individual revocation or post-mint access/policy/retention/output-scan/MIME-profile change denies token replay and bytes immediately on the next authoritative check until a successful current-version rescan;
...
Phase 1 extends the Alpha shared lock-plan helper while preserving the relative order of every common class. A fence performs its qualified-source network lookup before taking database locks. After an API idempotency lock when applicable, coordinated transactions acquire: (1) actor user, workspace membership, and current authorization-epoch projection; (2) provider current-price and FX-rate-set/selectors; (3) stable server-connection or deployment rows; (4) exact/current target configuration and MCP discovery pointers/snapshots; (5) capability-status projections; (6) source-provenance/repository-classification and workspace unverified-upload-floor pointers; (7) artifact rows; (8) current schema-scan-policy, input-scan-policy, and output/artifact-scan-policy plus MIME-profile pointers/versions, ordered by policy kind then MIME; (9) workspace/provider/data/destination policy rows; (10) quota/budget accounts ordered by scope, currency, and unit; and (11) run, attempt, job, and reservation rows in that order. IDs within a class are sorted. Fence workers release lease/job locks before starting this transaction; identity, price/FX, disable, provenance/classification, artifact publication/access/rescan, scan-policy/profile activation, other policy, and quota mutations use only the same relevant subsequence. Advancing a provenance/classification or scan-policy/profile pointer atomically invalidates older eligibility/attestations, so activation never requires locking every artifact. Reverse-order input and opposite worker/control start tests fail on any timeout, deadlock, raw coordinated lock outside the helper, or order inversion.
...
- artifact reclassification, quarantine, or erasure supersedes labels that no longer have qualifying evidence and prevents their use in training/activation;
...
|---|---|---|---|
| P1-01 Identity | Workspaces, memberships, API keys, RBAC, authorization epochs, audit | Registry Alpha identity | End-to-end 60-second/cache-bypass matrix passes; every fence freshly reauthorizes the admitted actor and any membership/role/epoch change cancels unfenced work with no send |
| P1-01A Artifacts | Subject/epoch-bound encryption-gateway ingestion, periodic/precommit authorization, per-upload erasable hierarchy, non-overwritable ciphertext, trusted source floors, quiescent cleanup, scan/rescan/erasure, immutable URI, revocable grant | Identity, object storage | Auth loss fences upload leases and returns no artifact within the 60-second budget; no upload plaintext reaches durable storage; client cannot lower floor; expired/revoked/sensitive paths destroy both subkeys before terminal state; drift blocks grant/read/dispatch |
...
| P1-08 Telemetry | Traces, logs, metrics, exact-rate/FX reservation and reconciliation | All request paths | Trace chain covers route/outcome; immutable sourced FX versions preserve reproducible native/normalized sub-minor costs and residuals; stale liabilities alert |
...
- `routing_decisions`, `routing_candidates`, `routing_feature_snapshots`, with every live decision bound to the exact evaluated stratum and approved gate-evidence version or explicitly marked static/shadow-only; each priced candidate/selection records provider-native amount, base currency, immutable FX rate-set/rate/source/revision/timestamps, exact oriented factor, normalized amount, and quote context/expiry
...
- `artifact_source_attestations` with server-verified repository identity/commit or data-owner-approved source record, encrypted expected canonical bundle/manifest identity, immutable classification/version, authority, and audit lineage; `run_artifacts` as a visibility-domain/role-qualified link to immutable encrypted `artifacts` and append-only exact-content scan/current-erasure projections; `artifact_uploads` stores only opaque upload ID, originating subject/workspace/admitted authorization epoch, last qualified authorization check/expiry and safe decision, safe encryption-gateway/origin ID, unique ciphertext key/version, upload-key reference, keyed plaintext identity, ciphertext integrity, client declaration, trusted source-attestation/floor version, constraints, `open|completed|cleanup_pending|upload_erasure_pending|upload_erased|deleted` state, expiry, qualified `U`/`W`/`C`, authorization/write-fence/lease/cleanup/quiescence evidence, and key/deletion attestations—never plaintext, raw identity response/keys, an ordinary digest, or an `EphemeralUploadTarget` credential. `upload_content_keys` carries only the opaque external hierarchy handle, purpose/version, `provisional|active|destruction_pending|destroyed` state, activation/destruction outbox evidence, and recovery-copy destruction attestation; target/grant rows contain only non-secret metadata/HMAC verifiers, and inherited grant events/projections plus exact-lifetime encrypted envelopes retain only non-secret tombstones after expiry/erasure
- append-only `usage_reservations` linked one-to-one with a dispatch fence/attempt, plus exact provider-native and normalized decimal/rational reservation/commit/release/reconciliation events, raw usage dimensions, immutable provider-price and FX-set/rate references, base/quote currencies, factors/timestamps, settlement rules/residuals, and workspace/provider quota-budget balances
...
- Every upload is encrypted by the controlled gateway before first object-store persistence under a unique external hierarchy; durable systems retain only ciphertext, ciphertext integrity, opaque handle/state, hierarchy-keyed plaintext fingerprint, and safe authorization-check evidence. The target and row bind originating subject/workspace/admitted epoch. Before first byte, no later than every `U` during streaming, before ciphertext commit, on target replay, and at completion, a qualified cache-bypassing lookup plus locked comparison requires current membership/upload permission, unchanged epoch, active upload/hierarchy, and unfenced lease; `P + R + U + M <= 60 seconds`. Identity/permission mutations fence/cancel open leases through the same lock order. Every finalized artifact stores immutable trusted source-attestation ID/version, transiently verified expected versus computed canonical identity, and server-derived classification floor; ordinary expected/computed plaintext digests are not retained. A claimed attestation is usable only on exact byte identity. Effective classification is the maximum of floor, client declaration, and detector/profile result; missing provenance uses the workspace floor defaulting to `private_internal`. Client fields cannot lower policy, and current floor/attestation changes invalidate prior eligibility.
- Upload authorization failure, completion, content erasure, and expiry cleanup use one locked monotonic state machine and database server time. `open -> completed` requires `now < expires_at`, current bound authorization, a fresh successful precommit check, current clean scan/retention permission, and an active hierarchy. Authorization loss or expiry performs/preserves `open -> cleanup_pending`, permanently forbids completion, fences/cancels gateway writes, destroys both upload subkeys, and repeatedly deletes only the exact ciphertext version. `cleanup_pending -> deleted` requires attested key destruction, closed/hard-expired leases, the later applicable authorization-fence/expiry time plus `W + C`, and provider-consistent absence. Retention-forbidden completion or later reclassification uses `upload_erasure_pending -> upload_erased`; all reads/finalization stop at the first transition, and terminal erasure requires both subkeys destroyed even if ciphertext deletion is delayed. Cleanup/key failures remain retryable, visible, and alerted; a completed artifact cannot enter abandoned cleanup.
...
- Sensitive artifact access uses audited, short-lived grants bound to the authenticated subject, workspace, authorization epoch, and exact immutable artifact version; every mint/replay/redemption rechecks current membership/role, visibility, classification, retention, integrity, and exact current policy/profile scan attestation, so possession does not bypass or outlive ordinary authorization or scanner-policy changes.
...
- sealed `EphemeralUploadTarget` delivery only to the service-controlled streaming encryption gateway after its unique per-upload external hierarchy activates, bound to originating subject/workspace/authorization epoch, with five-minute maximum expiry, exact create-only non-overwritable ciphertext key/version and size/type/checksum scope, no redirects/read/list/key authority, cache-bypassing qualified authorization at mint/replay, first byte, intervals `U`, precommit, and completion under `P + R + U + M <= 60`, identity-change lease fencing, a qualified gateway write fence plus bounded post-expiry/revocation materialization/visibility window, bounded non-swappable plaintext memory with immediate wiping and no spool, no-store/non-recording transfer, query/form/header redaction, only exact-lifetime encrypted secret-store replay with no premature eviction, and no plaintext, raw identity response, or credential in application/object-storage metadata, database, queue, transcript, history, referrer, DOM, audit, error, log, trace, cache, backup, or analytics capture;
...
- gateway-attested ciphertext version plus ephemeral client integrity-proof/length verification, transient expected-versus-computed provenance comparison with only upload-keyed fingerprint/ciphertext-integrity persistence, upload-key destruction before abandoned or retention-forbidden terminal state, archive traversal/expansion protection, malware/type scanning, and versioned per-MIME metadata/embedded-content extraction plus all-page/frame render/OCR secret/PII classification with complete coverage before finalization/routing;
...
- upload hierarchy/gateway state transitions; target subject/workspace/epoch binding; `P + R + U + M` authorization expiry; first-byte/periodic/precommit/completion checks; permission-change lease fencing; ciphertext-integrity/keyed-fingerprint verification; and key-destruction-before-erasure/cleanup ordering;
...
- generated Python, TypeScript, and MCP interfaces match OpenAPI semantics, including decimal/rational money and immutable FX-version fields, subject/epoch-bound sealed encryption-gateway upload/access-grant handling, strict outcome envelopes/artifact references, unsupported-host failure, and bounded `read_artifact` parity;
- upload contracts expose client declaration plus opaque authorized provenance/hierarchy references and a sealed subject/workspace/epoch-bound gateway target, but never content keys, direct object-store/plaintext targets, raw identity responses, client-writable source floor/effective class, or ordinary plaintext digests; absent, cross-workspace, epoch-mismatched, permission-revoked, stale, unauthorized, or non-gateway paths fail closed, and abandoned/revoked/retention-forbidden terminal representations require hierarchy-destruction evidence;
...
- encryption-gateway upload/download authorization plus fake-clock cleanup: plaintext exists only in bounded gateway/scan memory, while object storage, database, WAL, replica, backup, queue, log, trace, and cache inspection sees ciphertext/keyed identity/opaque handle only. Target mint/replay, first-byte, every `U`, precommit, and completion checks bind the originating subject/workspace/epoch and current upload permission under `P + R + U + M <= 60`; deterministic barriers remove permission or advance the epoch before/during/after a chunk and on either side of precommit. A mutation winning before the one-shot commit fence prevents commit; a fence winning first permits only its exact ciphertext commit, accepts no more bytes, but later auth loss prevents artifact finalization and enters one cleanup path. Expiry/auth loss always returns no artifact, and a late ciphertext write cannot orphan. Cleanup destroys both upload subkeys, repeatedly deletes through the applicable fence/expiry plus `W + C`, survives worker/key-service failure, and reaches `deleted` within 15 minutes only after key destruction and quiescent absence; retention-forbidden content reaches `upload_erased` only after subkey destruction, with restored ciphertext undecryptable even if deletion lags;
- upload provenance fixtures show a malicious or mistaken client declaration cannot lower the server-derived source floor: verified public repository/commit and data-owner-approved records retain their exact attestation, arbitrary uploads default to the workspace `private_internal` floor, detector-clean proprietary source stays private, and later floor elevation blocks grants, reads, routing, and unfenced dispatch;
...
- reverse-ordered IDs and opposite fence/control start orders prove Phase 1 actor/epoch, price, execution-target, source-provenance, artifact, scan/policy, quota, and run/reservation locks preserve the shared class order without timeout or deadlock;
...
- artifact upload through the service encryption gateway with a unique erasable hierarchy, trusted source floor, current-policy/profile-attested completion, authoritative URI issuance, authorized viewer/download grants, and rejection of underclassified, unsafe, mutable/overwritten, wrong-version, integrity-mismatched, stale-scan, unfinalized, expired, direct-object-store, or cross-workspace artifacts; durable-store/restore inspection proves plaintext is absent and sensitive/abandoned upload keys are destroyed before terminal state;
- artifact grants are carried only in redacted headers and never appear in URLs, access logs, traces, history, or referrers;
- access-grant and upload-target mint/replay crash and fake-clock matrices prove the encrypted envelope is durably prepared before the domain/idempotency/outbox commit, same-key recovery reauthorizes the bound subject/epoch before promoting/replaying exactly that credential at `expires_at - epsilon`, authorization loss/epoch mismatch returns no target and fences its lease, the envelope becomes logically unreadable at expiry, backing storage never evicts it prematurely, rollback/uniqueness orphans expire inaccessible, and missing committed or expired envelopes fail without reminting; ordinary rows retain only HMAC verifiers/non-secret references;
...
- individual grant revocation advances the event-backed projection, suppresses mint replay, and denies every later redemption even if envelope erasure is delayed; membership/role removal, authorization-epoch change, visibility/classification/retention-policy revocation, output/artifact scan-policy or MIME-profile change, quarantine, or integrity-state change likewise denies token replay and artifact bytes with an otherwise valid grant until a current-version rescan succeeds;
- HTTP upload targets point only to the service encryption gateway and are returned only on authorized creation or matching reauthorized idempotency replay with no-store; tests prove subject/workspace/epoch binding, current upload permission at first byte/each `U`/precommit/completion, immediate lease fencing on detected authorization loss, exact-lifetime replay availability at `expires_at - epsilon` only while authorized, immediate denial at auth/expiry boundary, no premature envelope eviction, no direct plaintext/object-store path, per-upload hierarchy activation before first byte, ciphertext-only durable writes, and credential/plaintext redaction or disabled capture across app, proxy, CDN, object-store audit, database/WAL/replica/backup, queues, logs, traces, browser navigation/referrers/history/DOM, errors, caches, and analytics. Redirect, expiry, revocation, epoch mismatch, overwrite, over-size, wrong-type/checksum, and broader-authority attempts fail closed; completion after auth loss/server expiry returns no artifact even before the sweeper runs, a formerly authorized write that materializes after cleanup's first absence check is fenced/observed/deleted before quiescent terminal attestation, and uncompleted exact ciphertext versions remain non-completable and reach `deleted` within the 15-minute SLO only after subkey destruction across crash/retry/completion races;
...
- **Part-time secondary Swift reviewer:** blinded 20% calibration base plus targeted units needed for every scored family's 10-present/10-absent reliability floor, all disputed high/critical findings, and adjudication support
...
| Revoked upload target remains writable | Ciphertext chunks or a finalized artifact appear after membership/permission/epoch change | Bind target to subject/workspace/epoch; cache-bypassing checks at first byte/each `U`/precommit/completion; authorization mutations fence leases; key-destroying cleanup and late-write quiescence tests |
...
- an authorized HTTP client, or an MCP host with a non-recording upload-target channel, can stream a repository snapshot and diff through the service encryption gateway under a subject/workspace/epoch-bound target and unique erasable hierarchy and receive finalized workspace-scoped `artifact://` identifiers pinned to immutable ciphertext versions; current upload permission is rechecked within the inherited 60-second budget through precommit/completion, authorization loss fences the lease and returns no artifact, no durable plaintext/direct object-store upload path exists, abandoned/revoked/retention-forbidden uploads become terminal only after key destruction, and an unsupported MCP host fails closed without receiving credentials;
...
- the exact decision, version, attempts, provider-native and normalized costs, immutable FX source/rate/set/factor/timestamps, artifacts, and evidence are traceable and replayable without consulting a latest rate;
