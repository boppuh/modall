...
- Every retained result/artifact remains releasable only while its exact-content scan attestation matches the current output/artifact-scan-policy and applicable MIME-profile versions. A change immediately yields `rescan_required` and blocks inline reads, publication, grant mint/replay/redemption, artifact bytes, and later dispatch consumption until a same-content current-version rescan succeeds. Non-text bytes remain quarantined until detected type, malware, archive safety, and the allowlisted type's complete content-aware extraction/OCR/secret-PII classification plan pass; executables, encrypted/uninspectable content, unsupported types, mismatches, incomplete coverage, and scanner failure cannot be published or downloaded.
...
- Run admission and every identity/authorization-epoch update, connection disable, material configuration change, capability-state change, discovery-publication/observation replacement, input-scan-policy change, output/artifact-scan-policy or MIME-profile change, and connection-policy update acquire the affected rows/advisory locks in one documented global order. Where client idempotency applies, its scoped key lock precedes the lifecycle lock sequence; worker fencing, result publication/rescan, and artifact access use the same relevant subsequence without an idempotency lock. Admission inserts its run/queue row before releasing lifecycle locks. Therefore a control mutation that locks second must observe and cancel the admitted undispatched run, while admission that locks second must observe the new state and reject before enqueue; there is no read/enqueue gap or inverted lock order. An output/artifact policy activation need not enumerate artifacts: advancing its locked current-version pointer immediately makes every older scan attestation ineligible for publication, replay, grant, redemption, or dispatch consumption.
...
5. current discovery-publication/observation pointers and referenced `discovery_snapshots`, ordered by connection, generation, then snapshot ID;
6. `capability_status_projections`, ordered by capability-version ID;
7. affected `artifacts`, ordered by artifact ID;
8. current input-scan-policy and output/artifact-scan-policy plus MIME-profile pointers/versions, ordered by policy kind then MIME;
...
- After scanning and output-schema validation, derive `effective_output_classification = max(declared_input_classification, connection_version.minimum_output_classification, detector_or_content_profile_classification)` in the fixed policy lattice; no detector-clean result inherits a lower class merely from its input. Only an effective `public` or `non_confidential` result that policy permits may be persisted inline or with an ordinary content digest. An effective `restricted_sensitive` result—whether raised by a match or required by the connection minimum—is never stored inline: when policy explicitly permits an encrypted restricted artifact, identify it only with an opaque ID and a versioned HMAC fingerprint whose key remains in the secret manager outside ordinary storage; when policy forbids preservation, discard the raw value and retain no content-derived digest or fingerprint—only non-reversible classification/rule IDs, detected type, size, scanner-profile/coverage status, and typed safe audit metadata. Return only a redacted placeholder and safe metadata to ordinary run APIs.
...
- Persist an append-only scan attestation over the exact result/artifact content identity, detected MIME, output/artifact-scan-policy version, applicable MIME-profile version, classification, and complete coverage counters. A result or artifact is releasable only while that attestation exactly matches the locked current required policy/profile pointers. Activating any changed policy or applicable profile immediately makes older attestations logically `rescan_required` and suppresses inline result reads, publication, grant mint/replay/redemption, artifact reads, and later dispatch consumption without waiting for a fan-out update. A durable job may rescan retained bytes in quarantine and append a superseding attestation only if a commit-time lock recheck proves the scanned content identity and policy/profile versions are still current; unavailable bytes, scanner failure, or newly disallowed content remains quarantined or is discarded under policy. A read authorization that linearizes after policy activation therefore returns no bytes; an activation that locks after authorization cannot retroactively reclaim bytes from that already-authorized response.
...
| `discovery_snapshots` | Immutable normalized and sanitized discovery result, content-deduplicated independently of observation time | Append-only |
...
| `scan_policy_versions` | Immutable bounded input rules, output/artifact rules, and per-MIME scanner-profile definitions with release compatibility metadata | Append-only |
| `scan_policy_pointers` | Current input, output/artifact, and per-MIME profile version/generation selectors | Compare-and-set under shared lock plan |
...
| `run_attempts` | Exact dispatch attempt, receipt, and output-scan decision plus exact output-scan attestation | Append-only events/status |
...
| `artifacts` | Clean content-addressed artifact metadata, or opaque restricted-sensitive metadata with a secret-manager-keyed plaintext fingerprint and ciphertext integrity checksum, pinned to an immutable object version plus current scan-status projection | Immutable content identity; compare-and-set scan projection |
...
- Every run attempt persists its pre-send dispatch fence, exact per-run-keyed clean-input fingerprint reference/state and scan-policy version/decision, nullable credential-binding version, optional provider receipt, reconciliation state, output classification, exact output-scan-policy/profile attestation and status, and result-fingerprint kind plus nullable digest/HMAC key version. A rejected sensitive input is never a run attempt, a reclassified request's fingerprint becomes cryptographically non-comparable, a stale output attestation suppresses result reads, and a discarded sensitive result has no content-derived fingerprint.
...
- Every retained result/artifact has an append-only scan attestation bound to its exact immutable content identity, current required output/artifact-scan-policy version, and applicable MIME-profile version. All result/artifact release paths compare that attestation to the current locked pointers; mismatch is authoritatively `rescan_required` even before its projection is materialized, returns no bytes/token, and can become ready only through a current-version rescan commit.
...
- `GET /v1/runs/{id}` — immutable request while retained plus current status and safe result metadata; during or after request security erasure, return only `request_content_state=erasure_pending|erased`, safe reason metadata, and no arguments. A retained inline result whose scan attestation no longer matches the current output policy/profile returns only a `rescan_required` placeholder and no result content.
...
- `GET /v1/artifacts/{id}` — return authorized metadata, classification, immutable object-version identity, current scan eligibility/readiness, permitted viewer modes, and a clean-content digest only when policy allows. A scan-attestation mismatch reports safe `rescan_required` metadata and no releasable viewer mode. Restricted plaintext fingerprints remain internal and are never returned by ordinary APIs.
...
| E3-T5 | Implement paginated tool discovery, recursive whole-schema secret/PII classification, and schema bounds | L | E3-T4 | Every decoded key/leaf is scanned before any persistence; description/default/example/custom-annotation matches plus scanner/limit failures reject the complete attempt, degrade/block dispatch, and leave no schema-derived state; 100-tool, cursor-loop, duplicate, and schema-bomb tests pass |
...
| E5-T2 | Implement run/admitted-authorization identity, request-key erasure, attempt/event/reconciliation, output classification, artifact scan attestations/status, and grant models | L | E1, E2-T5 | Actor/epoch and effective output are durable; requests/fingerprints use erasable subkeys; every terminal, erasure, scan/quarantine, grant, key, and reconciliation state has replayable lineage |
...
| E5-T7 | Quarantine; policy/profile-versioned output classification and attestation; malware/type/archive plus per-MIME extraction/render/OCR; invalidation/rescan; validate, redact, and store results | L | E5-T2, E0-T4 | Detector-clean proprietary output remains restricted; policy/profile drift immediately blocks old attestations until current-version rescan; sensitive or incomplete/invalid/scanner-failed content cannot publish |
...
| 05 | `feat(registry): add entries connections and immutable snapshots` | Context/scheme-qualified `SafeUrl`, inert schema-URI references, explicit connection output classification, core registry migrations, repositories, lifecycle, canonical hashing | 03, 04 | HTTPS-only navigable/endpoint schemes, missing/changed output class, local-ref, no-external-fetch, and versioning golden tests |
...
| 07 | `feat(mcp): connect and discover remote Streamable HTTP servers` | Scheme-qualified `SafeUrl`, method-aware redirects, SDK/negotiation, address-pinned transport, paginated tools, scan, discovery generations/CAS publication, reconciliation, health | 05, 06 | Dual-era, overlapping-refresh last-start-wins, invocation-no-redirect, active-scheme, metadata-secret, staleness, rebinding, and SSRF suites pass |
...
| 10 | `feat(jobs): add durable jobs encrypted run ledger and erasure keys` | Leases, events, actor/admitted epoch, attempts, idempotency, per-run keys, shared identity-aware lock plan, output/artifact attestations, grant projections | 03, 04, 08 | Crash recovery, actor/epoch lineage, no-compare erasure, scan/grant replay, key lifecycle, and lock-order tests pass |
| 11 | `feat(invocation): execute MCP tools with bounded inputs and results` | Fresh fence authorization, input scan/erasure, single-use lease, health dispatch, no redirects, policy/profile-bound output scan/rescan, cancellation, artifacts | 07, 10 | Actor removal, redirect, breaker, scan-policy, and erasure races fail safely; stale outputs release no bytes; fault, binary, malware/archive, and integrity suites pass |
| 12 | `feat(api): expose registry capability and run APIs` | REST resources, stable Run/event contracts, scanned idempotency/preflight, body-based registry search, prepared access-grant envelopes, current-scan reauthorization, auth epochs, generated client | 08–11 | OpenAPI/client fixtures, search leakage, admission/policy races, envelope/revoke crash matrix, stale-scan denial, grant/input secrecy, and API E2E pass |
...
| 14 | `feat(web): add capability catalog playground and runs` | Version detail, JSON editor, preflight confirmation, safe artifact access/revocation, run polling/timeline | 11–13 | Playwright scenarios C–E plus individual grant-revocation denial |
