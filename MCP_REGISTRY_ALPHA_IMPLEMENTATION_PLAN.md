...
- A tool schema containing valid local `$ref` fragments preserves each exact decoded string value, hashes deterministically under canonical JSON, and is invocable without network resolution; external schema references remain visible but non-invocable and are never fetched.
...
- Tool arguments matching a credential, token, secret, high-entropy detector, or disallowed policy-defined PII—and any input scanner failure—are rejected before request persistence or enqueue and again before dispatch. Initial rejection retains no argument, digest, or fingerprint. If a later scan-policy version rejects a previously queued clean request, the rejection transaction first scrubs every persisted/cached request copy and then retains only its existing keyed fingerprint plus non-content-derived provenance; neither path sends provider data. Server authentication uses an opaque credential binding instead.
...
- Every navigable or source URL-valued field—including connection endpoints, redirects, imported source locations, upstream search results, and retained resource links—must cross the one typed `SafeUrl` parser/sanitizer before persistence, audit, error interpolation, logging, tracing, or API output. It rejects userinfo, fragments, ambiguous/double encoding, credential/token/signature/secret/high-entropy-shaped decoded path segments, and non-allowlisted or credential-shaped query names/values. Raw URL secrets never leave the bounded parsing boundary. JSON Schema `$id`, `$schema`, and `$ref` strings instead use the inert schema-URI-reference contract in Section 6.2; they are never passed to transport or `SafeUrl`. Registry Alpha does not issue delegated upload URLs; Phase 1's separately typed, short-lived `EphemeralUploadTarget` is a sealed outbound secret capability and is never accepted or represented as `SafeUrl`.
...
- Parse JSON Schema `$id`, `$schema`, and `$ref` strings as bounded `InertSchemaUriReference` values under RFC 3986/JSON Schema 2020-12 syntax while preserving their exact strings in the canonical schema. Local fragments such as `#/$defs/item` resolve only inside the same immutable document. Relative or absolute external references are preserved and marked `external_ref_unresolved`, never fetched; a schema requiring one is visible but non-invocable with stable reason `mcp_schema_external_ref_unsupported`. Credential/secret/control-character scanning runs before persistence and rejects the whole tool snapshot safely rather than rewriting a reference. These inert values never enter HTTP clients, browser navigation, resource links, logs, or `SafeUrl`.
...
- Run creation repeats the bounded input scan under the current immutable scan-policy version and computes the clean canonical request plus versioned HMAC request fingerprint, never an ordinary input digest. It then performs the idempotency lookup/lock and fingerprint comparison before validating whether the submitted preflight is still unconsumed; an authorized matching record returns its replay representation without another token consumption. Only the no-record branch acquires the same lifecycle/configuration/observation/policy locks used by control mutations, rechecks under those locks, and atomically consumes the preflight while persisting the request/run/idempotency/queue rows. Immediately before fencing, the worker scans the stored arguments again and the fence transaction binds that clean decision to the current scan-policy version. A changed version forces a fresh scan. If it newly rejects the input, one fail-closed transaction marks request content unavailable to readers, nulls/tombstones canonical arguments in the run and job/queue payload, deletes any cached copy, replaces any idempotency replay payload containing arguments with the same run reference plus a `request_scrubbed` representation, appends the security-erasure/run event, and only then finalizes `dispatch_eligibility_revoked`; it retains only the existing versioned HMAC request fingerprint plus non-content-derived policy/rule/size metadata. Failure to complete every scrub keeps the run quarantined and non-dispatchable, and all read paths suppress its arguments.
...
- Clean, output-schema-valid results may be persisted under the declared run classification with an ordinary content digest. A sensitive match upgrades the output classification and is never stored inline: when policy explicitly permits an encrypted restricted artifact, identify it only with an opaque ID and a versioned HMAC fingerprint whose key remains in the secret manager outside ordinary storage; when policy forbids preservation, discard the raw value and retain no content-derived digest or fingerprint—only non-reversible classification/rule IDs, detected type, size, and audit metadata. Return only a redacted placeholder and safe metadata to ordinary run APIs.
...
| `runs` | Operator-requested direct MCP invocation with clean canonical arguments while retained, accepted input-scan decision/version, exact Alpha `server_connection_version_id`, and monotonic retained/scrubbed request-content state | Append/supersede status; security scrub may only null content and append evidence |
...
| `artifact_access_grants` | Short-lived, subject/artifact-version-bound authorization metadata plus versioned HMAC token verifier; never the bearer token | Expiring append-only |
...
- A dispatch-time security reclassification overrides normal input retention: before terminal rejection it atomically scrubs run, queue/job, idempotency replay, and cache copies and leaves only the keyed request fingerprint plus non-content-derived erasure evidence.
...
- `GET /v1/runs/{id}` — immutable request while retained plus current status and result metadata; after a security scrub, return only `request_content_state=scrubbed`, safe reason metadata, and no arguments.
...
- `POST /v1/artifacts/{id}/access-grants` — after current subject, workspace, classification, and retention checks, mint a sealed `ArtifactAccessGrantToken` bound to that subject and exact artifact version. The token appears only in this authorized no-store response or a matching same-key idempotency replay; the ordinary ledger stores a non-secret grant ID/reference, the grant row stores a versioned HMAC verifier, and the full token lives only in an encrypted secret-store replay envelope erased at the grant's short expiry. It is excluded/redacted from request/response body capture, logs, traces, audit payloads, errors, analytics, browser history/DOM/referrers, and caches. The grant is not sufficient without the same authenticated subject, allowing content retrieval to remain a safe `GET`.
- `GET /v1/artifacts/{id}/content` — require normal authentication plus the bound grant in the redacted `X-Modall-Artifact-Grant` header, never a URL, and stream only the exact immutable object version through the isolated artifact path. Clean text and JSON are parsed, escaped, and rendered in the sandboxed viewer; only explicitly allowlisted non-text types that completed all byte-level scans are attachment-only. Active, unknown, unsupported, type-mismatched, or quarantined artifacts return no content bytes. Ingress, proxy, application, and trace configuration must redact the grant header. Responses use `Cache-Control: no-store`, `X-Content-Type-Options: nosniff`, restrictive sandbox CSP, and same-origin/cross-origin isolation headers.
...
- Safe viewer for text and structured JSON plus authenticated short-lived artifact access. Only explicitly allowlisted, fully byte-scanned non-text media is download-only; active, unknown, unsupported, type-mismatched, or quarantined content exposes safe metadata/placeholders only and cannot be viewed or downloaded.
...
| ID | Task | Size | Depends on | Acceptance |
|---|---|---:|---|---|
| E2-T1 | Implement `SafeUrl` for navigable/source URLs, `InertSchemaUriReference` for JSON Schema keywords, plus registry entry/source models | M | E1 | Local fragments survive exactly without transport access; external refs are inert/non-invocable; only canonical `SafeUrl` reaches navigable URL types |
...
| E2-T4 | Implement sanitized discovery snapshot persistence | M | E2-T2, E2-T3 | Duplicate snapshot is deduplicated; local schema references preserve exact canonical provenance; external refs never fetch; secret/token/cookie/PII fixtures never persist unsanitized |
...
| E3-T6 | Implement clamped cache hints, forced reconciliation scheduling/bypass, observations, and change handling | M | E3-T5, E2 | Huge/absent `ttlMs` and missing-notification tests still force a complete bypass within 10 minutes, append observations, mark disappeared tools unavailable, create a fresh pending generation on A→B→A rollback, and leave 99 unchanged versions enabled when one of 100 tools changes |
| E3-T7 | Implement connection health, reconciliation-staleness, and circuit-breaker state | M | E3-T4, E2-T2 | Fault injection drives deterministic states; a bypass age of 15 minutes degrades the connection and independently fails dispatch fencing until fresh complete discovery |
...
| E5-T2 | Implement run, attempt, event, receipt/reconciliation, output-scan, and immutable artifact/access-grant models | L | E1, E2-T5 | Every terminal, indeterminate, quarantined, and artifact-access state has complete lineage; grant rows store only versioned HMAC verifiers and secret replay envelopes expire with grants; fingerprint kind/key version is explicit; credentials are optional but exactly versioned when present |
| E5-T3 | Implement idempotency-first replay, lock-serialized run admission, single-use preflight consumption, role authorization, and fail-closed classification policy | M | E5-T2, E1 | After bounded scan/fingerprint, lookup precedes preflight validation; the no-record branch shares lifecycle locks with control mutations so admission is accepted-and-visible or rejected; a lost-`202` replay returns one run, while conflicts, expiry, sensitive input, or stale/disallowed state never enqueue another |
| E5-T4 | Implement bounded pre-persistence input secret/PII scanning and local-only JSON Schema validation | M | E2-T5, E0-T4 | Local references validate from the immutable document; unresolved external refs are non-invocable without network access; pathological schemas/scanner failures fail safely |
| E5-T5 | Implement MCP tool dispatch, lock-serialized dispatch-time eligibility/input rescanning and atomic request scrubbing, deadline, and evidence-aware cancellation | L | E3, E5-T1, E5-T2 | A newly sensitive queued input is scrubbed from run/job/idempotency/cache/API before terminal rejection; pre-fence cancel, stale discovery, cancellation, timeout, concurrent scan/policy/lifecycle change, and worker-loss tests preserve execution truth without leaking input |
...
| E5-T7 | Quarantine, byte/malware/type/archive-scan, classify, output-schema-validate, normalize, redact, and store results | L | E5-T2, E0-T4 | EICAR, executable, archive-bomb/traversal, type-mismatch, schema-invalid, secret/PII, scanner-failure, size, and active-content fixtures cannot publish unsafely; discarded low-entropy secrets leave no digest/fingerprint and preserved restricted artifacts use versioned HMAC fingerprints |
...
| ID | Task | Size | Depends on | Acceptance |
|---|---|---:|---|---|
| E6-T1 | Establish error, pagination, filtering, optimistic concurrency, and rotation-safe durable idempotency with sealed-secret replay envelopes | M | E0, E1 | Replay/conflict/expiry tests plus HMAC rotation never duplicate execution; credential responses never enter ordinary replay rows, and envelope expiry/missing keys fail closed |
...
| E6-T5 | Implement secret-scanning run preflight, no-store run/event/cancel, and authorized artifact-read endpoints | M | E5, E6-T1 | Non-dispatching clean confirmation, pre-persistence secret rejection, E2E invocation, immutable artifact access, grant-token secret-envelope replay/expiry, and authenticated-cache contract tests pass |
...
| E8-T6 | Implement retention/deletion jobs and audit evidence | M | E2, E5 | Expired test artifacts are deleted and recorded, while lifecycle streams backing any live/retained projection are exempt and replay remains complete |
...
| 10 | `feat(jobs): add durable jobs and run ledger` | Leases, events, attempts, idempotency, shared ordered lock-plan helper, artifact metadata | 03, 04, 08 | Crash-recovery and lock-order-helper tests |
| 11 | `feat(invocation): execute MCP tools with bounded inputs and results` | Pre-persistence input secret scanning, atomic queued-request scrubbing on policy rejection, input/output validation, lock-serialized dispatch, evidence-aware cancellation, byte-scanned immutable artifacts | 07, 10 | Input-secret non-persistence/scrubbing, dispatch/control-race, invocation-fault, invalid-output, secret-fingerprint, malware/type/archive, and artifact-integrity suites |
| 12 | `feat(api): expose registry capability and run APIs` | REST resources, fixed public Run status, forward-compatible event envelope, scan-before-hash idempotency, lock-serialized admission, run preflight, sealed access-grant replay, auth epochs, generated client | 08–11 | OpenAPI/old-client fixtures, admission/control races, grant-token and input-secret non-persistence, API E2E |
...
- JSON Schema 2020-12 fixtures with nested `$defs`/local `$ref`, inert `$id`/`$schema`, unresolved relative/absolute external refs, and secret-bearing refs; local schemas invoke, external schemas never fetch and remain non-invocable, and secret refs reject without persistence;
- pagination, huge/negative/absent cache hints, forced no-cache reconciliation without notifications, one-of-many tool change without unrelated churn, authentication, timeout, cancellation acknowledgement/loss, output-schema-invalid, and malicious binary responses;
...
- a network-spy fixture proves discovery, validation, UI rendering, and hashing never fetch absolute/relative JSON Schema references, while local fragment resolution remains deterministic;
...
- artifact access-grant same-key replay reads the encrypted TTL-matched envelope while live, stores only a non-secret grant reference/versioned HMAC verifier in ordinary rows, and returns replay-expired after envelope/grant erasure without minting a second grant;
...
- backdated lifecycle streams older than 90 days remain replay-complete while their projection or retained evidence exists, and retention never deletes an authoritative prefix without a verified checkpoint contract;
...
- fake-clock reconciliation proves untrusted TTLs cannot extend the local cap, missed bypass deadlines degrade/block dispatch, and successful complete bypass restores eligibility;
...
- low-entropy credential and scanner-failure argument fixtures leave no request row, idempotency payload, content-derived audit value, log/trace body, confirmation token, queue item, or provider send; when a scan-policy change newly rejects a previously persisted request, the rejection barrier proves run arguments, job/queue payload, idempotency replay, caches, and API reads are scrubbed/suppressed before terminal state and that partial scrub failure stays quarantined without send.
...
- Artifact access-grant mint/replay and header redemption expose no bearer token through database rows, audit/log/trace/error/body capture, caches, browser history/DOM/referrers, or analytics, and expired envelopes cannot replay.
- Sensitive or unscannable tool arguments never reach the upstream server; initial rejection never persists, and a policy change that newly rejects a queued clean run atomically scrubs all run/job/idempotency/cache/API copies before terminal rejection.
...
| Untrusted cache hints conceal remote tool drift | Reconciliation age grows or stale bindings remain enabled | Clamp server TTLs, force no-cache reconciliation, alert before the deadline, and degrade/block overdue connections |
