...
6. affected `artifacts`, ordered by artifact ID;
7. current input-scan-policy and output/artifact-scan-policy plus MIME-profile pointers/versions, ordered by policy kind then MIME;
8. current global/deployment and connection-policy pointers/versions, ordered by policy scope then ID;
9. affected `runs`, `run_attempts`, and `jobs`, in that order and then by ID.

Transactions skip irrelevant classes but never reorder them. They choose the documented row or advisory lock for each key, never both; multi-row sets are sorted before acquisition. A worker completes and commits lease claiming before beginning the fence transaction, so it carries no run/job lock backward into steps 2–8. Control mutations acquire steps 2–8 before locking queued rows in step 9. The helper rejects an out-of-order plan in tests and emits only bounded lock-class timing telemetry, never identifiers.
...
- The transaction that creates `dispatch_fenced` uses the relevant classes from steps 2–9 of the shared lock plan for connection/current health generation, configuration, discovery observation, capability status, any input artifacts, input-scan policy, global/connection policy, and attempt/job state. While holding them, require connection status `active`, circuit breaker `closed`, and a dispatch-eligible current health generation; revalidate the clean input-scan decision/current version, every referenced artifact's current scan attestation when applicable, live tool name, invocable capability projection, exact current connection-configuration version and its required minimum output classification, current present discovery observation/snapshot, `last_complete_bypass_at` younger than the 15-minute dispatch-staleness bound, and current input/output policy compatibility. Half-open probes use a separate non-run path. Write the fence only if every check passes. If breaker/degradation, scan-profile/policy change, other control change, or staleness wins first, fencing fails and no call occurs; if fencing commits first, the attempt is already in at-most-once uncertain-execution semantics. Health and policy mutations take the same ordered locks, so there is no unlocked check-to-fence gap.
...
| E1-T3 | Implement provider-specific OIDC authorization-freshness adapter/qualification, epochs, and local-development principal | M | E1-T2 | Add/remove conformance proves `P + R + M <= 60s`; derived cache TTL/polling also proves `P + R + L + M <= 60s`, privileged actions bypass the cache, and missing/static sources, incomplete refresh, exhausted budgets, or unqualified IdPs block closed-alpha deployment; local-principal builds are developer-preview-only |
...
| E3-T3 | Enforce the shared pre-log context/scheme-qualified `SafeUrl` boundary and safe HTTP transport with resolved-address pinning | L | E1-T5, E2-T1 | HTTPS-only deployed endpoint/source rules plus path/query/userinfo/fragment/encoding checks leave no unsafe URL; every hop repeats scheme/address/TLS validation; active-scheme, rebinding, SSRF, and downgrade tests pass |
...
| E5-T2 | Implement run, request-key hierarchy/erasure, attempt, event, receipt/reconciliation, output classification, immutable artifact scan attestations/status, and event-backed access-grant models | L | E1, E2-T5 | Effective output class is independently derived; retained requests/fingerprints use erasable subkeys; every terminal, erasure/non-comparable, policy/profile-bound scan, rescan-required/quarantine, grant, and key state has replayable lineage |
...
| E5-T5 | Implement MCP dispatch with single-use scanned-argument lease, health-eligible fencing, lock-serialized rescanning, cryptographic content/fingerprint erasure, deadline, and evidence-aware cancellation | L | E3, E5-T1, E5-T2 | Fence requires active/closed/current health and the exact scanned lease survives one send; newly sensitive input reaches terminal only after both subkeys are destroyed and replay is non-comparable; cancellation, timeout, breaker races, and worker loss preserve truth without leakage |
...
| 04 | `feat(identity): add workspace roles auth and audit foundation` | Qualified OIDC freshness adapter, derived cache deadline, privileged authoritative refresh, local-development auth, workspace repositories, RBAC, immutable secret-version adapter, typed/scanned control text, audit | 03 | IdP budget/cache/bypass and secret rotation matrices pass; credential/PII in names, tags, notes, or reasons rejects before state/audit persistence; unqualified OIDC remains developer-preview-only |
...
| 07 | `feat(mcp): connect and discover remote Streamable HTTP servers` | Shared scheme-qualified `SafeUrl`, SDK wrapper, negotiation, address-pinned transport, paginated tools, recursive whole-schema classification, TTL-clamped reconciliation, health | 05, 06 | Both protocol eras plus active-scheme, metadata secret/PII, zero-schema-persistence, cache-staleness, DNS-rebinding, and SSRF suites |
| 08 | `feat(registry): materialize capability versions from discovery` | Immutable occurrence-generated per-tool versions with exact protocol/trust binding, snapshot observations, status projections, drift policy | 05, 07 | Same-schema protocol switch and assurance downgrade create fresh review-required versions; one-of-100 and A→B→A observation/event tests pass |
...
| 10 | `feat(jobs): add durable jobs encrypted run ledger and erasure keys` | Leases, events, attempts, idempotency, per-run external encryption/fingerprint key handles, ciphertext/keyed-only request state, shared lock plan, output/artifact scan attestations, grant projections | 03, 04, 08 | Crash recovery, no-compare erasure, scan/grant replay, key lifecycle, and lock-order-helper tests |
| 11 | `feat(invocation): execute MCP tools with bounded inputs and results` | Input scanning, ephemeral decryption/rescanning, attested erasure, health-eligible dispatch, policy/profile-bound output scanning and rescan, cancellation, artifacts | 07, 10 | Breaker/scan-policy races and post-erasure enumeration fail safely; stale output attestations release no bytes; fault, binary, malware/archive, and integrity suites pass |
...
- provider-specific OIDC qualification and fake-clock live-token add/remove tests prove `P + R + M <= 60s` for authoritative checks and `P + R + L + M <= 60s` for cached decisions; a refresh just before provider propagation cannot extend access past the bound, privileged actions synchronously bypass cached snapshots, expired UI deadlines hide tenant data, group-less/static issuance-time responses and incomplete/failed refreshes fail closed, and an unqualified or budget-exhausted provider prevents closed-alpha deployment while the local-principal build reports developer-preview-only release metadata;
