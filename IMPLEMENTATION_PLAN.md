...
- `POST /v1/artifact-uploads/{id}/complete` — under the shared artifact/policy lock order, require server time strictly before `expires_at`, close the upload, verify the exact storage version's ephemeral client integrity proof, size, content type, and scan attestation against the current output/artifact policy and applicable MIME-profile versions, then persist kind-appropriate integrity metadata and mint the authoritative `artifact://` URI; an expired `open` row is atomically moved to `cleanup_pending` and returns no artifact even if the sweeper is late
- `GET /v1/artifacts/{id}` — return authorized metadata, current policy/profile scan eligibility/readiness, exact immutable storage-version identity, and a clean-content digest only when policy permits; a stale attestation reports safe `rescan_required` metadata and no viewer mode, never a restricted plaintext fingerprint or ambient object-store credential
- `POST /v1/artifacts/{id}/access-grants` — authorize the current subject, workspace, classification, retention, integrity, and exact current output/artifact scan attestation before mint or replay, then mint the sealed Alpha `ArtifactAccessGrantToken` bound to that subject and exact artifact version; stale scans return no token, ordinary replay storage retains only a non-secret reference/HMAC verifier, and the encrypted token envelope remains replayable through exact grant expiry and becomes logically unreadable then
...
- `GET /v1/artifacts/{id}/content` — require ordinary authentication plus the subject/workspace/version/authorization-epoch-bound grant in a redacted header and, on every redemption, reauthorize current membership/role, epoch, visibility, classification, retention, integrity, and exact current output/artifact-scan-policy plus MIME-profile attestation before streaming through the isolated no-store path
...
For result consumption without REST credentials, `read_artifact(artifact_uri, offset, max_bytes)` uses the authenticated MCP session to obtain a qualified current authorization snapshot and, under the shared artifact/policy lock order, recheck subject, workspace membership/role, authorization epoch, artifact visibility, classification, retention, exact immutable version/kind-appropriate integrity state, and an attestation matching the current output/artifact-scan-policy plus applicable MIME-profile versions on every call. It returns at most 256 KiB per chunk with total length, detected content type, a clean-content digest only when policy permits, and the next offset; restricted plaintext fingerprints remain internal. Safe text/JSON uses typed content, and only explicitly allowlisted bytes that completed their full current content-aware profile use base64. Repeated bounded calls can consume a large result, while stale or incomplete authorization, `rescan_required`, active, encrypted/uninspectable, unknown, unsupported, type-mismatched, coverage-incomplete, quarantined, expired, cross-workspace, integrity-failed, or policy-forbidden content fails closed. The tool never returns an ambient object-store URL or credential.
...
| P1-01A Artifacts | Non-overwritable fenced ingestion, expiry-driven quiescent cleanup, policy/profile-bound scan attestations and rescan invalidation, immutable artifact URI, and event-revocable sealed `ArtifactAccessGrantToken` | Identity, object storage | Expiry/quiescence and overwrite tests pass; any scan-policy/profile drift immediately blocks grant replay/redemption/read/dispatch until a same-content current-version rescan; individual revocation suppresses replay/redemption |
...
| P1-04 Runtime | Input scanning/encryption/content-and-fingerprint erasure, policy/profile-attested output scanning, adapter pools, quote/price freshness, dispatch eligibility, reservation, deadlines, fallback, circuit breakers | P0 adapters | Rejected input becomes terminal only after both subkeys are destroyed; stale output/artifact attestations release no bytes; every fence rechecks current health/artifacts/price and reserves worst-case atomically; expiry/repricing sends nothing and concurrency cannot overspend |
...
- artifact upload, current-policy/profile-attested completion, authoritative URI issuance, authorized viewer/download grants, and rejection of unsafe, mutable/overwritten, wrong-version, integrity-mismatched, stale-scan, unfinalized, expired, or cross-workspace artifacts;
