...
Persist the normalized task, original request, and source-derived routing features only as encrypted task content. Before any durable write, create one unique external per-task hierarchy with purpose-separated encryption and fingerprint subkeys for the original request, normalized task, and feature snapshot; a routed Run references that hierarchy and never copies those bytes into a second per-run envelope. Raw request bytes exist only in bounded no-capture scan/normalization memory, and feature extraction produces an immutable encrypted `task_feature_snapshot` tied to exact extractor, feature-schema, input-scan-policy, and feature-policy versions.
...
- taxonomy, feature-schema/policy, and feature-extractor versions;
...
The candidate/feature snapshot is append-only ciphertext, but its usability is not permanent: it inherits the per-task current-policy and erasure lifecycle in Section 7.2. Candidate identities, row count, filter outcomes, scores, and individualized estimates that depend on request/source features cannot be duplicated into ordinary decision, event, audit, or analytics columns. After task erasure, only the selected exact implementation, safe version IDs, locally authored reason, accepted quote/reservation lineage, and generic unavailable tombstone remain.
...
- `POST /v1/routes` — scan in bounded no-capture memory, provision the per-task external hierarchy, encrypt the original request, normalized task, and extracted feature snapshot before their first durable write, then create and persist a content-blind route-only decision; scan/policy failure retains no task payload or source-derived feature
- `POST /v1/routed-runs` — perform the same task-content admission or reference an authorized active task hierarchy, atomically route, authorize, create the canonical Run resource without copying task bytes into a per-run envelope, enqueue, and return `202 Accepted`
...
Routing admission scans raw input in bounded no-capture memory, activates the per-task hierarchy, normalizes and extracts features in quarantine, and encrypts all three content classes before committing the task/route transaction. The router decrypts a snapshot only into a single-use bounded `ScannedTaskLease` after checking its keyed identity and exact current input/feature-policy attestation, then wipes it after selection. Durable candidate and selection rows store only the content-blind allowlist from Section 7.2. Policy drift blocks route replay, dispatch, analysis, and training until a same-content rescan/re-extraction commits a current attestation; retention-forbidden content takes the whole task hierarchy through erasure.
...
- `routing_decisions`, `routing_candidates`, `routing_feature_snapshots`, with every live decision bound to the exact evaluated stratum and approved gate-evidence version or explicitly marked static/shadow-only. `routing_candidates` is one opaque encrypted/padded snapshot reference per decision, not item-level rows whose identities/count leak task-conditioned eligibility; candidate membership, filters, scores, individualized estimates, and every source-derived feature value are ciphertext under the task hierarchy, with only keyed identity and exact extractor/schema/input/feature-policy attestation outside. The decision envelope contains only selected implementation, safe policy/health/reason/version IDs, accepted quote/reservation lineage, and erasure state. Each allowed price record stores provider-native amount, base currency, immutable FX rate-set/rate/source/revision/timestamps, exact oriented factor, normalized amount, and quote context/expiry
...
- `task_instances` containing only the content-blind envelope, ciphertext/opaque references for original request and normalized task, hierarchy-keyed identities, current-policy attestation, and `active|rescan_required|task_erasure_pending|task_erased` lifecycle; `task_content_keys` contains the unique external hierarchy handle plus purpose/version and activation/destruction evidence for distinct original-request, normalized-task, feature, dispatch, and fingerprint subkeys, and routed Runs reference rather than copy this content
...
- Every routed task is encrypted before first persistence under one unique external hierarchy with purpose-separated encryption/fingerprint subkeys for the original request, normalized task, source-derived feature snapshot, and serialized dispatch arguments. A routed Run references that hierarchy and cannot copy content into run, route, job, event, audit, cache, training-manifest, or telemetry fields. The only durable plaintext task/routing fields are the fixed content-blind allowlist in Section 7.2; every count, bucket, identifier, URI, revision, string, hash, digest, and embedding derived from input/source belongs in ciphertext. Route/replay/dispatch/analysis/training requires active keys and an exact current input/feature-policy attestation. Policy drift immediately suppresses every use. Retention-forbidden change enters `task_erasure_pending`, invalidates derived dataset/model lineage, destroys all content/fingerprint subkeys and recovery copies, purges caches/materializations, installs non-comparable tombstones, and reaches `task_erased` only with destruction evidence; retained route selection metadata is not replay authority.
...
- upload contracts expose client declaration plus opaque authorized provenance/content-hierarchy/source-hierarchy references and a sealed subject/workspace/epoch-bound gateway target, but never content keys, direct object-store/plaintext targets, raw identity responses, source payload/signature/repository/commit/manifest identity, client-writable source floor/effective class, publicly verifiable identity oracle, or ordinary plaintext digests; absent, cross-workspace, epoch-mismatched, permission-revoked, stale, unauthorized, or non-gateway paths fail closed, and abandoned/revoked/retention-forbidden terminal representations require destruction evidence for both hierarchies;
...
- encryption-gateway upload/download authorization plus fake-clock cleanup: plaintext and decrypted provenance exist only in bounded gateway/scan memory, while object storage, database, WAL, replica, backup, queue, log, trace, and cache inspection sees ciphertext/keyed identity/opaque handle plus safe policy metadata only. Target mint/replay, first-byte, every `U`, precommit, and completion checks bind the originating subject/workspace/epoch and current upload permission under `P + R + U + M <= 60`; deterministic barriers remove permission or advance the epoch before/during/after a chunk and on either side of precommit. A mutation winning before the one-shot commit fence prevents commit; a fence winning first permits only its exact ciphertext commit, accepts no more bytes, but later auth loss prevents artifact finalization and enters one cleanup path. Expiry/auth loss always returns no artifact, and a late ciphertext write cannot orphan. Cleanup destroys every upload-content and source-identity subkey, repeatedly deletes through the applicable fence/expiry plus `W + C`, survives worker/key-service failure, and reaches `deleted` within 15 minutes only after dual-hierarchy destruction and quiescent absence; retention-forbidden content reaches `upload_erased` only after both hierarchies are destroyed, with restored artifact and provenance ciphertext undecryptable and keyed identities non-enumerable even if deletion lags;
