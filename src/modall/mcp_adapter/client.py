...
from modall.mcp_adapter.policy import EndpointPolicy, LimitedTransport, TransportLimits
...
class McpClientAdapter:
    ...
    async def discover(
        ...
        await self._endpoint_policy.validate(endpoint)
        ...
        inner = self._transport or httpx.AsyncHTTPTransport(retries=0)
    ...
    async def _discover(
        ...
        if _contains_obvious_secret(text) or (
            credential_text is not None and credential_text in text
        ):
                ...
                if key == "patternProperties" and isinstance(child, dict):
                    for pattern in child:
                        ...
                        try:
                            ...
                            return False
                if key == "$ref" and isinstance(child, str) and "://" in child:
                    return False
