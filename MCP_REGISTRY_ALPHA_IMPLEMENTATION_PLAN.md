...
- Concurrent scheduled, notification-triggered, and operator discoveries use monotonically allocated per-connection generations; only the latest-started request on the still-current configuration may publish, so an older late completion cannot replace newer tools, timestamps, cache state, or connection health.
...
- An imported official-registry entry retains its upstream name, version, HTTPS-only context-qualified `SafeUrl` source location, and versioned keyed payload fingerprint without retaining an unsafe/active-scheme raw URL, unsanitized metadata, or an ordinary digest that could act as a low-entropy secret oracle.
...
- Tool arguments matching a credential, token, secret, high-entropy detector, or disallowed policy-defined PII—and any input scanner failure—are rejected before request persistence or enqueue and again before dispatch. Initial rejection retains no argument, digest, or fingerprint. Retained clean arguments are envelope-encrypted under a unique externally erasable per-run key hierarchy with purpose-separated encryption and request-fingerprint subkeys; durable queues/replays/caches never duplicate plaintext or key material. If a later scan-policy version rejects them, dispatch and reads stop in `erasure_pending`; terminal rejection occurs only after both subkeys are verifiably destroyed, active storage is replaced by a generic non-comparable no-replay tombstone, and database/WAL/replica/backup/cache ciphertext and keyed fingerprints are unrecoverable. Neither path sends provider data. Server authentication uses an opaque credential binding instead.
...
- Minting, replaying, or redeeming an artifact grant reauthorizes current subject/workspace membership and role, authorization epoch, artifact visibility, classification policy, retention, exact current scan attestation, and exact-version integrity; revocation or scan-policy/profile drift after mint denies token replay and bytes even while the token is unexpired.
...
- OIDC mode cannot be enabled until a provider-specific adapter proves that the complete authorization path fits one 60-second end-to-end bound from an authoritative group/revocation change to Modall's decision. Qualification records the conservative IdP propagation bound, lookup allowance, and safety margin; any cached snapshot TTL/poll interval must fit only in the remaining budget, and privileged mutations, every initial/fallback dispatch fence, and artifact grant mint/redemption synchronously bypass that cache. Endpoint presence alone is insufficient. An unqualified provider blocks the deployable closed-alpha release. The audited local principal remains available only in explicit local-development mode, and any artifact using it is labeled developer preview rather than closed alpha.
...
- Official Registry search accepts its free-form term and opaque cursor only in a body-capture-disabled non-mutating POST. The application scans the decoded bounded term before caching or forwarding; matches/failure leave no query-derived state or upstream request. Browser/proxy URLs, history, referrers, telemetry, audit, errors, and cache keys never carry the raw term/cursor, and the adapter redacts the final upstream URL after encoding a clean term.
...
- Every operator-supplied free-form display name, tag, note, and reason crosses a bounded typed control-text boundary before domain mutation or audit creation. Reason identity is an enum; optional text and display metadata have per-field syntax/length limits and recursive secret/PII classification. A match or scanner failure rejects the request before state change, and audit payloads accept only these validated value objects, stable IDs/enums, and before/after version IDs—never arbitrary strings or configuration bodies.
...
- Role, membership, artifact-policy, visibility, classification, retention, authorization-epoch, output-scan-policy, or applicable MIME-profile changes between grant mint and redemption deny content on the next authoritative check; policy/profile drift requires a current-version rescan attestation before release.
...
- A tool content digest covers only that tool's normalized name, title, description, annotations, input schema, output schema, exact `server_connection_version_id`, negotiated protocol revision, `implementation_identity_assurance`, normalized identity source, and implementation revision when one is available. The complete discovery-snapshot ID/digest is provenance in a separate observation and never participates in every tool digest, so an unrelated tool addition, removal, or change cannot churn unchanged capability versions. Credential rotation, negotiated-protocol change, identity-source change, or assurance upgrade/downgrade therefore changes the content digest even when the advertised tool and implementation revision are byte-identical.
...
- Every binding records the negotiated protocol revision, `implementation_identity_assurance` (`pinned`, `declared`, or `unverified`), normalized identity source, and optional implementation revision; all four participate in version identity. A changed protocol revision, implementation revision, source, or assurance creates a new `pending_review` capability version and prevents a prior enablement or stale trust metadata from crossing the changed wire contract.
...
- A disabled or degraded connection does not accept new dispatches. Every transition that opens the circuit breaker atomically advances the connection/health generation, moves the connection to `degraded`, and cancels admitted but unfenced runs under the same connection-first lock order; a half-open state admits only the dedicated health probe, never an ordinary run. Degradation never stops recovery: a durable forced-reconciliation job remains scheduled for every degraded connection and retries complete no-cache discovery with bounded exponential backoff/jitter capped at five minutes until success or explicit disable; operators may also request a read-only bypass refresh.
...
- Run admission and every identity/authorization-epoch update, connection disable, material configuration change, capability-state change, discovery-publication/observation replacement, schema-scan-policy change, input-scan-policy change, output/artifact-scan-policy or MIME-profile change, and connection-policy update acquire the affected rows/advisory locks in one documented global order. Where client idempotency applies, its scoped key lock precedes the lifecycle lock sequence; worker fencing, schema/result publication/rescan, and artifact access use the same relevant subsequence without an idempotency lock. Admission inserts its run/queue row before releasing lifecycle locks. Therefore a control mutation that locks second must observe and cancel the admitted undispatched run, while admission that locks second must observe the new state and reject before enqueue; there is no read/enqueue gap or inverted lock order. A schema or output/artifact policy activation need not enumerate retained content: advancing its locked current-version pointer immediately makes every older attestation ineligible for schema/result reads, validation, enablement, admission, publication, replay, grant, redemption, or dispatch consumption.
...
8. current schema-scan-policy, input-scan-policy, and output/artifact-scan-policy plus MIME-profile pointers/versions, ordered by policy kind then MIME;
9. current global/deployment and connection-policy pointers/versions, ordered by policy scope then ID;
10. affected `runs`, `run_attempts`, and `jobs`, in that order and then by ID.
...
- Ordinary reads may honor an entry within that effective TTL. Independently, every active connection must complete a scheduled full no-cache reconciliation at least every 10 minutes with bounded jitter that never pushes the deadline later, regardless of the server hint or notification support. Before any scheduled, notification-triggered, or operator discovery performs network I/O, a short transaction locks the connection, monotonically increments `latest_started_discovery_generation`, captures that generation plus the exact current connection-configuration version, and releases all locks. After complete pagination and schema scanning, publication reacquires the shared connection/configuration/observation/capability/schema-policy locks and succeeds only through compare-and-set when the captured generation, configuration, and schema-policy version still equal their current values. A superseded completion is discarded with safe `discovery_superseded` evidence and cannot write a cache/snapshot/observation/capability/schema-attestation transition, update reconciliation timestamps, or move the connection to `active`; raw pages and provisional schema keys are destroyed. Thus a request started later always wins even if it finishes first, and a failed latest generation is recovered by a new generation rather than an older stale success. When a missed deadline or current-generation failure degrades a connection, the scheduler preserves one durable recovery job and keeps attempting complete bypass with bounded exponential backoff/jitter capped at five minutes until a winning successful pagination atomically restores current observations and `active`, or an operator explicitly disables the connection; lease recovery cannot silently drop this job. Operator refresh also bypasses the SDK/list cache. A bypass path performs a fresh upstream listing and replaces the cache only after the winning complete publication; if the SDK lacks a bypass API, use a fresh no-cache client instance/path.
...
- Persist only allowlisted protocol metadata in normalized snapshots. Unknown `_meta` and extension values are dropped by default; an explicitly enabled diagnostic capture is sanitized for secrets, tokens, cookies, credentials, and configured PII patterns before encrypted restricted storage. This metadata rule does not sanitize or exempt JSON Schema content, which follows the whole-document rejection rule below.
...
- Before normalization, hashing, caching, persistence, audit construction, logging/tracing, or API/UI mapping, recursively traverse the entire bounded decoded input and output schema and classify every member name and scalar leaf for credentials, tokens, secrets, high-entropy material, control characters, and policy-defined PII. This includes `title`, `description`, `default`, `examples`, `enum`, `const`, all definition/property content, URI keywords, and custom annotations. Discovery captures the exact current immutable schema-scan-policy version before scanning and publication locks/rechecks that pointer; a policy change during discovery makes the attempt non-publishable. A match, traversal-limit breach, or scanner failure rejects the complete discovery attempt with a stable safe reason, stores only non-content-derived rule/type/count telemetry, immediately degrades and blocks the connection, and leaves its prior snapshot historical rather than current/dispatchable; no partial or redacted schema, content-derived digest/fingerprint, cache entry, capability version, or diagnostic payload is retained. Only a completely clean schema is preserved exactly, envelope-encrypted from its first durable write under a per-schema external content key; ordinary snapshot/version rows contain only ciphertext or an opaque encrypted-content reference plus safe metadata. Schema-key preparation, publication commit, activation, rollback, and orphan expiry use the same external-key/outbox invariants as retained requests, and no schema is readable or invocable before key activation. For that clean schema, parse JSON Schema `$id`, `$schema`, `$ref`, and `$dynamicRef` strings as bounded `InertSchemaUriReference` values under RFC 3986/JSON Schema 2020-12 syntax while preserving their exact strings in the canonical schema. Local targets—such as `$ref: "#/$defs/item"` and `$dynamicRef: "#node"` with a same-document `$dynamicAnchor`—resolve only inside the same immutable document through the bounded local validator. Relative or absolute external targets through either applicator are preserved and marked `external_ref_unresolved`, never fetched; a schema requiring one is visible but non-invocable with stable reason `mcp_schema_external_ref_unsupported`. These inert values never enter HTTP clients, browser navigation, resource links, logs, or `SafeUrl`.
- Every accepted input/output schema receives an append-only attestation over its exact canonical content identity, schema-scan-policy version, bounded traversal coverage, and decision. Capability list/detail/version APIs, schema diffs, enablement, argument validation, run admission, and every dispatch fence lock the current schema-policy pointer and release/use no schema bytes unless the exact attestation matches. Activating a new schema policy therefore makes every older attestation immediately `schema_rescan_required`, makes affected versions non-invocable, and suppresses schema fields from APIs/UI without a fan-out race. A durable job may decrypt a retained quarantined schema and append a new attestation only when commit-time locks prove the same content identity and still-current policy. A clean rescan restores eligible historical display and, only when every other current-binding check passes, invocability; a sensitive, failed, unavailable-byte, or superseded-policy rescan remains quarantined and releases no schema. When current policy forbids retention, the job destroys the per-schema content key, replaces the active payload reference with an erased tombstone, and verifies that database/WAL/replica/backup ciphertext is no longer decryptable before finalizing erasure.
...
- Persist an append-only scan attestation over the exact keyed result/artifact content identity, detected MIME, output/artifact-scan-policy version, applicable MIME-profile version, classification, and complete coverage counters. A result or artifact is releasable only while that attestation exactly matches the locked current required policy/profile pointers. Activating any changed policy or applicable profile immediately makes older attestations logically `rescan_required` and suppresses result decryption/reads, publication, grant mint/replay/redemption, artifact reads, and later dispatch consumption without waiting for a fan-out update. A durable job may decrypt retained ciphertext only inside bounded quarantine, rescan it, and append a superseding attestation only if a commit-time lock recheck proves the same keyed content identity and still-current policy/profile versions. If current policy still permits retention, the ciphertext remains encrypted and only its authorized presentation/classification changes. If reclassification forbids retention, the job atomically enters `result_erasure_pending`, suppresses all reads and fingerprints, irreversibly destroys the result hierarchy's encryption and fingerprint subkeys across active/recovery copies, purges caches, replaces active ciphertext/fingerprint references with a generic tombstone, and finalizes `result_erased` only after destruction is attested; failure remains quarantined, unreadable, and alerted. Restored database/WAL/replica/backup ciphertext and keyed bits are then undecryptable and non-enumerable. A read authorization that linearizes after policy activation therefore returns no bytes; an activation that locks after authorization cannot retroactively reclaim bytes from that already-authorized response.
...
| `server_connections` | Stable connection identity, current status, and monotonic latest-started/published discovery generations | Audited mutable |
...
| `discovery_snapshots` | Immutable normalized and sanitized discovery result with envelope-encrypted schema payload references, content-deduplicated independently of observation time | Append-only safe metadata; schema payload erasure is separately attested |
| `discovery_publications` | Winning complete discovery generation, captured connection-configuration version, referenced snapshot, protocol, and publication/reconciliation timestamps | Append-only after generation/configuration compare-and-set |
...
| `schema_content_keys` | Opaque per-schema external key handle and provisional/active/destroy-pending/destroyed lifecycle, never key material | Activate once; destroy once with attestation |
| `capabilities` | Stable logical tool identity | Mutable display metadata |
| `capability_versions` | Immutable whole-schema-scan-clean discovered schema, content digest, occurrence generation, and binding identity; excludes lifecycle status | Append-only |
| `mcp_tool_bindings` | Version-to-exact-server-connection-version/tool binding including negotiated protocol revision | Immutable |
| `capability_observations` | Per-winning-publication present/absent observation linking each listed or previously current tool name to its capability version and snapshot | Append-only |
...
| `scan_policy_versions` | Immutable bounded schema, input, output/artifact rules, and per-MIME scanner-profile definitions with release compatibility metadata | Append-only |
| `scan_policy_pointers` | Current schema, input, output/artifact, and per-MIME profile version/generation selectors | Compare-and-set under shared lock plan |
| `runs` | Operator-requested direct MCP invocation with originating actor, admitted authorization epoch, envelope-encrypted canonical arguments while retained, opaque per-run content-key handle, accepted input-scan decision/version, exact Alpha `server_connection_version_id`, and monotonic `retained|erasure_pending|erased` content state | Append/supersede status; erasure may only tombstone ciphertext after verified key destruction and append evidence |
...
| `run_attempts` | Exact dispatch attempt, receipt, encrypted/keyed result reference and key handle, output-scan decision/attestation, and monotonic retained/erasure state | Append-only events/status; erasure tombstones content only after key destruction |
...
| `consumed_run_preflights` | Unique signed-token JTI, prospective run idempotency key, and run lineage inserted atomically with run creation; no copied content fingerprint | Append-only |
| `artifacts` | Immutable uploaded-input or encrypted-result artifact metadata pinned to an exact object version; every result artifact carries ciphertext integrity, a per-result keyed plaintext fingerprint/key handle, and current scan/erasure projections, never a persistent ordinary plaintext digest | Immutable content identity; compare-and-set scan/erasure projections |
| `artifact_scan_attestations` | Exact artifact/content identity, detected MIME, immutable output/artifact-scan-policy and MIME-profile versions, classification, coverage counters, decision, and supersession lineage | Append-only |
| `artifact_access_grants` | Short-lived, subject/workspace/artifact-version/authorization-epoch-bound metadata plus versioned HMAC token verifier; never the bearer token | Expiring append-only identity |
...
- Every discovery network attempt first allocates a monotonically increasing per-connection generation and captures the current configuration under lock. Publication compare-and-sets both values against the current connection; only the latest-started matching generation may append a `discovery_publication` plus observations/version transitions, point to a new or deduplicated snapshot, replace cache state, advance reconciliation time, or restore `active`. Superseded completion is non-publishable and raw pages are discarded.
...
- Every persisted schema payload is ciphertext under one external per-schema key and has an append-only exact-content schema-scan attestation. A current-policy pointer mismatch atomically makes the schema non-readable and the capability non-invocable; no API, validator, enablement, admission, or fence path may decrypt it first and check later. Rescan publication compares the schema identity and current policy under the shared locks. Policy-forbidden retention reaches `schema_erased` only after attested key destruction and payload-reference tombstoning; delayed destruction remains quarantined and unreadable.
- Every MCP tool binding references the exact `server_connection_version_id` and negotiated protocol revision; both participate in the capability-version content digest. Discovery snapshot identity is excluded from the tool digest and retained through `capability_observations`.
...
- Every run attempt persists its pre-send dispatch fence, exact per-run-keyed clean-input fingerprint reference/state and scan-policy version/decision, nullable credential-binding version, optional provider receipt, reconciliation state, output classification, exact output-scan-policy/profile attestation, opaque per-result hierarchy handle, ciphertext/integrity reference, keyed fingerprint, and monotonic retained/erasure state. A rejected sensitive input is never a run attempt, a reclassified request's fingerprint becomes cryptographically non-comparable, and every retained result is encrypted before its first durable write. A stale output attestation suppresses result decryption; policy-forbidden reclassification reaches `result_erased` only after both result subkeys are destroyed and active content/fingerprint references are generically tombstoned, while a discarded ephemeral result leaves no content-derived value.
...
- Artifact-grant status is event-authoritative. Mint atomically appends `minted` and creates the active projection; revoke locks the grant/projection, appends one idempotent `revoked` event with a typed safe reason code, advances the projection, suppresses credential-envelope replay, and enqueues envelope erasure. Expiry similarly appends `expired`. Token verification, mint replay, and redemption all require the locked/current `active` projection; a revoked/expired grant can never return its token or content, even if envelope cleanup is delayed.
- Every retained result/artifact has an append-only scan attestation bound to its exact immutable keyed content identity, current required output/artifact-scan-policy version, and applicable MIME-profile version. All result/artifact release paths compare that attestation to the current locked pointers and require its content hierarchy active; mismatch is authoritatively `rescan_required` even before its projection is materialized, returns no bytes/token, and can become ready only through a current-version rescan commit.
...
- Tool inputs and outputs: 14 days by default; input expiry destroys the per-run hierarchy and output expiry destroys the distinct per-result hierarchy before ciphertext/fingerprint tombstoning.
...
- Discovery metadata and immutable encrypted schemas: retained while current-policy attested and referenced by a retained run or by any current connection/capability observation, binding, or projection. Snapshot-digest deduplication never resets age, so cleanup protects an old byte-identical snapshot while a current observation points to it; deletion requires locked referential rechecks and destroys its schema payload keys before tombstoning. A schema-policy mismatch blocks decryption regardless of retention age.
- Large artifacts: 14 days, then tombstoned with the identifier appropriate to their kind (policy-permitted clean uploaded-input digest, restricted upload fingerprint, or per-result keyed fingerprint) and a deletion event retained; result-artifact expiry destroys its per-result hierarchy before tombstoning, and discarded sensitive content never gains an identifier.
...
- `POST /v1/registry/searches` — perform a non-mutating official-registry search from a bounded JSON body containing `query` and optional opaque `cursor`. Ingress/proxy body capture is disabled. The application scans the decoded query for secrets/PII before any canonicalization, HMAC-keyed cache lookup, audit construction, or upstream call; a match or scanner failure returns a stable safe error and leaves no query-derived state. Only a clean bounded query reaches the adapter, which encodes it at the final upstream hop while redacting the complete outbound URL/query from telemetry. The browser-facing URL, history, referrer, caches, errors, and safe audit metadata never contain the raw term or cursor. This non-mutating POST requires authorization but no `Idempotency-Key`.
...
- `GET /v1/capabilities/{id}/versions/{version_id}` — exact schemas and binding only when every schema has an attestation matching the locked current schema-scan policy; otherwise return safe `schema_rescan_required|schema_quarantined|schema_erased` metadata and no schema bytes.
...
- `POST /v1/runs` — hold the submitted request in bounded ephemeral memory, repeat the fail-closed input scan, derive the ephemeral preflight-verification value only for token validation, and acquire the scoped idempotency lock. If a record exists, reauthorize its actor/workspace: a live comparable record recomputes through that run's erasable fingerprint subkey and either returns the retained representation on a match or conflicts on a mismatch; erased/non-comparable or replay-expired state returns `idempotency_replay_expired` without comparison. Only if no record exists does the transaction acquire actor membership/current authorization epoch and the connection/configuration/discovery-publication/observation/capability-status/input-scan-policy/connection-policy locks in the documented order, validate the token against the identical clean request/key, require its JTI to be unconsumed, recheck current state, provision the unique provisional per-run encryption/fingerprint hierarchy, and atomically insert consumed JTI, admitted actor/authorization epoch, encrypted invocation/per-run-keyed fingerprint, idempotency record, and queue row before releasing the lifecycle locks. A concurrent insert conflict rereads the winner and uses the same replay branch; any unreferenced provisional hierarchy self-destructs. The endpoint requires a declared data classification and returns `202`; sensitive input or token reuse with another key fails without persistence or enqueue.
- `GET /v1/runs` — filter by status, capability, connection, actor, and time.
- `GET /v1/runs/{id}` — immutable request while retained plus current status and safe result metadata; during or after request security erasure, return only `request_content_state=erasure_pending|erased`, safe reason metadata, and no arguments. A retained logical-inline result is decrypted only after authorization and locked exact-current scan-attestation checks; mismatch or `result_erasure_pending|result_erased` returns only a safe placeholder and no result content.
...
Every privileged mutation, initial or fallback dispatch fence, artifact-access-grant mint, artifact redemption, and MCP `read_artifact` chunk performs a synchronous qualified-source lookup that bypasses the shared snapshot cache and fails closed on timeout, incomplete data, or refresh failure; it therefore spends no `L` budget. A fence also locks and compares the current membership/role/authorization epoch with the epoch stored at admission, and any change requires new confirmation rather than executing queued intent. Other identity-scoped requests may reuse a snapshot only until its server-authored `authorization_expires_at = source_checked_at + L`; refresh scheduling never extends that deadline without another successful source lookup. Session responses carry that absolute deadline, and the frontend hides tenant content at or before it using the qualification clock-skew allowance, so browser caching adds no separate staleness interval. Frontend query keys include subject, workspace, and the epoch, cached tenant data is never rendered until the current session check succeeds, and the client purges all identity-scoped query data on logout, token refresh, subject/workspace/epoch change, visibility regain with a stale session check, expiry, or any `401`/`403`. Qualified group, deployment membership/role, and policy changes advance the epoch so the next permitted response invalidates the prior cache.

### 8.5 Artifacts

- `GET /v1/artifacts/{id}` — return authorized metadata, classification, immutable object-version identity, current scan eligibility/readiness, permitted viewer modes, and a clean-content digest only when policy allows; for encrypted result artifacts that digest is computed transiently during an authorized decryption and never persisted. A scan-attestation mismatch reports safe `rescan_required` metadata and no releasable viewer mode. Keyed plaintext fingerprints remain internal and are never returned by ordinary APIs.
- `POST /v1/artifacts/{id}/access-grants` — after current subject, workspace, visibility, classification, retention, integrity, and exact current output/artifact-scan-policy plus MIME-profile attestation checks under the shared lock order, mint a sealed `ArtifactAccessGrantToken` bound to that subject, workspace, exact artifact version, and current authorization epoch through the Section 8 credential-envelope prepare/commit protocol. The token appears only in this authorized no-store response or a matching same-key replay; replay repeats the current scan-attestation check and a stale artifact returns no token. The ordinary ledger stores a non-secret grant/envelope reference, the grant row stores a versioned HMAC verifier, and the full token lives only in the prepared-then-committed encrypted envelope, remains replayable through the grant's immutable short expiry, and is logically unreadable thereafter even if physical deletion finishes later. It is excluded/redacted from request/response body capture, logs, traces, audit payloads, errors, analytics, browser history/DOM/referrers, and caches.
...
- `GET /v1/artifacts/{id}/content` — require normal authentication plus the bound grant in the redacted `X-Modall-Artifact-Grant` header, never a URL. On every call, obtain a qualified current authorization snapshot under the Section 8 freshness rule, resolve the verifier to the exact grant, require its current event-backed status projection to be `active`, and under the shared artifact/policy lock order recheck subject, workspace membership/role, token epoch against the current epoch, artifact visibility, classification policy, retention, exact immutable version/integrity, grant expiry, and that the artifact's scan attestation exactly matches the current output/artifact-scan-policy and applicable MIME-profile versions. Any stale authorization refresh, scan-attestation mismatch, `rescan_required`, revoked/expired projection, or post-mint authorization/policy change returns no bytes; possession never bypasses current authorization. Clean text and JSON are parsed, escaped, and rendered in the sandboxed viewer; only explicitly allowlisted non-text types that completed all content-aware scans are attachment-only. Active, encrypted/uninspectable, unknown, unsupported, type-mismatched, coverage-incomplete, or quarantined artifacts return no content bytes. Ingress, proxy, application, and trace configuration must redact the grant header. Responses use `Cache-Control: no-store`, `X-Content-Type-Options: nosniff`, restrictive sandbox CSP, and same-origin/cross-origin isolation headers.

Result publication writes only ciphertext to a unique non-overwritable object key or versioned bucket and records the exact storage version plus the per-result hierarchy handle, ciphertext integrity checksum, and purpose-separated keyed plaintext fingerprint; even initially clean result artifacts never persist an ordinary plaintext digest. When policy permits returning a clean-content digest, compute it transiently during an already-authorized decryption and do not persist it. A finalized artifact never follows a mutable key: reads verify the pinned version, ciphertext integrity, current scan attestation, and active result-key state before release, and a still-valid upload credential cannot replace finalized content.
...
- Detail view with description, server, immutable version ID, content digest, occurrence generation, annotations, status, and history diff; schemas/diffs appear only after a current-policy attestation check and otherwise show safe rescan/quarantine/erasure state.
...
| E1-T3 | Implement provider-specific OIDC authorization-freshness adapter/qualification, epochs, and local-development principal | M | E1-T2 | Add/remove conformance proves both 60-second equations; every fence and privileged action bypasses cache; admitted epoch/membership/role changes block unfenced sends; missing/static/incomplete/unqualified sources block closed alpha; local principal is developer-preview-only |
| E1-T4 | Implement typed/scanned control text plus append-only audit service and middleware | M | E1-T1, E1-T2 | Every free-form name/tag/note/reason is bounded and secret/PII-classified before mutation; audit accepts only safe value objects, IDs/enums, and version references; all listed controls emit actor-attributed events |
...
| E2-T4 | Implement sanitized discovery snapshot persistence, encrypted schema payloads/keys, attestations, and current-observation retention protection | L | E2-T2, E2-T3 | Duplicate snapshot is deduplicated but protected while current; schema plaintext never enters durable stores; local references preserve provenance; external references never fetch; key activation/erasure and unsafe-metadata paths pass |
...
| E3-T5 | Implement paginated discovery, policy-versioned recursive whole-schema classification/attestation, rescan/erasure, and schema bounds | L | E3-T4 | Every key/leaf is scanned under the locked current policy before encrypted persistence; policy drift instantly hides/blocks old schemas, and clean rescan or attested erasure passes; sensitive/scanner/limit failures leave no releasable schema-derived state |
| E3-T6 | Implement clamped cache hints, per-connection discovery generations/CAS publication, durable forced reconciliation, observations, and change handling | M | E3-T5, E2 | Only latest-started matching configuration can publish/restore active; older late completion is discarded; durable <=5-minute recovery, worker loss, A→B→A, removal, and one-of-100 drift pass |
| E3-T7 | Implement connection health generation, reconciliation-staleness, and circuit-breaker state | M | E3-T4, E2-T2 | Breaker-open atomically degrades/cancels unfenced runs; half-open permits only probes; every fence requires active/closed/current-health under the connection lock; recovery continues until complete no-cache discovery |
...
| E4-T2 | Implement body-based pre-forward-scanned search, pagination, timeout, HMAC-keyed cache, and circuit breaker | M | E4-T1 | Secret/PII/scanner-failed queries never reach the Registry or URL/telemetry/storage surfaces; upstream outage returns explicit stale/unavailable state |
...
| E5-T2 | Implement run/admitted-authorization identity, request/result-key erasure, attempt/event/reconciliation, output classification, artifact scan attestations/status, and grant models | L | E1, E2-T5 | Actor/epoch and effective output are durable; requests and all retained results/fingerprints use separate erasable hierarchies; every terminal, erasure, scan/quarantine, grant, key, and reconciliation state has replayable lineage |
| E5-T3 | Implement idempotency-first replay, lock-serialized run admission, encrypted request/per-run-keyed fingerprint persistence, single-use preflight consumption, role authorization, and fail-closed classification policy | M | E5-T2, E1 | Lookup precedes preflight validation; clean live records compare through the erasable subkey, erased records return generic no-replay without comparison, and the no-record branch writes one encrypted request plus reference-only queue/replay state; conflicts/expiry/sensitive/stale input never re-enqueue |
...
| E5-T5 | Implement MCP dispatch with fresh actor authorization, single-use scanned lease, health fencing, rescanning, content/fingerprint erasure, no redirects, deadline, and cancellation | L | E3, E5-T1, E5-T2 | Fence requires current actor epoch/role and active health; lease sends once; redirect never resends and is indeterminate; reclassified input erases both subkeys; auth, cancellation, timeout, breaker, and loss races preserve truth |
| E5-T6 | Implement identity-aware shared lock-plan helper, durable pre-send dispatch fence, indeterminate reconciliation, and no-retry policy | M | E5-T5 | Admission/fence/identity/control paths use the order; reverse barriers prevent inversion; crash/redirect tests prove linearization and never duplicate a fenced call |
| E5-T7 | Quarantine; policy/profile-versioned output classification; encrypted-from-first-write result persistence; invalidation/rescan/erasure; per-MIME extraction/OCR; validate/redact | L | E5-T2, E0-T4 | No result plaintext enters durable stores; policy drift blocks decryption, and forbidden reclassification destroys both result subkeys before erasure; proprietary, sensitive, incomplete, invalid, or scanner-failed content cannot publish |
...
| E6-T2 | Implement body-based scanned upstream catalog search and import endpoints | M | E4, E6-T1 | Role, degraded-upstream, secret/PII search, and unsafe-source/scheme paths fail without forwarding or echoing raw input |
...
| E6-T4 | Implement current-schema-attested capability catalog/version endpoints | M | E2, E6-T1 | Historical safe metadata remains queryable, but stale/quarantined/erased schema payloads are suppressed; current clean attestations gate exact schema reads and enablement |
| E6-T5 | Implement secret-scanning run preflight, no-store run/event/cancel, artifact-grant mint/revoke, and current-policy-attested artifact-read endpoints | M | E5, E6-T1 | Revocation, replay/expiry, membership/role/epoch/visibility/retention/integrity, and output-scan-policy/profile drift deny token replay/bytes until current rescan; invocation/cache tests pass |
...
| E7-T3 | Build body-based official-registry search/import flow with no query/history persistence | M | E6-T2, E7-T1 | Remote and catalog-only states are distinct; raw terms/cursors never enter browser URLs, history, referrers, analytics, or error capture |
...
| E8-T3 | Add CSP, output escaping, artifact grants, header redaction, current-policy scan gates/rescan, integrity checks, and isolation | M | E5-T7, E6-T5, E7 | Grants never leak; policy/profile drift blocks stale scans, and per-MIME extraction/OCR/coverage plus malware/type/archive, overwrite, isolation, expiry, and active-content suites pass |
...
| 05 | `feat(registry): add entries connections and immutable snapshots` | Context/scheme-qualified `SafeUrl`, inert schema-URI references, encrypted schema payload/key and scan-attestation models, explicit output classification, migrations, lifecycle, canonical hashing | 03, 04 | HTTPS-only schemes, current schema-policy gates, key lifecycle, missing/changed output class, local-ref, no-external-fetch, and versioning tests pass |
...
| 07 | `feat(mcp): connect and discover remote Streamable HTTP servers` | Scheme-qualified `SafeUrl`, method-aware redirects, SDK/negotiation, address-pinned transport, paginated tools, policy-versioned schema scan/rescan, discovery generations/CAS publication, reconciliation, health | 05, 06 | Dual-era, schema-policy invalidation, overlapping-refresh last-start-wins, invocation-no-redirect, active-scheme, metadata-secret, staleness, rebinding, and SSRF suites pass |
...
| 09 | `feat(catalog): search and import official registry entries` | Body-based pre-forward-scanned search, upstream adapter, scheme-qualified `SafeUrl`, HMAC-keyed cache/provenance, import, catalog-only state | 05 | Secret/PII queries never reach upstream or URL/telemetry/storage surfaces; recorded contract, active/unsafe source URL, and provenance tests pass |
| 10 | `feat(jobs): add durable jobs encrypted run ledger and erasure keys` | Leases, events, actor/admitted epoch, attempts, idempotency, per-request/per-result keys, shared identity-aware lock plan, output/artifact attestations, grant projections | 03, 04, 08 | Crash recovery, actor/epoch lineage, request/result no-compare erasure, scan/grant replay, key lifecycle, and lock-order tests pass |
| 11 | `feat(invocation): execute MCP tools with bounded inputs and results` | Fresh fence authorization, input scan/erasure, single-use lease, health dispatch, no redirects, encrypted result publication, policy/profile rescan/erasure, cancellation, artifacts | 07, 10 | No retained result plaintext; actor, redirect, breaker, scan-policy, and request/result erasure races fail safely; stale outputs release no bytes; content suites pass |
| 12 | `feat(api): expose registry capability and run APIs` | REST resources, current-schema-attested capabilities, stable Run/event contracts, scanned idempotency/preflight, body-based search, prepared grant envelopes, current-result-scan reauthorization, auth epochs, generated client | 08–11 | OpenAPI/client, schema-policy suppression, search leakage, admission/policy races, envelope/revoke crash, stale-result denial, grant/input secrecy, and API E2E pass |
...
- authorization-freshness qualification rejects group-less/static-claim sources, derives `L` from `P + R + L + M <= 60s`, disables caching when no positive budget remains, and requires privileged cache bypass;
...
- effective result-class lattice derivation from independent input, immutable connection minimum, and detector/profile values; exact policy/profile scan-attestation matching, encrypted-from-first-write result key lifecycle, rescan-required invalidation, forbidden-retention erasure, quarantine, redaction, per-MIME required-coverage accounting, and fail-closed extraction/OCR/scanner behavior;
...
- body-based Registry search fixtures prove secret/PII and scanner-failed terms are rejected before the upstream adapter, raw terms/cursors never enter browser/proxy URLs, history, referrers, telemetry, audit, errors, or cache keys, and only a clean bounded term is encoded at the final redacted upstream hop;
...
- artifact access-grant fault injection before/after secret-store prepare, PostgreSQL commit, and envelope promotion proves a committed grant always references one durable envelope; same-key replay at `expires_at - epsilon` promotes/reads that exact credential, replay at/after expiry returns no credential, rollback and uniqueness-loser envelopes expire inaccessible, promotion never moves expiry, and premature/missing/corrupt committed state fails closed without minting another grant;
- artifact-grant revoke/redemption barriers prove exactly one lock order, replayed revoke is idempotent, any redemption begun after the revoked projection commits returns no bytes, and a delayed envelope erasure cannot re-enable token replay; subject role/membership removal, authorization-epoch advance, artifact visibility/classification/retention-policy change, quarantine, expiry, or integrity-state change also deny bytes despite a valid token, and stale/failed authorization refresh fails closed;
- immutable-version secret retrieval without disclosure and alias rotation through new binding/connection versions;
- provider-specific OIDC qualification and fake-clock live-token add/remove tests prove `P + R + M <= 60s` for authoritative checks and `P + R + L + M <= 60s` for cached decisions; a refresh just before provider propagation cannot extend access past the bound, privileged actions and every dispatch fence synchronously bypass cached snapshots, admitted epoch mismatch cancels unfenced work, expired UI deadlines hide tenant data, group-less/static issuance-time responses and incomplete/failed refreshes fail closed, and an unqualified or budget-exhausted provider prevents closed-alpha deployment while the local-principal build reports developer-preview-only release metadata;
...
- overlapping scheduled, notification, and operator discoveries allocate ordered generations; an older request completing after a newer published request cannot touch cache/snapshot/observations/capability state/reconciliation timestamps or restore `active`, and configuration change invalidates every captured older generation;
- switching the negotiated MCP protocol revision while tool schemas and implementation revision remain byte-identical creates new `pending_review` capability versions for the affected connection; the prior versions remain historical/disabled and execution history never combines wire contracts;
...
- barrier races between breaker-open/degradation and dispatch prove both paths take the connection/health lock, every fence rechecks `active` plus closed/current health, admitted unfenced runs are cancelled, and half-open probes cannot become ordinary runs;
...
- deterministic barriers between artifact grant mint/replay/redemption or inline result read and output/artifact scan-policy or MIME-profile activation prove a request linearized after activation releases no token/bytes from an old attestation. Old content is immediately `rescan_required`; only a successful current-version rescan of the same immutable content identity restores readiness, while missing bytes, failure, or a second policy change stays quarantined/discarded;
...
- retained clean-request fixtures prove PostgreSQL/base backups, WAL-restored databases, replicas, queues, idempotency responses, and durable caches contain only ciphertext/keyed bits or references and no recoverable key; transaction rollback leaves only a provisional hierarchy that self-destructs, activation-handler crashes retry idempotently, and no run dispatches before activation. After low-entropy scan-policy reclassification, both per-run subkeys are destroyed, restored pre-erasure ciphertext cannot decrypt, retained fingerprint bits cannot test enumerated emails/account numbers, active rows expose only generic `erased_no_compare`, and every same-key replay fails without request comparison; delayed/failed destruction remains `erasure_pending`, non-comparable, quarantined, and unsent across crashes/retries.
...
- An initially clean retained result is recoverable only through authorized decryption under a current scan attestation; a later policy that forbids it suppresses bytes immediately and completes only after result-key destruction makes database/object/WAL/replica/backup ciphertext unrecoverable.
...
- A grant minted immediately before individual grant, membership/role, artifact-policy revocation, or output/artifact scan-policy/profile change cannot retrieve content after the authoritative grant/epoch/state/policy projection changes; replaying the original mint key after revocation or scan invalidation never returns the token, and a current-version rescan is required before any new grant/read.
- Sensitive or unscannable tool arguments never reach the upstream server; initial rejection never persists, and a policy change that newly rejects a queued encrypted request immediately suppresses reads, dispatch, and comparison, remains visibly `erasure_pending` through destruction failure, and becomes terminal only after attested destruction of both per-run subkeys makes historical ciphertext and keyed fingerprints unrecoverable/non-enumerable, purges caches, and installs a generic no-replay tombstone.
...
| Untrusted cache hints or out-of-order refreshes conceal remote tool drift | Reconciliation age grows, an older fetch replaces newer state, or stale bindings remain enabled | Clamp TTLs; generation/CAS latest-started publication; forced reconciliation; degrade/block overdue connections; durable bounded-backoff recovery until winning bypass/disable |
| SSRF, active-scheme navigation, or secret exposure through URL-like/search fields | Requests target internal/metadata addresses, active schemes reach UI links, search terms or credentials appear in URLs, or schema references are fetched | Contextual HTTPS-only `SafeUrl`; body-based pre-forward-scanned search; inert same-document-only schema references; IP/redirect validation and egress policy |
...
| Schema bombs or detector-policy drift | CPU/memory spikes, stale schema bytes remain visible, or a newly recognized credential survives retention | Schema bounds; policy-versioned attestations; immediate read/invocation invalidation; encrypted schema quarantine and attested key erasure |
| Secret leakage or later result reclassification | Headers appear in telemetry, or plaintext from an initially clean result remains in durable history | Secret references, centralized redaction, encrypted-from-first-write results with per-result erasure keys, restore tests, least-privilege access |
