...
- `GET /v1/artifacts/{id}` — return authorized metadata, current trusted source-floor and policy/profile scan eligibility/readiness, exact immutable storage-version identity, and a clean-content digest only when policy permits; for encrypted result artifacts the digest is computed transiently during authorized decryption and never persisted. Stale provenance/scan reports safe reclassification/`rescan_required` metadata and no viewer mode, never a keyed plaintext fingerprint or ambient object-store credential
...
Routing and run requests accept only finalized, unexpired artifacts owned by the same workspace, allowed by request/provider data policy, and carrying an append-only scan attestation over the exact immutable content identity that matches the locked current output/artifact-scan-policy version and applicable MIME-profile version. Each upload uses a unique non-overwritable object key or versioned-bucket write. Completion closes the upload, verifies an ephemeral client integrity proof and length against the exact storage version, detects archive expansion/path traversal, and applies the Registry Alpha malware/type plus per-MIME metadata/embedded-content extraction, all-page/frame render/OCR, secret/PII classification, and complete-coverage policy; a type without a complete tested profile remains quarantined. Clean uploaded-input artifacts may persist a policy-permitted content digest, while restricted uploads use ciphertext integrity plus a purpose-separated HMAC fingerprint and never an ordinary plaintext digest; provider/MCP result artifacts always use the separate encrypted per-result/keyed-fingerprint contract below, even when initially clean. Activating a changed policy or applicable profile immediately makes every older attestation logically `rescan_required` without waiting for fan-out: grant mint/replay/redemption, reads, routing, and initial/fallback dispatch all return no token/bytes/send. A durable job may rescan retained quarantined bytes and append a superseding attestation only when commit-time locks prove the same content identity and still-current policy/profile; unavailable bytes, scanner failure, newly disallowed content, or another intervening change remains quarantined or follows the applicable attested erasure path. Consumption reauthorizes the artifact and reads only that pinned version, verifying its kind-appropriate integrity value and current attestation; a still-valid upload credential cannot replace finalized content. HTTP/SDK clients upload through the `EphemeralUploadTarget`; the MCP facade exposes both `create_artifact_upload` and `complete_artifact_upload`, with target delivery allowed only through the non-recording transfer channel above and completion returning the authoritative `artifact://` URI so an eligible MCP control-plane client needs no REST credential. Large or retained non-text results use the same immutable artifact and authorized-access contract, so the console never reads object storage directly.
...
| P1-04 Runtime | Input scan/erasure, encrypted attested output lifecycle, pools, quote/exact-price freshness, dispatch/authorization eligibility, precise reservation, deadlines, fallback, breakers, no redirects | P0 adapters | Rejected input and forbidden results erase their own subkeys; no retained result plaintext reaches durable storage; stale/underclassified artifacts release/send nothing; fences reauthorize/recheck/reserve atomically; 3xx never resends; fractional costs reconcile exactly |
