...
| P0-02 Taxonomy | Versioned task and feature schema plus Phase 0 task-content/key authority | JSON Schema validation; feature provenance; no post-outcome fields; original/normalized/features/candidate snapshot encrypt before persistence under one current-attested erasable hierarchy | Backend/domain |
...
Admission through an API credential additionally binds the exact credential ID/version and its current revocation epoch, action scopes, workspace/environment scope, and spending/quota ceiling into the route/run. Before every initial or fallback fence, the qualified authorization step locks that credential's current projection and requires it to remain active with the admitted action, environment, and at-least-as-permissive spending/quota scope. Revocation, rotation without explicit continuity, expiry, or scope reduction cancels unfenced work with `api_credential_authorization_changed` and zero provider sends even when the human subject's membership epoch is unchanged. Credential mutations and dispatch fences share the same lock order.
...
- A run admitted through an API credential also retains its credential ID/version/revocation epoch and admitted scopes/ceilings. Every initial/fallback fence locks and revalidates the credential's current active projection and requires action/workspace/environment scopes and spending/quota authority to remain at least as permissive; revoke, expiry, unapproved rotation, or scope reduction cancels unfenced work with zero sends independent of actor membership state.
...
- API-credential barriers revoke, expire, rotate without continuity, or reduce action/environment/spending scope after admission while leaving actor membership unchanged, and prove every initial/fallback fence locks the current credential projection and sends nothing;
