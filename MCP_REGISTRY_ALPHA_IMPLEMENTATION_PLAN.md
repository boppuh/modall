...
- Establish and initialize the invocation transport session before the database dispatch fence but send no `tools/call` yet. Session initialization itself is fenced: immediately beforehand the worker repeats the cache-bypassing qualified-source actor authorization lookup without database locks, then a short transaction uses the shared actor/connection/configuration/discovery/health lock subsequence, reconciles the lookup, requires unchanged admitted membership/role/authorization epoch, connection lifecycle `active`, invocation breaker `closed` (or the one designated half-open probe), current dispatch-eligible health generations, and the exact bound configuration/credential/discovery/protocol/adapter versions, and records one short-lived single-use `session_initialization_fenced` lease bound to all those values and the lifecycle epoch. The service-controlled transport redeems that lease under the same locks immediately before retrieving/attaching the pinned credential and writing the first byte; an expired, cancelled, already-redeemed, or mismatched lease sends nothing. Actor authorization change, disable, material endpoint/credential change, or breaker-open takes the same locks, invalidates every affected unredeemed lease, and closes/cancels every transport from an older epoch/generation. If redemption linearizes first, later control mutation still invalidates the eventual proof and cancels the transport; it cannot pass the dispatch fence. Successful initialization publishes a short-lived single-use session proof only through a compare-and-set that the captured actor/lifecycle/configuration/credential/discovery/health values remain current, binding the observed server identity and negotiated live protocol revision. The dispatch fence repeats actor authorization and must consume that proof and match both exactly to the immutable capability/server-connection version and current discovery publication. A mismatch or stale/cancelled proof suspends the connection, wipes arguments/session state, sends zero tool-call bytes, and requires fresh stable discovery plus review. Thus a paused worker cannot emit credential-bearing initialization traffic after an actor or connection control mutation, and automatic negotiation cannot silently execute a stored binding under a different wire revision.
...
| `mcp_session_initialization_leases` | Short-lived one-shot pre-session authorization bound to attempt/probe, actor/admitted authorization epoch, lifecycle epoch, exact configuration/credential/discovery/protocol/adapter identity, and discovery/invocation-health generations; contains no credential | Mint/redeem/cancel/expire once |
...
| E3-T7 | Implement separate discovery-health and invocation-breaker generations, reconciliation staleness, and probe-only recovery | M | E3-T4, E2-T2 | Breaker-open atomically degrades/cancels unfenced runs; discovery success cannot close it; half-open permits exactly one qualified execution probe; every session-init/dispatch fence requires active/closed/current-health under the connection lock; discovery recovery continues until complete no-cache discovery |
...
| E5-T5 | Implement MCP dispatch with fresh actor authorization, current input attestation, pre-session lifecycle lease, single-use scanned lease, health fencing, retained-request rescanning/erasure, no redirects, deadline, and cancellation | L | E3, E5-T1, E5-T2 | No credential/init byte precedes a current redeemed session lease; fence and every request read require current input policy; terminal and queued reclassification destroys both subkeys without rewriting execution; lease sends once; auth, control-change, redirect, cancellation, timeout, breaker, and loss races preserve truth |
...
| 11 | `feat(invocation): execute MCP tools with bounded inputs results and errors` | Fresh fence authorization, pre-session lifecycle lease, current input scan/rescan/erasure, single-use lease, separate discovery/invocation health, no redirects, encrypted result/error publication, policy/profile rescan/erasure, cancellation, artifacts | 07, 10 | No credential/init byte after a winning control mutation and no retained result/error plaintext; actor, redirect, breaker, input/output-policy, and request/result erasure races fail safely; discovery cannot close the invocation breaker; stale requests/outputs release no bytes; content suites pass |
...
- barrier races between breaker-open/degradation and session initialization/dispatch prove all paths take the connection/health locks, every pre-session lease and dispatch fence rechecks `active` plus closed/current discovery and invocation health, admitted unfenced runs are cancelled, and half-open probes cannot become ordinary runs. A healthy `tools/list` while invocation remains open may refresh discovery evidence but cannot change breaker state or admit user work; only the single qualified half-open execution probe can close it;
