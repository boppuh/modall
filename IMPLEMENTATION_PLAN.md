...
| P1-04 Runtime | Input scanning, adapter pools, quote/price freshness, dispatch-time eligibility, atomic quota/budget reservation, deadlines, evidence-gated fallback, circuit breakers | P0 adapters | Every fence has a live unchanged price version and one worst-case reservation created in the same transaction; expiry/repricing sends nothing, concurrency cannot overspend, and terminal reconciliation commits/releases correctly |
