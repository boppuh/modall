...
| P0-04 Harness | Durable execution and restricted artifact capture | Resumable, idempotent matrix runs; reference-owned artifact retention blocks default cleanup only within approved source ceilings and invalidates dependent evidence before forced erasure; zero-null legacy visibility/binding backfill precedes atomic RLS activation; exclusive exact MCP/provider binding; evaluation-only domain enforced below repositories; hidden data absent from ordinary/Alpha-compat APIs; pinned primary slots and environment | Platform |
...
For result consumption without REST credentials, `read_artifact(artifact_uri, offset, max_bytes)` uses the authenticated MCP session to obtain a qualified current authorization snapshot and, under the shared provenance/artifact/policy lock order, recheck subject, workspace membership/role, authorization epoch, artifact visibility, an active/fresh source-authority projection exactly matching the artifact attestation when bound or exact current workspace unverified-upload-floor version otherwise, current trusted source floor/effective classification, retention, exact immutable version/kind-appropriate integrity state, active result-key state when applicable, and an attestation matching the current output/artifact-scan-policy plus applicable MIME-profile versions on every call. It returns at most 256 KiB per chunk with total length, detected content type, a transient clean-content digest only when policy permits, and the next offset; keyed plaintext fingerprints remain internal. Safe text/JSON uses typed content, and only explicitly allowlisted bytes that completed their full current content-aware profile use base64. Repeated bounded calls can consume a large result, while stale/incomplete authorization or source authority/provenance/floor, `source_revalidation_required`, `rescan_required`, result erasure, active, encrypted/uninspectable, unknown, unsupported, type-mismatched, coverage-incomplete, quarantined, expired, cross-workspace, integrity-failed, or policy-forbidden content fails closed. The tool never returns an ambient object-store URL or credential.
...
| P1-03 Run API | `/routed-runs`, shared `/runs` projection, exclusive execution binding, ordered in-place migration, execution-contract-versioned DB compatibility roles/policies, job creation | Identity, artifacts, jobs | Current clients share one ledger; every attempt has one exact target and minimum worker contract; pre-RLS backfill has no null window; Alpha/down-level roles can read/write only Alpha-contract direct workspace MCP rows, and rollback cancels/drains every newer-contract direct/routed/evaluation row before old workers start |
...
- `evaluation_runs`, `evaluation_attempts`, `evaluation_results`, `grader_results`; after the execution-binding and visibility migration below, Phase 0 writes evaluation executions as `run_kind=evaluation`, `run_visibility_scope=evaluation_restricted`, `execution_contract_version=phase0_evaluation_v1`, an exact visibility-domain ID, and an exact immutable `deployment_version_id`, then references those authoritative `runs`/`run_attempts` rows rather than duplicating dispatch state
...
- A successful HTTP artifact redemption creates no authority lasting for the response lifetime. Before every chunk of at most 256 KiB, the reader requires database server time before grant expiry, performs the cache-bypassing qualified authorization refresh, and only on success installs `download_authorization_expires_at = min(source_checked_at + D, grant_expires_at)`. It then requires current time before that deadline and locks/revalidates the grant, current human/API-credential projections, exact current source authority when bound or workspace unverified-upload floor otherwise, artifact/classification/retention/integrity, and scan/profile pointers before decrypting. The same deadline bounds the write; backpressure at expiry terminates the connection rather than permitting more writes. `P + R + D + M <= 60 seconds`; failure or expiry terminates before later bytes, and no speculative decrypted buffer crosses the check.
...
- Every active benchmark case/suite/attempt/report and gate-evidence version has a complete exact-artifact dependency set plus event-authoritative active retention-lease projections through its declared execution/review/reproduction deadline. The effective artifact delete time is the later of the inherited default and every active lease deadline, capped by each source's hard approved retention ceiling. Each gate-evidence current projection carries `artifact_dependency_valid_through`, the minimum effective deadline across its complete dependency set; every task-aware route requires database server time strictly before that value as well as `valid` state, so a late expiry job cannot extend activation. Cleanup locks the artifact, its leases, and referencing benchmark/evidence projections in the shared order; it cannot erase while a permitted lease remains active. Source/policy revocation or ceiling expiry first atomically invalidates all dependent benchmark/report/evidence projections and task-aware flags, preventing new reads, runs, reporting claims, or routing activation, and only then enters artifact erasure. Missing assets or leases fail closed and can never leave gate evidence marked valid.
...
- benchmark artifact retention-lease admission, effective-deadline calculation, source-ceiling enforcement, reference release, cleanup eligibility, and evidence/feature-flag invalidation before forced erasure;
...
- HTTP artifact-read deadlines and chunk state prove `download_authorization_expires_at <= min(last_qualified_check + D, grant_expiry)`, grant-expiry/refresh/new-deadline/authorize/decrypt/write ordering, bounded-write timeout under backpressure, and termination without a later chunk on any refresh/grant/identity/source-authority/artifact/policy failure;
...
- artifact-content contracts expose bounded HTTP streaming with no ambient object-store credential and a server-authored authorization deadline; generated clients tolerate termination between chunks and cannot request a chunk larger than 256 KiB or extend `D`/grant expiry;
...
- `read_artifact` returns authorized, integrity-checked bounded chunks only after the exact current source-authority or unverified-floor projection and complete classification attestations match the current output/artifact policy and MIME profile; it rejects `source_revalidation_required`, `rescan_required`, active, encrypted/uninspectable, unknown, unsupported, type-mismatched, coverage-incomplete, quarantined, expired, or cross-workspace results without exposing restricted plaintext fingerprints;
...
| Benchmark asset expires while still authoritative | The 14-day artifact job erases an early snapshot before a three-week calibration/report finishes, or live gate evidence outlives its inputs | Exact-version reference-owned retention leases through declared execution/review/reproduction windows, capped by source approval; cleanup lock/recheck; invalidate reports, gate evidence, and task-aware flags before forced erasure |
...
| Long artifact response outlives authorization | A backpressured download emits bytes after grant expiry or human/API-credential/source-authority revocation | At-most-256-KiB chunks; server-authored deadline capped by grant expiry and `D`; cache-bypassing identity/grant/authority/artifact/policy check before decrypting each chunk; terminate on timeout or mismatch |
...
- an MCP-only result client can retrieve permitted text/JSON or bounded binary chunks through `read_artifact` using its MCP session, while HTTP clients use subject/API-credential/source-authority-bound short-lived grants and at-most-256-KiB response chunks whose server deadline is capped by grant expiry and `D`; both paths recheck the exact current authority/artifact/scan state before each chunk, stop within the inherited 60-second authorization budget, and never receive object-store credentials;
