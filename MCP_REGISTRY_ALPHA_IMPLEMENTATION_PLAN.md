...
- Official Registry search accepts its free-form term and only a Modall-issued authenticated encrypted cursor in a body-capture-disabled non-mutating POST. Every caller-controlled field is bounded/scanned before caching or forwarding; the recovered upstream cursor is separately scanned in no-capture memory after binding verification, and any match/failure/tampering leaves no query-derived state or upstream request. Browser/proxy URLs, history, referrers, telemetry, audit, errors, and cache keys never carry the raw term/cursor, and the adapter redacts the final upstream URL.
...
| `control_text_content_keys`, `control_text_scan_attestations` | Per-value external encryption/fingerprint hierarchy plus exact current control-text-policy decision for retained optional names/tags/notes; ordinary domain/audit rows keep only opaque references or closed enums | Activate/destroy once; attestations append/supersede |
...
- `POST /v1/registry/searches` — perform a non-mutating official-registry search from a bounded JSON body containing `query` and an optional Modall-issued sealed cursor. Ingress/proxy body capture is disabled. Before any canonicalization, cache lookup, audit construction, or upstream call, scan every caller-controlled decoded field, including `query`; reject unknown fields and any match/scanner failure without derived state. Never accept a raw upstream cursor from a caller. A returned cursor is an authenticated encrypted fixed-expiry envelope over the exact upstream cursor, Registry source/contract version, normalized query identity, workspace, and next-page position. On use, verify those bindings, decrypt only in bounded no-capture adapter memory, apply the same secret/PII classifier to the recovered upstream cursor, and forward only if clean; tampering, expiry, mismatch, or scanner failure sends nothing. Only clean values reach the final upstream hop, whose complete URL/query is redacted from telemetry. Browser URLs, history, referrers, caches, errors, and audit metadata never contain raw term/cursor. This non-mutating POST requires authorization but no `Idempotency-Key`.
...
| E1-T4 | Implement encrypted current-policy-attested control text plus append-only reference-only audit service and middleware | M | E1-T1, E1-T2 | Every retained optional name/tag/note is encrypted under a per-value erasable hierarchy; policy drift suppresses/rescans/erases it, reasons are enums, and audit stores only safe IDs/enums/references; restore tests prove old ciphertext non-enumerable |
...
- body-based Registry search fixtures prove secret/PII and scanner-failed values in every caller field reject before the upstream adapter; raw upstream cursors are never accepted from callers, sealed cursors are workspace/query/source/page/expiry-bound, and tampered or sensitive recovered cursors send nothing. Raw terms/cursors never enter browser/proxy URLs, history, referrers, telemetry, audit, errors, or cache keys;
...
- retained control-text fixtures place later-recognized low-entropy credentials/PII in initially accepted names/tags/notes, then activate a stricter policy and prove immediate read/UI/audit suppression, quarantine-only rescan, both-subkey destruction before erasure, and non-enumerable restored database/WAL/replica/backup ciphertext with only generic reference tombstones remaining;
