...
6. Restrict API `/metrics` and the configured worker metrics port (9101 by default) to the monitoring
   network; neither is an internet-facing endpoint. Configure the Prometheus scrape targets with
   job labels `modall-api` and `modall-worker` so target-loss paging remains active.
