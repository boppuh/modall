...
| P0-03 Corpus | 75–100-case calibration suite plus fresh per-attempt activation holdouts whose first tranche is 200–300 cases and final size follows the complete joint-gate simulation | Component graph is audited/frozen; revealed components retire; each fresh holdout has a conservative >=90% joint-pass probability at the preregistered above-threshold planning alternative and attempt alpha allocations | Swift/evaluation |
...
| P0-08 Analysis | Baseline comparison, joint power, and sequential confidence bounds | Reproducible report; sizing simulates every observed/confidence/zero-success quality-and-cost gate jointly at the above-threshold planning alternative with a >=90% Monte Carlo lower bound; activation uses the same paired bootstrap and attempt alpha allocations; no revealed component is reused | Data/full-stack |
...
| P1-01A Artifacts | Non-overwritable presigned ingestion, completion/scanning, immutable artifact URI, and sealed `ArtifactAccessGrantToken` | Identity, object storage | Secret envelopes/no-store/redaction pass; every redemption reauthorizes current membership, epoch, visibility, classification, retention, scan, and integrity; post-mint revocation, overwrite, expiry, and wrong-version tests fail closed |
...
- G1 sizing/analysis fixtures prove repository-only and archetype-only resampling are rejected, the frozen composite cluster preserves every linked case, attempt-specific confidence uses preregistered alpha, the complete observed-plus-confidence quality/cost rule is jointly simulated at an above-five-point/below-1.20 alternative with a >=90% Monte Carlo lower bound, an exact-five-point alternative is not mislabeled 90%-powered, and `g1_revealed` components cannot be reused or pooled;
