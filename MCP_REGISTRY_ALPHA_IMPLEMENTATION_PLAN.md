...
- Run admission and every identity/authorization-epoch update, connection disable, material configuration change, capability-state change, discovery-publication/observation replacement, control-text/remote-metadata/schema/input/output/artifact-scan-policy or MIME-profile change, and connection-policy update acquire the affected rows/advisory locks in one documented global order. Where client idempotency applies, its scoped key lock precedes the lifecycle lock sequence; worker fencing, control-text/metadata/schema/request/result publication/rescan, and artifact access use the same relevant subsequence without an idempotency lock. Admission inserts its run/queue row before releasing lifecycle locks. Therefore a control mutation that locks second must observe and cancel the admitted undispatched run, while admission that locks second must observe the new state and reject before enqueue; there is no read/enqueue gap or inverted lock order. A scanner-policy activation need not enumerate retained content before taking effect: advancing its locked current-version pointer immediately makes every older attestation ineligible for the corresponding control-text/metadata/schema/request/result reads and for validation, enablement, admission, publication, replay, grant, redemption, or dispatch consumption.
