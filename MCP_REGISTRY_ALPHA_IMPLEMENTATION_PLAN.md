...
- Tool arguments matching a credential, token, secret, high-entropy detector, or disallowed policy-defined PII—and any input scanner failure—are rejected before request persistence or enqueue and again before dispatch. Initial rejection retains no argument, digest, or fingerprint. Retained clean arguments are envelope-encrypted under a unique externally erasable per-run key hierarchy with purpose-separated encryption and request-fingerprint subkeys and an exact input-policy attestation; durable queues/replays/caches never duplicate plaintext or key material. A later policy version immediately suppresses reads/replay/comparison for terminal and nonterminal runs until quarantine rescan; if it rejects, state enters `erasure_pending` and terminal erasure occurs only after both subkeys are verifiably destroyed, active storage is replaced by a generic non-comparable no-replay tombstone, and database/WAL/replica/backup/cache ciphertext and keyed fingerprints are unrecoverable. Execution/result history is unchanged, and neither path sends provider data. Server authentication uses an opaque credential binding instead.
...
- A tool content digest is an opaque composite over the normalized remote-metadata HMAC identity—including remote name/title/description/annotations and implementation identity source/revision—the input/output schema HMAC identities, exact `server_connection_version_id`, negotiated protocol revision, and safe `implementation_identity_assurance` enum; raw metadata/schema bytes or an ordinary content-derived digest never enter it. The complete discovery-snapshot ID/digest remains separate observation provenance, so unrelated tool drift cannot churn a version. Credential rotation, protocol change, identity-source/revision change, or assurance change creates a new digest even with byte-identical advertised schemas. Destruction of a participating metadata or schema fingerprint subkey marks the tool digest non-comparable, suppresses it from public detail, and leaves only an opaque generic tombstone projection; retained HMAC/composite bits in immutable rows, WAL, replicas, and backups cannot test candidate content without the destroyed subkey.
...
- Every binding records the negotiated protocol revision, safe `implementation_identity_assurance` enum (`pinned`, `declared`, or `unverified`), and current-attested encrypted remote-metadata reference containing the exact identity source plus optional implementation revision; their opaque identities participate in version identity. A changed protocol revision, implementation revision, source, or assurance creates a new `pending_review` capability version and prevents a prior enablement or stale trust metadata from crossing the changed wire contract.
...
- Run admission and every identity/authorization-epoch update, connection disable, material configuration change, capability-state change, discovery-publication/observation replacement, remote-metadata/schema/input/output/artifact-scan-policy or MIME-profile change, and connection-policy update acquire the affected rows/advisory locks in one documented global order. Where client idempotency applies, its scoped key lock precedes the lifecycle lock sequence; worker fencing, metadata/schema/request/result publication/rescan, and artifact access use the same relevant subsequence without an idempotency lock. Admission inserts its run/queue row before releasing lifecycle locks. Therefore a control mutation that locks second must observe and cancel the admitted undispatched run, while admission that locks second must observe the new state and reject before enqueue; there is no read/enqueue gap or inverted lock order. A scanner-policy activation need not enumerate retained content before taking effect: advancing its locked current-version pointer immediately makes every older attestation ineligible for the corresponding metadata/schema/request/result reads and for validation, enablement, admission, publication, replay, grant, redemption, or dispatch consumption.
...
8. current remote-metadata-scan-policy, schema-scan-policy, input-scan-policy, and output/artifact-scan-policy plus MIME-profile pointers/versions, ordered by policy kind then MIME;
...
| `mcp_tool_bindings` | Version-to-exact-server-connection-version/tool binding including negotiated protocol revision, safe assurance enum, and encrypted current-attested remote tool-name/identity-source/revision reference | Immutable |
...
| `scan_policy_versions` | Immutable bounded remote-metadata, schema, input, output/artifact rules, and per-MIME scanner-profile definitions with release compatibility metadata | Append-only |
| `scan_policy_pointers` | Current remote-metadata, schema, input, output/artifact, and per-MIME profile version/generation selectors | Compare-and-set under shared lock plan |
...
| `input_scan_attestations` | Exact per-run keyed request identity, immutable input-scan-policy version, bounded traversal coverage, decision, and supersession lineage for terminal and nonterminal runs | Append-only |
| `run_attempts` | Exact dispatch attempt, receipt, encrypted/keyed successful-result or upstream-error-content reference and key handle, output-scan decision/attestation, stable safe error envelope, and monotonic retained/erasure state | Append-only events/status; erasure tombstones content only after key destruction |
...
- Every retained request, including one on a terminal run, has an append-only attestation over its exact per-run-keyed identity and input-scan-policy version. Reads, replay comparison, and unfenced dispatch require an active hierarchy and an attestation matching the locked current pointer. Policy change immediately produces logical `request_rescan_required`; only a clean quarantine rescan restores comparison/read eligibility, while sensitive/failure/missing-content paths remain non-comparable and destroy both per-run subkeys before terminal erasure without rewriting execution history.
...
| E5-T2 | Implement run/admitted-authorization identity, request/result-key erasure, input/output attestations, attempt/event/reconciliation, upstream-error classification, artifact status, and grant models | L | E1, E2-T5 | Actor/epoch and effective output are durable; requests and every retained result/error fingerprint use separate erasable hierarchies; terminal requests remain policy-attested; every erasure, scan/quarantine, grant, key, and reconciliation state has replayable lineage |
...
| E5-T5 | Implement MCP dispatch with fresh actor authorization, current input attestation, single-use scanned lease, health fencing, retained-request rescanning/erasure, no redirects, deadline, and cancellation | L | E3, E5-T1, E5-T2 | Fence and every request read require current input policy; terminal and queued reclassification destroys both subkeys without rewriting execution; lease sends once; auth, redirect, cancellation, timeout, breaker, and loss races preserve truth |
...
| E5-T7 | Quarantine successful and upstream-error content; policy/profile-versioned output classification; encrypted-from-first-write persistence; invalidation/rescan/erasure; per-MIME extraction/OCR; validate/redact | L | E5-T2, E0-T4 | No result or upstream-error plaintext enters durable stores/events; policy drift blocks decryption, and forbidden reclassification destroys both result subkeys before erasure; only stable safe error metadata survives rejected content |
...
| E6-T5 | Implement secret-scanning run preflight, current-input-attested no-store run/event/cancel, safe upstream-error projection, artifact-grant mint/revoke, and current-output-attested artifact reads | M | E5, E6-T1 | Terminal/nonterminal request policy drift denies arguments/replay until rescan or erasure; raw upstream errors never enter events; grant revocation and output-policy/profile drift deny tokens/bytes; invocation/cache tests pass |
...
| 09 | `feat(catalog): search and import official registry entries` | Body-based pre-forward-scanned search, upstream adapter, scheme-qualified `SafeUrl`, encrypted current-policy remote metadata/erasable identity, safe cache/provenance, import, catalog-only state | 05 | Secret/PII queries or metadata never reach unsafe URL/plaintext storage/API surfaces; policy rescan/erasure, recorded contract, active/unsafe source URL, and provenance tests pass |
| 10 | `feat(jobs): add durable jobs encrypted run ledger and erasure keys` | Leases, events, actor/admitted epoch, attempts, idempotency, per-request/per-result keys, current input/output attestations, shared lock plan, safe error envelopes, grant projections | 03, 04, 08 | Crash recovery, actor/epoch lineage, terminal-request and result/error no-compare erasure, scan/grant replay, key lifecycle, and lock-order tests pass |
| 11 | `feat(invocation): execute MCP tools with bounded inputs results and errors` | Fresh fence authorization, current input scan/rescan/erasure, single-use lease, health dispatch, no redirects, encrypted result/error publication, policy/profile rescan/erasure, cancellation, artifacts | 07, 10 | No retained result/error plaintext; actor, redirect, breaker, input/output-policy, and request/result erasure races fail safely; stale requests/outputs release no bytes; content suites pass |
| 12 | `feat(api): expose registry capability and run APIs` | REST resources, current-metadata/schema-attested capabilities, current-input-attested requests, stable safe Run/event errors, scanned idempotency/preflight/search, prepared grant envelopes, current-result reauthorization, auth epochs, generated client | 08–11 | OpenAPI/client, metadata/schema/request/result policy suppression, search/error leakage, admission races, envelope/revoke crash, grant/input secrecy, and API E2E pass |
...
- current input-attestation checks for terminal and nonterminal request reads/replay/dispatch plus clean rescan and sensitive/failure erasure transitions;
...
- terminal-run barriers activate a stricter input policy before GET/replay and prove completed/failed/cancelled requests immediately return `request_rescan_required`; clean quarantine rescan restores authorized reads, while newly sensitive, scanner-failed, missing-key, and restore-copy paths remain suppressed and reach terminal erasure only after both request subkeys are destroyed without changing execution/result status.
...
- A completed run whose retained arguments become sensitive under a newer input policy immediately hides them, never replays/compares the request under the stale attestation, and completes cryptographic erasure without rewriting its terminal execution/result timeline.
...
| Secret leakage or later request/result/error reclassification | Headers or upstream errors appear in telemetry, terminal arguments remain readable under stale policy, or plaintext from an initially clean result remains in durable history | Secret references; centralized redaction; current input/output attestations for terminal and live rows; encrypted-from-first-write requests/results/error bodies with erasable keys; restore tests; least-privilege access |
...
- no raw credential, stale-policy terminal request, or upstream error body is stored or emitted outside its encrypted current-attested content boundary;
- URL-field/endpoint, input-secret/rescan, remote-metadata, schema, result/error content, and tenant isolation security tests pass;
