...
- Upload completion and expiry cleanup use one locked monotonic state machine. `open -> completed` publishes the pinned artifact; `open -> cleanup_pending -> deleted` permanently forbids completion and deletes only the recorded object version/multipart upload. Cleanup failures retry and alert, and no transition can move a completed upload into deletion.
...
- object upload/download authorization plus fake-clock abandoned-upload cleanup: uploaded-but-uncompleted bytes and multipart state are deleted after expiry, cleanup retries survive worker failure, and a completion/expiry barrier proves only one state-machine branch wins without deleting a finalized artifact;
