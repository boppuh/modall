...
| 04 | `feat(identity): add workspace roles auth and audit foundation` | Qualified OIDC freshness adapter, derived cache deadline, privileged authoritative refresh, local-development auth, workspace repositories, RBAC, immutable secret-version adapter, audit | 03 | Live-token group add/remove/revocation proves the end-to-end budget equations, cache-expiry and privileged bypass behavior, static-claim rejection, closed-alpha block/developer-preview classification for unqualified OIDC, and secret-rotation matrix |
