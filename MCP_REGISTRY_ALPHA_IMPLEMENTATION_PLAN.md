...
- OIDC mode cannot be enabled until a provider-specific adapter proves current group membership and revocation propagation within 60 seconds; endpoint presence alone is insufficient, and an unqualified provider falls back to audited local principals.
...
| E1-T3 | Implement provider-specific OIDC authorization-freshness adapter/qualification, epochs, and local principal | M | E1-T2 | Documentation plus add/remove conformance proves current groups and revocation within 60 seconds; missing/static group sources, incomplete refresh, and unqualified IdPs fail deployment/mutation; local fallback is audited |
...
| 04 | `feat(identity): add workspace roles auth and audit foundation` | Qualified OIDC freshness adapter, local auth, workspace repositories, RBAC, immutable secret-version adapter, audit | 03 | Live-token group add/remove/revocation, static-claim rejection, local-fallback, and secret-rotation matrix |
...
| 11 | `feat(invocation): execute MCP tools with bounded inputs and results` | Input scanning/scrubbing, validation, lock-serialized dispatch, cancellation, and per-MIME malware/type/archive/extraction/OCR-classified artifacts | 07, 10 | Input secrecy, races/faults, invalid output, visible/embedded binary secret/PII, incomplete-coverage, malware/archive, and integrity suites |
...
- provider-specific OIDC qualification and live-token add/remove tests prove current group and revocation propagation within 60 seconds; group-less/static issuance-time refresh responses are rejected, incomplete/failed refresh blocks, and unqualified providers cannot enable OIDC mode;
...
- Credential rotation.
- IdP authorization-freshness qualification, including group removal while an original token remains cryptographically valid.
- Restore from backup.
