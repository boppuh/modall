...
2. While retention permits, a routing decision records every candidate, filter, score, input feature, selected implementation, and model/policy version inside the task's encrypted erasable snapshot; ordinary rows expose only random non-catalog-resolvable handles and lose every task-to-catalog edge on erasure.
...
Publishing freezes the capability version, schemas, task claims, price policy, required permissions, and source digest. A `deployment` is a stable operational identity; each immutable `deployment_version` freezes the provider/model revision or local CLI/static-analysis implementation, adapter version, environment, policy, and source/image digest used by an attempt. Operational deployment fields such as current health, capacity, and active version may change, but changes are audited. MCP implementations instead bind to the existing immutable `server_connection_version`; one catalog implementation can never carry both target kinds. Direct Registry runs may retain the exact MCP target. For a task-backed evaluation/routed run, this complete tuple is stored only inside the per-task encrypted execution binding and the run/attempt retains its random unresolvable handle, preserving exact dispatch while allowing task erasure to destroy the task-to-catalog relationship.
...
Persist the normalized task, original request, source-derived routing features, and task-to-implementation mapping only as encrypted task content. Before any durable content write, prepare one unique short-lived provisional external per-task hierarchy with purpose-separated encryption and fingerprint subkeys for the original request, normalized task, feature/candidate snapshots, selected execution binding, serialized dispatch content, and derived usage dimensions; encrypt and fingerprint those values with the provisional hierarchy, then atomically persist only ciphertext/keyed references, the opaque handle in `provisional`, exact current-policy attestations, the task/route/idempotency rows, an optional blocked queue row, and one activation outbox record. Only the idempotent post-commit handler may activate the hierarchy and mark the task usable. Rollback, uniqueness loss, or missing commit leaves no owner and lets the short-lived provisional hierarchy self-destruct; no reader, replay, post-commit routing selection, runnable-queue consumer, dispatch, analysis, or training path may use it. A committed activation job retries only until the immutable provisional expiry. Failure or expiry makes the task and every unfenced routed run non-dispatchable, enters monotonic `task_erasure_pending`, suppresses every descendant result/artifact/grant, destroys/verifies every task and descendant-result subkey plus ciphertext/cache reference, and reaches `task_erased` plus a stable terminal route/run failure only after the complete dependency set is attested erased; it never creates a replacement hierarchy or re-encrypts from unavailable plaintext. Phase 0 evaluation executions and Phase 1 routed Runs reference the activated authority and never copy those bytes into a second per-run envelope. Raw request bytes exist only in bounded no-capture scan/normalization memory, and feature extraction produces an immutable encrypted `task_feature_snapshot` tied to exact extractor, feature-schema, input-scan-policy, and feature-policy versions.

The durable plaintext task/routing allowlist is deliberately content-blind: opaque task/workspace IDs; hierarchy and lifecycle state; non-task-conditioned policy/schema/extractor/router/candidate-universe/health version IDs; effective classification/execution-zone enums; random task-scoped execution and gate-evidence binding handles that cannot be joined to the capability/deployment/evaluation-stratum catalogs; bounded generic lifecycle/erasure reason enums; and timestamps. The selected capability/deployment, selected taxonomy/stratum, exact gate-evidence version/projection/deadline, task-conditioned reason, and mappings from the random handles are ciphertext under the task hierarchy and have only purpose-keyed identities outside it. Original or normalized strings, repository/artifact URIs, paths, revisions, identifiers, counts, buckets, hashes, digests, embeddings, and any other request- or source-derived value are never durable plaintext even when they appear low sensitivity. `task_instances`, `routing_feature_snapshots`, and task execution/gate bindings contain only ciphertext/opaque references, purpose-separated keyed fingerprints, current-policy attestations, and that safe envelope. Queues, caches, events, audit, telemetry, WAL, replicas, backups, and training manifests obey the same rule. When erasure begins, even otherwise safe global version links that could classify this particular task are removed from active projections; terminal history retains only random unresolvable task/workspace/tombstone IDs, generic erased state/time, and legally required non-content financial totals with no task-to-catalog edge.

Every route, replay, dispatch, analysis, or training read requires an active hierarchy and an attestation matching the locked current input and feature-policy pointers. A pointer or classification change immediately suppresses those uses and enters rescan or monotonic `task_erasure_pending`. Every result, result artifact, access grant, run, and attempt created from a routed/evaluation task first receives an opaque dependency on the exact task/cascade generation in the same publication transaction; release paths lock that parent projection as well as independent output policy. If task retention becomes forbidden, one barrier advances the cascade generation, cancels unfenced work, suppresses descendant result/grant reads, and freezes new publication before destroying the original-request, normalized-task, feature, candidate, execution-binding, dispatch, and fingerprint subkeys plus every descendant result encryption/fingerprint hierarchy across active and recovery copies. It purges caches and materialized-dataset membership, removes every task-to-catalog/implementation/stratum link, replaces active identities with generic non-comparable/unresolvable tombstones, and reaches `task_erased` only after the locked complete dependency set is attested erased. A separately unchanged output policy cannot keep a descendant readable. Any derived training dataset is invalidated immediately; a model version whose lineage consumed the task is ineligible for new activation or dispatch until an approved deletion/unlearning procedure produces evidence or the model is retired. Historical state may retain only generic outcome/lifecycle facts and non-content financial settlement totals; it cannot retain a catalog-resolvable selected implementation, task-conditioned reason, or deterministic replay authority after erasure.

The complete task-erasure set also includes the gate-evidence-binding and task-to-reservation mapping subkeys; neither authorization provenance nor accounting lineage may retain a task-to-stratum or task-to-provider oracle after the hierarchy is destroyed.
...
Every benchmark source, diff, hidden label/evidence artifact, retained candidate result, and committed report snapshot also receives an explicit reference-owned `benchmark_artifact_retention_lease` before the referencing benchmark case/version, activation attempt, or report becomes usable. The immutable lease identity binds the exact artifact version, reference owner/version, current classification/source/retention-policy versions, declared `required_through`, and the source/data owner's hard `retention_ceiling`; append-only events and a locked current projection carry its active/released/expired/invalidated state. Admission rejects a benchmark or G1 attempt unless every asset can remain eligible through its preregistered execution, review, and reproducibility window; an active lease may extend the inherited 14-day cleanup default only up to that hard ceiling and never overrides source-authority, privacy, scan, or erasure policy. Artifact cleanup locks and rechecks all live lease projections and cannot tombstone a referenced version before their effective deadlines. Gate evidence carries the minimum dependency deadline; every task-aware route/run/attempt stores a random gate-binding handle whose task-encrypted mapping contains the exact evidence version/projection generation/stratum/deadline, and every initial/fallback fence decrypts that mapping once and rechecks `valid` state and database server time before the deadline. If policy revocation or the hard ceiling requires earlier erasure, one transaction first marks every dependent case/suite/attempt/report and gate-evidence version invalid, disables any task-aware feature flag that cites that evidence, blocks new evaluation reads/runs, and commits the global fence barrier; queued attempts can no longer fence and a durable sweep cancels them/fallback eligibility without requiring a plaintext task-to-evidence index. Only then may artifact key destruction proceed. A historical result may retain safe non-content status, but it cannot remain reproducible or activation-authoritative after an input asset is unavailable.
...
- a random task-scoped execution-binding handle; the exact selected implementation and task-conditioned reason are encrypted under the task hierarchy with the tie-break input and expose only purpose-keyed identities outside it;
- exploration flag and safe experiment version; task-conditioned assignment inputs remain encrypted;
- inside the encrypted candidate snapshot, every candidate's quote/estimate expiry, immutable provider price-version ID, provider-native exact amount, workspace/evaluation base currency, immutable FX-rate-set/version and exact conversion factor when currencies differ, normalized exact amount, and worst-case reservation vector; outside it while active, only a random task-scoped reservation handle and required non-content monetary totals/FX evidence. The erasable task-scoped mapping supplies any attempt-to-provider/price linkage and is destroyed or severed after settlement when task erasure begins;
- correlation and trace IDs.

The candidate/feature/selection snapshot is append-only ciphertext, but its usability is not permanent: it inherits the per-task current-policy and erasure lifecycle in Section 7.2. The selected taxonomy literal, taxonomy-derived features, candidate identities, selected exact implementation, task-conditioned reason, row count, filter outcomes, scores, and individualized estimates that depend on request/source features cannot be duplicated into ordinary decision, run, attempt, event, audit, index, or analytics columns. A worker resolves the random task-scoped binding to exactly one encrypted capability/connection or deployment version only while the hierarchy is active and rechecks that exact target under the fence locks. Erasure destroys that mapping and removes task-conditioned global-version links from current projections. After task erasure, only generic outcome/lifecycle facts, non-content settlement totals, random unresolvable tombstones, and opaque schema lineage that cannot be expanded through the task record remain; neither task category nor selected implementation is recoverable from the task/route/run/attempt rows or restored durable copies.
...
Every task-aware route, run, and attempt binds a random non-catalog-resolvable gate-evidence handle. Its task-encrypted, purpose-keyed mapping contains the exact gate-evidence version, admitted current-projection generation, `evaluation_stratum_id`, and `artifact_dependency_valid_through`; static/shadow selections explicitly carry no activation authority. Before every initial or fallback dispatch fence, the worker decrypts the mapping while the task hierarchy is active, locks that evidence projection with the keyed binding/task/route/attempt, and requires it still be the cited current `valid` version for the exact stratum and database server time strictly before the dependency deadline. Evidence invalidation atomically advances that projection and disables the flag first, creating a global fence barrier; a durable sweep then cancels unfenced jobs/attempts and closes fallback eligibility without a plaintext task-to-evidence index. A fence that linearized first remains under ordinary fenced/indeterminate semantics, but no queued selection can acquire a new fence from expired or revoked evidence. Task erasure destroys the gate mapping, so retained history cannot reveal its stratum.
...
| P0-02 Taxonomy | Versioned task and feature schema plus Phase 0 task-content/key authority | JSON Schema validation; feature provenance; no post-outcome fields; one provisional hierarchy encrypts original/normalized/features/candidates/selected execution and gate-evidence bindings before persistence, commits with its activation outbox, activates only after commit, and makes rollback/activation-failure ciphertext undecryptable and unusable | Backend/domain |
| P0-03 Corpus | 75–100-case calibration suite plus fresh per-attempt representative activation holdouts whose first universal tranche is 200–300 cases and final size follows the complete joint-gate simulation | Each scored family has >=10 dual-reviewed present and absent units and per-family/global kappa >=0.80; labels freeze mechanisms; post-reveal gaps retire the attempt; every asset has an exact-version retention lease covering the declared review/reproduction window within its source ceiling; each holdout has >=90% conservative probability of clearing the success LCB >5-point and complete cost gate under the above-margin planning alternative, and cannot authorize unrepresented/private traffic | Swift/evaluation |
| P0-04 Harness | Durable execution and restricted artifact capture | Resumable, idempotent matrix runs; reference-owned artifact retention blocks default cleanup only within approved source ceilings and invalidates dependent evidence before forced erasure; zero-null legacy visibility/binding backfill precedes atomic RLS activation; random task-scoped handles resolve one encrypted exact provider binding; every result binds the parent cascade; evaluation-only domain is enforced below repositories; hidden data is absent from ordinary/Alpha-compat APIs; pinned primary slots and environment | Platform |
...
| P0-07 Router V0 | Filter/rank/reason implementation | Deterministic replay from active/current encrypted task/selection snapshots; no hidden data access; erased tasks destroy selected-target mappings and descendant outputs, retaining only generic unresolvable non-replay tombstones | Backend/data |
...
Phase 1 inherits the Registry Alpha authorization-freshness contract without resetting any timer: the conservative provider propagation bound `P`, qualified-source lookup allowance `R`, server snapshot TTL/poll interval `L`, and safety margin `M` must satisfy `P + R + L + M <= 60 seconds`. Privileged mutations, every initial/fallback dispatch fence, artifact grant mint/replay/redemption, every bounded HTTP artifact-response chunk, and every `read_artifact` chunk synchronously bypass the shared authorization cache and require `P + R + M <= 60 seconds`; timeout or incomplete source data fails closed. Upload-target mint, replay, the encryption-gateway pre-first-byte and pre-ciphertext-commit checks, periodic checks, and completion do the same; a stream lasting beyond a configured authorization interval `U` pauses for another qualified check, with `P + R + U + M <= 60 seconds`. Each HTTP artifact chunk and complete MCP `read_artifact` response receives a server-authored authorization deadline no more than `D` after its last qualified check and no later than its grant, inbound client-session authentication, or admitting-credential expiry; decryption, serialization, and bounded transport write must finish before that deadline or the server wipes buffers and terminates the response/transport, with `P + R + D + M <= 60 seconds`. Each run, upload, access grant, derived bearer token, inbound MCP client-session authorization projection, and outbound MCP server-session proof binds its originating actor and admitted human authorization epoch plus the exact admitting API-credential projection when one is used. A dispatch fence requires a fresh qualified lookup plus locked current membership/execution-role/unchanged-epoch check, an upload-write check requires current membership/upload permission, unchanged human epoch, unchanged active API-credential version/revocation epoch/upload scope when present, active upload/content/source hierarchies, and an unfenced lease, and every HTTP or MCP artifact-read operation requires current artifact-read permission, unchanged bound human/API-credential projections, an active exact-matching source-authority projection when one is bound or the exact current workspace unverified-upload-floor version otherwise, and current artifact/scan/task-parent state. Any human or bound API-credential authorization change cancels unfenced execution, atomically enqueues fencing/abort of every affected open upload lease, invalidates affected inbound client-session projections and outbound leases/proofs, and makes affected grant replay/redemption or subsequent HTTP/MCP response chunks fail on their next locked check; gateway mismatch aborts the multipart/ciphertext commit, advances cleanup, destroys every upload-content and source-identity subkey, and returns no artifact. Server-authored snapshot/stream authorization expiry propagates to clients and gateways so caching cannot add another freshness interval. Phase 1 also preserves Registry Alpha's immutable MCP binding identity: negotiated protocol revision participates in every MCP capability-version digest, and a protocol switch creates a fresh `pending_review` version before routing or execution can use the changed wire contract.

Admission through an API credential additionally binds the exact credential ID/version and its current revocation epoch, action scopes, workspace/environment scope, and applicable spending/quota ceiling into every route, run, artifact upload/target, artifact access grant/token, inbound MCP client-session authorization projection, and outbound MCP server-session lease/proof. Before every outbound session-initialization lease mint/redemption/proof publication, every initial/fallback dispatch fence, every upload-target mint/replay, first-byte, periodic, ciphertext-precommit, and completion check, every access-grant mint/replay/redemption, and every inbound MCP meta-tool call/write, the qualified authorization path locks that credential's current projection and requires it to remain active, unexpired, and authorized for the admitted action, workspace, environment, and at-least-as-permissive applicable ceilings. Phase 1 extends the Alpha `mcp_session_initialization_leases` and live-session proofs only for direct and routed outbound MCP execution, adding the nullable exact admitting API-credential ID/version/revocation epoch/scopes/ceilings. Separately, a new `mcp_client_session_authorization_projection` represents authentication of a client to the platform MCP facade and binds subject, workspace, human authorization epoch, authentication expiry, and nullable exact API-credential lineage/scopes/ceilings; it is not an execution lease and cannot authorize a target-server initialization or dispatch. The outbound pre-session transaction performs the fresh human lookup, locks both human and API-credential projections in the shared identity class, and refuses to mint/redeem a lease on any mismatch before retrieving the target-server credential or writing initialization bytes. API-credential mutation invalidates affected unredeemed outbound leases, outbound proofs, initialized target transports, and inbound client-session projections/transports. An outbound proof publishes only by compare-and-set against the still-current credential projection and is consumed only by its run's dispatch fence; every inbound meta-tool call, including each `read_artifact` call, instead locks and rechecks its own client-session projection and the current human/API-credential projections. Revocation, rotation without explicit continuity, expiry, or scope reduction therefore cancels unfenced runs, fences every open upload lease, and denies every access-grant replay/redemption or MCP artifact read admitted by that credential with `api_credential_authorization_changed`, even when the human subject's membership epoch is unchanged. A precommit token may prove that ciphertext commit was authorized before a later credential mutation, but completion still rechecks the credential and returns no artifact after the mutation; cleanup destroys both upload hierarchies. Credential mutations, outbound session initialization, inbound client calls, upload checks, grant checks, artifact reads, and dispatch fences share the same lock order.

Endpoints:

- `POST /v1/artifact-source-authorities` — validate and classify the qualified connector or audited data-owner approval in bounded no-capture memory under the exact current source-authority-metadata scan/retention policy, allocate a random non-enumerable authority ID, prepare one unique short-lived provisional external hierarchy with purpose-separated envelope-encryption and keyed-identity subkeys, and encrypt the raw repository/signing identity before its first durable write. Atomically persist only ciphertext/opaque references, the provisional handle, keyed identity, first immutable current-policy attestation/material version/refresh-evidence row, `activation_pending` projection, and activation outbox. Only idempotent post-commit activation makes the authority usable; rollback/uniqueness loss self-destructs, and missing/failed activation enters attested erasure without reminting. Caller-supplied IDs, floors without the dedicated permission, and unqualified or policy-forbidden evidence fail closed
- `POST /v1/artifact-source-authorities/{id}/refresh` — require the authority envelope hierarchy active and its exact-content attestation current, capture the exact current material version/generation and connector configuration, decrypt the locator only into bounded no-capture memory, obtain qualified connector visibility/classification evidence without database locks, and wipe the locator. Then reacquire the shared source-authority locks and require that captured projection/configuration/attestation still current before appending immutable refresh evidence and compare-and-setting its bounded `valid_through`. Append and select a new immutable material version/generation only when security-relevant state changed; an unchanged successful refresh leaves that material identity stable, while late evidence from a superseded/revoked projection and timeout/incomplete evidence cannot extend freshness or revive authority
- `POST /v1/artifact-source-authorities/{id}/revoke` — synchronously append a typed revocation event and advance the current projection before queuing dependent revalidation/erasure and authority-envelope reclassification under the current source-authority-metadata policy; all later upload, grant, read, route, and fence checks fail from the pointer without waiting for fan-out, while a policy-permitted audit envelope stays encrypted and unusable as authorization
- `POST /v1/artifact-source-authorities/{id}/erase` — synchronously revoke and suppress the authority, queue dependent artifact reclassification/erasure, and enter `authority_erasure_pending`; destroy both authority-envelope subkeys and recovery copies, purge ciphertext/keyed-identity caches, and install a generic non-comparable tombstone before terminal `authority_erased`. Terminal erasure requires destruction evidence plus every dependent link either rebound under an allowed current authority/unverified floor or source-erased; failure remains revoked, unreadable, and alerted
- `POST /v1/artifact-uploads` — synchronously reauthorize current membership/upload permission and, when API-credential authenticated, lock/revalidate that exact credential's active version/revocation epoch plus `artifact_upload:create` workspace/environment scope and applicable ceiling. When trusted provenance is claimed, lock an active/fresh source-authority projection and bind its exact random ID/material version/generation into the upload and target together with the immutable server-derived classification floor and upload-specific provenance-attestation version whose signed content identity/manifest is expected for this exact upload; otherwise lock and bind the exact current workspace unverified-upload-floor version into both. Also bind the originating subject/workspace, current human authorization epoch, and nullable exact API-credential identity/epoch/scopes/ceilings. Under the shared quota lock order, atomically reserve the declared maximum ingress bytes, prospective stored bytes, one upload-concurrency slot, and worst-case gateway/storage spend against both workspace and credential accounts before committing the upload; insufficient aggregate capacity creates no upload or target. Provision/activate both (a) a unique external per-upload content hierarchy and (b) a distinct unique external per-upload source-identity hierarchy, each with purpose-separated encryption/fingerprint subkeys, and return one dedicated short-lived `EphemeralUploadTarget` for the service-controlled streaming encryption gateway scoped to that reservation and a unique create-only ciphertext object/version. Before the database commit, encrypt the complete upload-specific connector/data-owner provenance envelope—including repository identity, visibility, commit, expected canonical identity/manifest, and signature—under the source hierarchy; ordinary rows retain only the random authority ID/version/generation or unverified-floor version, safe classification/policy versions, ciphertext/opaque references, and keyed identity. A reusable logical source authorization is cloned into a fresh encrypted envelope for each upload and never supplies a shared content-identity key. A client declaration may raise but never lower the floor, and no target is minted before both hierarchies are active and the reservation is durable
- `POST /v1/artifact-uploads/{id}/complete` — synchronously reauthorize the bound subject/upload permission/unchanged human epoch and, when present, the bound API credential's exact active version/revocation epoch plus `artifact_upload:complete` workspace/environment scope and applicable ceiling. Then under the shared identity/provenance/artifact/policy lock order require server time strictly before `expires_at`, an active/fresh source-authority projection whose version/generation exactly matches the upload-specific attestation when trusted provenance was claimed—or the exact current workspace unverified-upload-floor version otherwise—active upload and source-identity hierarchies, and exactly one consumed `ciphertext_commit_fenced` token whose final qualified human/API-credential checks are within `U` and whose committed object/version/size/integrity receipt matches; an active/unconsumed lease is invalid. Close the upload, verify the gateway receipt plus ephemeral client integrity proof and plaintext size/type, decrypt the source envelope and uploaded bytes only into bounded scan quarantine, transiently verify the signature/connector evidence and compare the computed canonical identity with its expected identity, then wipe both and persist only purpose-keyed identities and a safe comparison result. Require an attestation against the current source-authority/classification or unverified-upload-floor, source-retention, output/artifact, and MIME-profile policies. Derive effective classification as the maximum of that current trusted floor, client declaration, and detector/profile result; if retention is allowed, the finalized artifact inherits the upload hierarchy and its dedicated source-identity envelope and mints the authoritative `artifact://` URI without a plaintext rewrite. Human or API-credential authorization loss, stale/revoked source authority, stale unverified floor, provenance/token mismatch, or scope reduction stays quarantined, returns no artifact, and enters cleanup that destroys both hierarchies. Sensitive/failed content whose policy forbids retention enters `upload_erasure_pending`, returns no artifact, and becomes `upload_erased` only after every upload-content and source-identity subkey is destroyed and ciphertext cleanup is attested; an expired `open` row atomically moves to `cleanup_pending` with the same dual-hierarchy destruction requirement even if the sweeper is late
- `GET /v1/artifacts/{id}` — return authorized metadata only after locking an active/fresh source-authority projection whose version/generation exactly matches the artifact attestation when bound, or the exact current workspace unverified-upload-floor version otherwise, plus the current trusted source-floor and policy/profile scan eligibility/readiness, exact immutable ciphertext storage-version identity, and a clean-content digest only when policy permits; the digest is computed transiently during authorized decryption and never persisted. Stale/revoked provenance authority, stale unverified floor, or scan/key state reports safe `source_revalidation_required|rescan_required|erasure_pending|erased` metadata and no viewer mode, never a keyed plaintext fingerprint or ambient object-store credential
- `POST /v1/artifacts/{id}/access-grants` — authorize the current subject, workspace, active exact-matching source-authority projection when bound or exact current workspace unverified-upload-floor version otherwise, trusted source floor/effective classification, retention, integrity, and exact current output/artifact scan attestation before mint or replay. When API-credential authenticated, lock/revalidate and bind that exact credential ID/version/revocation epoch plus `artifact:read` workspace/environment scope and applicable ceilings. Mint the sealed Alpha `ArtifactAccessGrantToken` bound to the subject, human authorization epoch, exact artifact version, nullable admitting API-credential projection/scope, and exact applicable source-authority ID/version/generation or unverified-floor version plus source/scan attestation versions; stale/revoked provenance authority, stale unverified floor, stale scans, or either authorization mismatch returns no token, ordinary replay storage retains only a non-secret reference/HMAC verifier plus safe binding IDs, and the encrypted token envelope remains replayable through exact grant expiry and becomes logically unreadable then
...
- `GET /v1/artifacts/{id}/content` — require ordinary authentication plus the subject/workspace/version/human-authorization-epoch, nullable API-credential projection, and exact applicable source-authority-or-unverified-floor/source-attestation/scan versions bound into the grant in a redacted header. On initial redemption and before every at-most-256-KiB response chunk, first require database server time strictly before grant expiry, then pause for a cache-bypassing qualified authorization refresh. Only on success set `download_authorization_expires_at = min(source_checked_at + D, grant_expires_at)`, require current server time strictly before that new deadline, and under the shared locks reauthorize current membership/role, epoch, visibility, an active/fresh exact-matching source-authority projection when bound or exact current workspace unverified-upload-floor version otherwise, trusted source floor/effective classification, retention, integrity, and exact current source/output/artifact-scan-policy plus MIME-profile attestations; all must still equal the grant bindings. If an API credential admitted the grant, the current request must authenticate as the same credential and the locked exact version/revocation epoch must remain active with `artifact:read` workspace/environment scope and at-least-as-permissive applicable ceilings. The server decrypts or buffers no next chunk until that check succeeds and gives the bounded write the same deadline, terminating the connection instead of continuing writes through backpressure after it. Grant expiry, revocation, source-authority or unverified-floor drift, either identity change, scan/policy drift, or refresh/write timeout terminates the isolated no-store stream before any later chunk and the old grant never becomes valid again after revalidation
- `POST /v1/routes` — scan in bounded no-capture memory, prepare a short-lived provisional per-task external hierarchy, encrypt the original request, normalized task, extracted feature/candidate snapshot, selected implementation/reason, exact gate-evidence mapping, and task-to-reservation mapping before their first durable write, then atomically persist only random non-catalog-resolvable execution/gate/reservation handles with the opaque provisional handle, current attestations, content-blind route envelope, idempotency state, and activation outbox; the route is unusable until post-commit activation, and rollback/activation failure self-destructs or explicitly destroys the hierarchy without replacement
- `POST /v1/routed-runs` — perform the same provisional task-content admission or reference an authorized active task hierarchy, atomically route, authorize, create the canonical Run resource with only random task-scoped execution/gate/reservation handles and no copied task bytes or catalog target IDs, enqueue, and return `202 Accepted`; a job referencing a newly committed provisional hierarchy remains nondispatchable until activation, and activation failure terminally fails unfenced work and descendant publication only after task-key destruction is attested
...
Every task-derived result/artifact endpoint above—including metadata, grant mint/replay/redemption, each HTTP chunk, each MCP `read_artifact` response, and outcome attachment—also locks and requires the exact active parent task/cascade generation bound before publication. Entering `task_erasure_pending` advances that generation, suppresses all of those paths immediately even when output policy is unchanged, and revokes descendant grants before their key-destruction cascade; no clean output rescan can revive them.
...
The precommit authorization transaction is the write linearization point: while holding the human-identity/API-credential/source-authority/upload locks it atomically consumes the open write lease and records one `ciphertext_commit_fenced` token bound to the current human authorization epoch, nullable exact API-credential version/revocation epoch/scopes/ceilings, exact applicable source-authority ID/version/generation or workspace unverified-upload-floor version, exact object/version, integrity, byte count, and authorization check. The gateway may perform the external ciphertext commit only with that token and accepts no later bytes. A human, API-credential, source-authority, or unverified-floor mutation locking first prevents the token; one locking afterward observes that ciphertext commit was already authorized but still marks the upload non-finalizable, so completion returns no artifact and key-destroying cleanup removes any materialized ciphertext. Lease recovery can never create or replay a second commit token.

Ordinary durable records retain only the opaque upload ID, bound originating subject/workspace/admitted human authorization epoch, nullable exact API-credential ID/version/revocation epoch and admitted scopes/ceilings, exact applicable random source-authority ID/version/generation or workspace unverified-upload-floor version, last qualified human/API-credential/source-authority-or-floor check/expiry and safe decision, safe gateway/origin identifier, never-reused ciphertext key/version scope, opaque upload-content and source-identity hierarchy handles/states, their purpose-keyed fingerprints, ciphertext integrity, safe classification/policy versions, lifecycle state `open|completed|cleanup_pending|upload_erasure_pending|upload_erased|deleted`, expiry, qualified `W`/`C`, authorization/write-fence/active-lease evidence, cleanup attempts, quiescence deadline, and key/deletion attestations—never plaintext, a content key, raw identity response, source payload/signature, repository/commit/manifest identity, publicly verifiable digest, or ordinary plaintext digest. A gateway/provider event or ciphertext HEAD restricted to that key records the actual version even when the client never calls completion; create-only policy permits at most one version. Human, API-credential, source-authority, or unverified-floor mutation, gateway check failure, completion, and expiry cleanup lock the same identity/credential/source-authority/upload subsequence and compare database server time. Completion may finalize only an `open` row with `now < expires_at`, unchanged current human authorization epoch, current upload permission, an unchanged active API-credential projection when one admitted the upload, an active/fresh exactly matching source-authority projection when claimed or exact current workspace unverified-upload-floor version otherwise, a fresh successful gateway check, and both hierarchies active/current; expiry, authorization loss, or applicable source-authority/floor mismatch atomically enters `cleanup_pending`, fences/cancels writes, and returns no artifact. The sweeper destroys every upload-content and source-identity encryption/fingerprint subkey, aborts the session, and repeatedly deletes their exact ciphertext versions through all leases and at least the later applicable expiry/fence time plus `W + C`; an object appearing during the interval restarts verification. `cleanup_pending -> deleted` requires attested destruction of both hierarchies, a closed/hard-expired lease set, elapsed quiescence, and provider-consistent absence. Completion-time retention rejection follows `open -> upload_erasure_pending -> upload_erased`, with immediate read/finalization/source-comparison suppression and terminal state only after both hierarchies are destroyed; ciphertext deletion continues independently and failure remains alerted. A late object after terminal attestation triggers delete plus incident but is already undecryptable. A completed artifact can enter only its separate policy-reclassification erasure path, which couples artifact-content and source-identity destruction, never abandoned cleanup. Application/proxy/CDN/browser/MCP-host capture is disabled or redacted. Browser and MCP clients stream directly to the non-navigating gateway primitive with no ambient cookies/referrer and immediate buffer disposal; unsupported MCP hosts receive no target. The target credential never enters generic URL/DOM/model/resource/database/audit/error/history/analytics/log/trace surfaces.
...
Artifact source sensitivity is independent of malware/secret/PII scanning and uses the ordered policy lattice `public < non_confidential < private_internal < restricted_sensitive`. Every trusted connector repository or data-owner approval first receives a random, non-enumerable `source_authority_id`; it is never derived from repository/source content. Its raw repository identity, connector locator, signing scope, and approval content are encrypted before first persistence under a unique external `artifact_source_authority_content_keys` hierarchy with separate encryption/fingerprint subkeys, never an application-stored key or publicly testable digest. The authority remains unusable until the provisional hierarchy and current `artifact_source_authority_scan_attestation` activate after the owning transaction commits. Immutable material `artifact_source_authority_versions` record the authority kind, active/revoked status, classification floor, and connector or signing-approval generation. Immutable `artifact_source_authority_refresh_evidence` records qualified observations and bounded validity, while one locked row in `artifact_source_authority_current_projections` selects the authoritative material version/generation plus latest refresh evidence, `valid_through`, envelope-key state, and exact current source-authority-metadata-policy attestation. An unchanged successful poll advances only the evidence/freshness pointer, so it does not invalidate artifacts bound to the same material version. Connector webhooks plus mandatory qualified polling append a new material version when repository visibility/classification changes and make the projection stale at the deadline if refresh fails; data-owner revocation advances it synchronously to a revoked material version. Authority metadata-policy drift suppresses refresh and every dependent use before decryption. A qualified quarantine reclassification may append a superseding attestation only for the same keyed envelope identity under the still-current policy; if retention becomes forbidden or the authority is erased, both authority subkeys and recovery copies are destroyed before the envelope/keyed identity are tombstoned and terminal state is allowed.

Upload creation resolves a trusted `source_classification_floor` from the locked active/fresh source-authority projection and either (a) its server-side repository connector attesting exact repository identity, visibility, commit, and canonical bundle/manifest digest, or (b) its approved immutable repository-source attestation signed by the designated data owner over the expected canonical artifact identity and classification version. The entire exact upload-specific signed/connector provenance payload is content: encrypt it before its first durable write under the separate per-upload source-identity hierarchy. Never persist its raw signature, repository identity, commit, manifest, ordinary digest, or publicly verifiable candidate oracle outside that ciphertext; only the random source-authority ID, its admitted material version/generation, and safe key/classification/policy version IDs may remain in the ordinary envelope. Completion independently computes the uploaded content identity and requires an exact transient match; a caller cannot attach a public attestation to different bytes. An arbitrary, mismatched, unverifiable, revoked, or stale-authority upload has no trusted public provenance and either fails completion when an authority was claimed or inherits the workspace's `unverified_upload_floor`, which defaults to `private_internal`, when no authority was claimed. Only a service identity or subject holding the dedicated `artifact_provenance:attest` permission may create/change a source attestation, and the action is audited without its content. The uploading client can declare a more restrictive class but cannot write the floor, provenance authority, or effective class. Completion derives `effective_artifact_classification = max(current_source_authority_floor, client_declared_classification, detector_or_content_profile_classification)`. A clean detector never proves public ownership or lowers proprietary source.

When trusted provenance is claimed, the upload-specific attestation, target, final artifact, route snapshot, and every attempt bind the exact random authority ID plus admitted material version/generation. Otherwise they bind the exact workspace unverified-upload-floor version. Completion, grant mint/replay/redemption, every HTTP/MCP read chunk, routing, and every initial/fallback dispatch fence lock the applicable current projection and require exact equality; an authority-backed use additionally requires the authority-envelope hierarchy active and its exact keyed-identity attestation current before releasing bytes or sending. An authority material change/revocation/staleness, authority metadata-policy drift/key loss, or unverified-floor change immediately yields `source_revalidation_required` without waiting for fan-out. A qualified revalidation may append a superseding upload-specific attestation only after decrypting and rechecking the exact immutable provenance/content identity under the new current authority version—or reclassifying an unverified artifact under the new workspace floor—and recomputes effective classification without lowering it. Thus a public-to-private repository transition or revoked signing approval blocks all formerly public use before asynchronous reclassification. If the new floor still permits retention, revalidation may restore only policy-compatible access under that stricter floor; if retention is forbidden, atomically enter source erasure with artifact/upload erasure, suppress every comparison and use, destroy every source encryption/fingerprint subkey and recovery copy, and retain only a generic non-comparable tombstone. Artifact/upload terminal erasure cannot precede attested destruction of both its content hierarchy and its dedicated source-identity hierarchy, and tombstoning removes the artifact-to-authority link so the surviving random authority row cannot identify an erased artifact.

Routing and run requests accept only finalized, unexpired artifacts owned by the same workspace, allowed by request/provider data policy, carrying an active/fresh exact-matching source-authority projection when bound or exact current workspace unverified-upload-floor version otherwise, an active external content hierarchy, and an append-only scan attestation over the exact immutable keyed identity that matches the locked current output/artifact-scan-policy version and applicable MIME-profile version. Every upload reaches object storage only as authenticated ciphertext at a unique non-overwritable key/version through the encryption gateway; neither the gateway nor application spools plaintext. Completion verifies the client/gateway proof and length against that exact version, decrypts only into bounded no-network quarantine, detects archive expansion/path traversal, and applies the Registry Alpha malware/type plus per-MIME extraction, all-page/frame render/OCR, secret/PII classification, and complete-coverage policy; a type without a complete tested profile remains quarantined. All uploaded inputs retain only ciphertext integrity and a purpose-separated keyed plaintext fingerprint, never an ordinary persistent plaintext digest. Activating a changed policy/profile immediately makes older attestations `rescan_required` and blocks key use, grants, reads, routing, and dispatch. A durable job may decrypt for quarantine rescan only when commit-time locks prove the same keyed identity/hierarchy, current source authority or unverified floor, and current policy/profile. If policy still permits retention it appends a superseding attestation; if policy forbids retention it enters erasure pending, destroys encryption/fingerprint subkeys before terminal erasure, and deletes ciphertext best effort, leaving historical versions/replicas/backups unrecoverable even if physical deletion lags. Consumption reauthorizes and reads only that pinned ciphertext version with the applicable current source projection/attestation and active hierarchy. HTTP/SDK clients upload through the gateway target; the MCP facade exposes both upload controls only through the non-recording channel above. Large results use the same immutable artifact authorization contract, so the console never reads object storage directly.
...
For result consumption without REST credentials, `read_artifact(artifact_uri, offset, max_bytes)` uses the inbound `mcp_client_session_authorization_projection` bound to its subject, workspace, admitted human authorization epoch, authentication expiry, and nullable exact admitting API-credential ID/version/revocation epoch/scopes/ceilings; an outbound target-server live-session proof is neither accepted nor consulted. Every call obtains a cache-bypassing qualified authorization snapshot and, under the shared identity/client-session/provenance/artifact/policy/task-parent lock order, locks and rechecks that inbound projection, the subject, workspace membership/role, authorization epoch, and—when present—the current active, unexpired credential projection with `artifact:read` workspace/environment scope and at-least-as-permissive ceilings. It also rechecks artifact visibility, an active/fresh source-authority projection exactly matching the artifact attestation when bound or exact current workspace unverified-upload-floor version otherwise, current trusted source floor/effective classification, retention, exact immutable version/kind-appropriate integrity state, active result-key state when applicable, the active matching parent-task cascade generation for a task-derived result, and an attestation matching the current output/artifact-scan-policy plus applicable MIME-profile versions. Only then does the server set `artifact_read_authorization_expires_at = min(source_checked_at + D, client_session_authentication_expires_at, admitting_api_credential_expires_at when present)`. Decrypting, producing, serializing, and writing the complete at-most-256-KiB tool result must finish before that deadline; timeout or backpressure wipes all plaintext/base64 buffers and terminates the response/transport rather than emitting later bytes. It returns total length, detected content type, a transient clean-content digest only when policy permits, and the next offset; keyed plaintext fingerprints remain internal. Safe text/JSON uses typed content, and only explicitly allowlisted bytes that completed their full current content-aware profile use base64. Repeated bounded calls can consume a large result, while stale/incomplete authorization; human/API-credential/client-session mismatch; source authority/provenance/floor mismatch; parent-task cascade change; `source_revalidation_required`; `rescan_required`; result erasure; active, encrypted/uninspectable, unknown, unsupported, type-mismatched, coverage-incomplete, quarantined, expired, cross-workspace, integrity-failed, or policy-forbidden content fails closed. The tool never returns an ambient object-store URL or credential, and `P + R + D + M <= 60 seconds` bounds the last bytes authorized by a stale identity or authority snapshot.

Acceptance criteria:

- identical idempotent requests produce one route/invocation;
- cross-workspace access tests fail closed;
- unfinalized, expired, overwritten, wrong-version, integrity-mismatched, stale/revoked-source-authority, stale-scan, unsafe, and cross-workspace artifacts are rejected before routing, enqueue, viewing, or download;
...
- a currently authorized subject can view safe text/JSON or download other content only through an active, unexpired subject/workspace/artifact-version/human-epoch-bound grant that also binds the nullable exact admitting API-credential projection and `artifact:read` scope, exact applicable source-authority-or-unverified-floor/source-attestation/scan versions, and required isolation headers. Initial redemption and every bounded HTTP response chunk receive a server-authored deadline no later than grant expiry and `D`; the stream pauses for a cache-bypassing human/API-credential, grant, source-authority-or-unverified-floor, artifact, and policy recheck before decrypting or emitting the next chunk. Individual revocation, admitting-credential mutation, source-authority drift/revocation/staleness, unverified-floor drift, or post-mint access/policy/retention/output-scan/MIME-profile change denies token replay and later bytes immediately on the next authoritative check; qualified revalidation/rescan requires a new grant and never revives the old binding;
...
- an MCP-only result client can consume safe text/JSON or bounded base64 chunks through `read_artifact` using its bound inbound client-session authorization projection; every call revalidates that projection plus the exact human/API-credential, parent-task, source/artifact/policy state and completes decrypt/serialization/write within the server-authored deadline, with the same authorization and integrity decisions as HTTP retrieval;
...
- while the task hierarchy remains active, an authorized response may decrypt and return the current immutable `evaluation_stratum_id`, selected gate-evidence version if any, and `task_aware|static|shadow_only` disposition; ordinary durable response/replay rows retain only the random task-scoped handles and encrypted mapping, and task-aware selection is impossible unless the request exactly matches a passing stratum;
...
Use curated HTTP/model/CLI adapters behind one async interface. Separate benchmark and production worker pools. Enforce per-provider concurrency, circuit breaking, retry budgets, absolute deadlines, and result-size limits. Every fenced side-effecting HTTP/MCP execution disables automatic redirects: any 3xx response is never followed or rewritten, sends no body to `Location`, becomes `indeterminate` because the original receiver may have acted, and suspends that target for reverification. Before canonical request persistence, apply the Registry Alpha bounded fail-closed input secret/PII scan; raw credentials are accepted only through opaque adapter secret bindings. Before a direct or routed MCP run initializes its server session, the inherited pre-session lease/proof path additionally binds and revalidates the admitting API-credential projection when present; mutation invalidates the lease/proof and cancels its transport before later initialization/tool-call traffic. A direct Registry run persists retained clean arguments only through its inherited unique per-run external hierarchy. A routed run instead references the unique per-task hierarchy from Section 7.2 for original, normalized, feature, and serialized dispatch content and never duplicates those values into a per-run envelope. Both use purpose-separated encryption/fingerprint subkeys and ciphertext/keyed envelopes; queues, replay state, caches, WAL, replicas, and backups never receive plaintext or key material. For every initial or fallback fence, decrypt once into the inherited single-use `ScannedArgumentLease`, re-scan and bind its exact hierarchy-keyed fingerprint/current input and feature policies in the fence, retain that same in-memory lease and serialization buffers until the transport reports the full request body written or definitive zero-byte failure, and then wipe them; never decrypt again after fencing. A partial write or post-fence crash follows no-retry indeterminate recovery. If a newer policy rejects previously queued content, inherit the applicable Alpha/task erasure state machine: immediately suppress reads/replay/dispatch/comparison/analysis/training, enter erasure pending, verify irreversible destruction of every content and fingerprint subkey for all durable copies, replace active identities with generic non-comparable tombstones, purge caches/references/derived-dataset membership, and only then finalize rejection; historical keyed bits in WAL/backups remain untestable, and delay/failure remains quarantined/non-comparable/non-dispatchable. While its encrypted task hierarchy remains active, a persisted route can supply authorized historical selection evidence; after erasure, only generic unresolvable lifecycle/settlement tombstones survive, with no continuing dispatch authority, feature replay, or task-to-selection linkage.

For task-backed execution, the worker also decrypts the exact selected capability/target mapping from the random task-scoped binding into that single-use lease before taking database locks; durable run/attempt/fence/event rows retain only the random handle and keyed mapping attestation, never a catalog-resolvable selected implementation. The fence locks and verifies the resolved current target, task hierarchy/cascade, mapping attestation, and exact gate-evidence projection/generation/dependency deadline. It cannot commit after task erasure begins or after gate evidence becomes invalid/expired. Task erasure cancels unfenced work, suppresses all descendants, destroys the execution-binding mapping plus descendant result hierarchies, and retains only generic non-expandable lifecycle/settlement history.

Phase 1 extends the Alpha shared lock-plan helper while preserving the relative order of every common class. A fence performs its qualified actor-source network lookup before taking database locks. After an API idempotency lock when applicable, coordinated transactions acquire: (1) actor user, workspace membership, current authorization-epoch projection, and the admitting API-credential current projection when present, ordered by workspace, subject, then credential ID; (2) provider current-price and FX-rate-set/selectors; (3) stable server-connection or deployment rows; (4) exact/current target configuration and MCP discovery pointers/snapshots; (5) capability-status projections; (6) source-authority identities, envelope-key/scan-attestation state, immutable material versions, current version/generation/freshness projections, upload-specific provenance attestations, repository-classification selectors, and workspace unverified-upload-floor pointers; (7) artifact and artifact-upload rows; (8) current source-authority-metadata, schema-scan, input-scan, task-feature, and output/artifact-scan policy plus MIME-profile pointers/versions, ordered by policy kind then MIME; (9) workspace/provider/data/destination policy rows; (10) quota/budget accounts ordered by scope, currency, and unit; and (11) task hierarchy, route, run, attempt, job, and reservation rows in that order. IDs within a class are sorted. Fence workers release lease/job locks before starting this transaction; human/API-credential identity, price/FX, disable, source-authority refresh/revocation/reclassification/erasure, artifact upload/publication/access/rescan, task-content rescan/erasure, scan-policy/profile activation, other policy, and quota mutations use only the same relevant subsequence. Advancing a source-authority/classification or scan-policy/profile pointer atomically invalidates older eligibility/attestations, so activation never requires locking every artifact or task. Reverse-order input and opposite worker/control start tests fail on any timeout, deadlock, raw coordinated lock outside the helper, or order inversion.

The helper places gate-evidence current projections and retention-lease projections with their policy inputs before task/route/run/attempt rows, and places parent-task dependency/cascade rows before descendant result/artifact/grant rows. Gate invalidation takes that same subsequence, advances the authoritative projection/flag barrier first so every referencing attempt fails its fence, and then durably sweeps unfenced attempts without a plaintext task-to-evidence index; task erasure advances the parent cascade and suppresses all descendants before their key destruction. MCP artifact reads take the identity/API-credential, inbound client-session projection, source-authority, artifact/policy, parent-task, and result-key subsequence, release the locks only after installing the bounded response deadline, and never treat the client-session projection as authority beyond that deadline. Outbound target-server session leases/proofs remain in the execution lock subsequence and cannot substitute for the inbound projection.
...
| P1-01A Artifacts | Human/API-credential-bound encryption-gateway ingestion and grants, versioned source authorities with policy-attested erasable envelopes/current projections, atomic upload quota reservations, periodic upload/download authorization, separate per-upload content/source-identity erasable hierarchies, non-overwritable ciphertext, trusted source floors, quiescent cleanup, scan/rescan/erasure, immutable URI, revocable grant | Identity, object storage | Human/API auth loss fences upload leases and denies grant replay/redemption/continued download within the 60-second budget; repository visibility, signing approval, authority-envelope policy/key state, or authority deletion advances one current pointer and blocks completion/grant/read/route/dispatch until exact revalidation; authority creation rollback cannot leave an active orphan and retention-forbidden authority identity is terminal only after its hierarchy is destroyed; concurrent target mint cannot overcommit aggregate workspace/credential byte, concurrency, or spend limits; completion/cleanup reconciles safely; neither upload plaintext nor provenance identity/signature reaches ordinary durable storage; client cannot lower floor; expired/revoked/sensitive paths destroy all applicable hierarchies before terminal state |
| P1-02 Routing API | `/routes`, schemas, prepare/commit/activate per-task request/normalized-task/feature/execution/gate-binding hierarchy, idempotency, stratum-scoped activation evidence | P0 router, P1-01A | Task ciphertext and its provisional handle/outbox commit atomically; no consumer runs before post-commit activation; rollback/uniqueness/activation failures leave no decryptable active orphan; current retained decisions replay and bind exact live gate evidence behind random handles; policy drift blocks use, cancels queued selections, cascades through results, and erasure destroys every content/fingerprint/selection/evidence-mapping copy while retaining only generic non-expandable history; unvalidated/changed/private strata cannot consume a universal activation flag |
| P1-03 Run API | `/routed-runs`, shared `/runs` projection, exact random task-scoped execution binding, ordered in-place migration, execution-contract-versioned DB compatibility roles/policies, job creation | Identity, artifacts, jobs | Current clients share one ledger; every attempt resolves one exact target only from its active encrypted task binding and minimum worker contract; erased history has no task-to-catalog edge; pre-RLS backfill has no null window; Alpha/down-level roles can read/write only Alpha-contract direct workspace MCP rows, and rollback cancels/drains every newer-contract direct/routed/evaluation row before old workers start |
| P1-04 Runtime | Input/descendant-result erasure, attested output, pools, exact price/FX quotes, dispatch authorization, gate-evidence fences, native/normalized reservation, deadlines, fallback, breakers, no redirects | P0 adapters | Forbidden task content erases its hierarchy, selected-target mapping, and all descendant result keys before terminal state; no result plaintext persists; every initial/fallback fence revalidates task/gate evidence/price/FX and reserves atomically; 3xx never resends; fractional native/normalized costs replay exactly |
...
- immutable `benchmark_artifact_retention_leases`, append-only `benchmark_artifact_retention_lease_events`, and locked `benchmark_artifact_retention_lease_projections`, one identity per exact artifact-version/reference-owner pair. The identity carries owner kind/ID/version, admitted source/classification/retention-policy lineage, `required_through`, and hard source/data-owner `retention_ceiling`; events/projections represent `active|released|expired|invalidated` and the effective deadline. Case/suite/attempt/report publication atomically creates its complete lease set plus active events/projections and fails when any required window exceeds its ceiling; the inherited artifact sweeper must lock/recheck these rows before the 14-day default can tombstone a version
...
- the inherited outbound `mcp_session_initialization_leases` and target-server live-session proofs gain nullable exact admitting API-credential ID/version/revocation epoch/scopes/ceilings and the parent's immutable `execution_contract_version`. Direct and routed MCP execution compares those fields at lease mint, redemption, proof publication, and dispatch consumption; credential mutation invalidates outbound leases, proofs, and target transports through the shared locks, and no worker may claim a row whose minimum contract it does not implement
...
- `evaluation_runs`, `evaluation_attempts`, `evaluation_results`, `grader_results`; after the execution-binding and visibility migration below, Phase 0 writes evaluation executions as `run_kind=evaluation`, `run_visibility_scope=evaluation_restricted`, `execution_contract_version=phase0_evaluation_v1`, an exact visibility-domain ID, and an exact random `task_execution_binding_id` whose task-encrypted mapping selects one immutable `deployment_version_id`, then references those authoritative `runs`/`run_attempts` rows rather than duplicating dispatch state
...
- `routing_decisions`, `routing_candidates`, `routing_feature_snapshots`, `task_execution_bindings`, and `task_gate_evidence_bindings`. `routing_candidates` is one opaque encrypted/padded snapshot reference per decision, not item-level rows whose identities/count leak task-conditioned eligibility; the selected taxonomy literal/stratum, taxonomy-derived features, candidate membership, filters, scores, individualized estimates, exact selected implementation, task-conditioned reason, exact gate-evidence version/current-projection generation/dependency deadline, and every source-derived feature value are ciphertext under the task hierarchy, with only keyed identity and opaque exact extractor/schema/input/feature-policy lineage outside. Task execution and gate-evidence bindings each have a random non-catalog-resolvable ID, ciphertext exact mapping, purpose-keyed mapping identity, current attestation, and `active|erasure_pending|erased` state; the gate binding is absent for an explicitly static/shadow-only decision. Active projections and task-backed attempts carry only those random handles and keyed attestations. Dispatch decrypts both mappings once and revalidates the resolved exact target and gate evidence under shared locks. Gate invalidation advances the authoritative evidence projection before a durable cancellation sweep, so every queued attempt fails its fence without a plaintext task-to-evidence index; task erasure destroys both mapping subkeys and every task-to-catalog/stratum edge. Each allowed price record stores provider-native amount, base currency, immutable FX-rate-set/rate/source/revision/timestamps, exact oriented factor, normalized amount, and quote context/expiry; legally required non-content settlement totals may survive without a task-to-implementation link
...
- `mcp_client_session_authorization_projections` for authentication of clients to the platform MCP facade, containing a random session ID, subject, workspace, admitted human authorization epoch, authentication expiry, nullable exact API-credential ID/version/revocation epoch/scopes/ceilings, and `active|invalidated|expired` state. These projections authorize only inbound meta-tool calls, are distinct from outbound target-server initialization leases/proofs, and are synchronously invalidated on bound human or credential mutation
- `permission_grants`, including dedicated audited `artifact_provenance:attest`; `workspace_policies` with a versioned `unverified_upload_floor`; and an additive `source_authority_metadata` kind in the inherited immutable scan/retention-policy versions and locked current pointers
...
- additive exclusive execution-target binding, parent-attempt, execution-disposition, and fallback lineage on existing `run_attempts`; a direct attempt points to exactly one immutable `server_connection_version`, while every task-backed evaluation/routed initial or fallback attempt points only to one random non-catalog-resolvable `task_execution_binding_id` whose task-encrypted current-attested mapping resolves to exactly one immutable MCP connection or deployment version while usable
...
- `artifact_source_authorities`, `artifact_source_authority_content_keys`, immutable `artifact_source_authority_scan_attestations`, immutable `artifact_source_authority_versions`, immutable `artifact_source_authority_refresh_evidence`, and locked `artifact_source_authority_current_projections`. A stable authority uses a random non-enumerable ID, workspace, qualified connector/data-owner kind, ciphertext/opaque envelope reference, keyed identity, exact current source-authority-metadata scan/retention-policy version, and `activation_pending|active|authority_revalidation_required|authority_erasure_pending|authority_erased` state—never a content-derived ID, raw repository/signing identity, ordinary digest, or key material. Its unique external content-key row carries only the hierarchy handle, purpose/version, immutable provisional expiry, `provisional|active|destruction_pending|destroyed` key state, activation/destruction outbox evidence, and recovery-copy destruction attestation for separate encryption/fingerprint subkeys. Creation prepares the provisional hierarchy before encrypting, commits its handle/ciphertext/attestation/outbox atomically, and permits use only after idempotent activation; rollback/uniqueness loss self-destructs, while activation failure destroys without replacement. Scan attestations bind the exact keyed envelope identity, current policy version, classification, coverage, retention decision, and supersession lineage. Material versions carry active/revoked status, classification floor, and connector/signing-approval generation; refresh rows carry the captured material/configuration/attestation generation, qualified safe observation lineage, observed time, and bounded `valid_through`. The current projection selects one material version/generation and latest refresh-evidence/freshness, key-state, and scan-attestation state. An unchanged successful refresh advances only the evidence pointer through an expected-prior-projection compare-and-set, while data-owner revocation or qualified connector visibility/classification change appends/selects a new material version; late evidence cannot overwrite a newer or revoked projection, and missed refresh makes it stale and unusable. All streams are append-only. Material or source-authority-metadata-policy projection change immediately invalidates dependent use without scanning every artifact; retention-forbidden reclassification or explicit erasure destroys both authority subkeys and recovery copies before terminal tombstoning
- one immutable per-upload `artifact_source_attestations` row containing the nullable random source-authority ID and exact admitted material version/generation or exact workspace unverified-upload-floor version, safe key/classification/policy version IDs, an encrypted connector/data-owner provenance envelope reference when applicable, source-hierarchy-keyed identity, `provisional|active|source_revalidation_required|source_erasure_pending|source_erased` state, and audit lineage—never plaintext repository identity/visibility/commit, expected bundle/manifest identity, raw signature, ordinary digest, or publicly verifiable candidate oracle. `source_identity_content_keys` carries only its unique external hierarchy handle, purpose/version, activation/destruction outbox evidence, and recovery-copy destruction attestation; even reused logical source authorization is copied into a fresh upload-scoped encrypted envelope. `run_artifacts` is a visibility-domain/role-qualified link to immutable encrypted `artifacts` and append-only exact-content/source-authority scan/current-erasure projections. `artifact_uploads` stores only opaque upload ID, originating subject/workspace/admitted human authorization epoch, nullable admitting API-credential ID/version/revocation epoch plus scopes/ceilings, last qualified human/API-credential authorization check/expiry and safe decision, safe encryption-gateway/origin ID, unique ciphertext key/version, upload/source-key references, exact upload-quota-reservation ID/version, keyed plaintext identity, ciphertext integrity, client declaration, trusted source-authority/attestation/floor version, constraints, `open|completed|cleanup_pending|upload_erasure_pending|upload_erased|deleted` state, expiry, qualified `U`/`W`/`C`, authorization/write-fence/lease/cleanup/quiescence evidence, and dual-hierarchy key/deletion attestations—never plaintext, raw identity response/keys, an ordinary digest, or an `EphemeralUploadTarget` credential. `upload_content_keys` carries only its separate opaque external hierarchy handle, purpose/version, `provisional|active|destruction_pending|destroyed` state, activation/destruction outbox evidence, and recovery-copy destruction attestation. Target and access-grant rows contain only non-secret metadata/HMAC verifiers plus bound subject/human epoch and nullable exact admitting API-credential ID/version/revocation epoch/scopes/ceilings; upload-target rows additionally bind the exact reservation and exact applicable source-authority-or-unverified-floor projection, while access-grant rows bind the exact applicable authority-or-unverified-floor, source-attestation, and scan versions. Inherited grant events/projections and exact-lifetime encrypted envelopes retain only non-secret tombstones after expiry/erasure
...
- append-only execution `usage_reservations` linked one-to-one with a dispatch fence/attempt while active, distinct from upload quota reservations, plus exact provider-native and normalized decimal/rational reservation/commit/release/reconciliation events, immutable provider-price and FX-set/rate references, base/quote currencies, factors/timestamps, settlement rules/residuals, and workspace/provider quota-budget balances. A task-backed attempt stores only a random task-scoped reservation handle; its exact attempt-to-provider/price mapping and raw token/tool/context/size or other task-derived usage dimensions are encrypted under purpose-separated subkeys in the task hierarchy, never in ordinary accounting rows. On task erasure, reconciliation first preserves required monetary settlement in the non-task financial ledger, then destroys/severs the task-scoped mapping so surviving totals, FX/source lineage, and non-content invoice/event IDs cannot be joined back to the task or selected implementation
...
- inherited `request_derived_content_dependencies` is extended with a parent-kind discriminator and nullable exact `task_instance_id`/cascade generation so every routed/evaluation result, result artifact, result-key hierarchy, and grant receives one complete parent-task dependency atomically before publication. Inherited `result_content_keys` and result ciphertext/keyed-fingerprint/activation/erasure fields on existing attempts/artifacts are reused; parent task `erasure_pending` suppresses them even when output policy is unchanged, and task terminal erasure waits for every descendant hierarchy/grant/reference to be attested erased. No parallel result store or plaintext inline column is introduced
- extensions to existing `idempotency_records` and `audit_events`; no second idempotency or audit ledger

The Phase 0 rolling migration first adds nullable `run_kind`, `run_visibility_scope`, `evaluation_visibility_domain_id`, `execution_binding_kind`, `deployment_version_id`, `task_execution_binding_id`, and immutable `execution_contract_version` to existing `runs` and `run_attempts`, plus database defaults/guarded insert logic that stamps every concurrent legacy Alpha write as `direct`, `workspace`, no evaluation domain, `mcp_connection` from its required server-connection binding, and `registry_alpha_v1`. Jobs, session-initialization leases/proofs, idempotency rows, events, and artifacts inherit the parent's contract version rather than choosing one independently. No evaluation writer or row is allowed yet. Deploy expand-compatible readers/workers, then use the existing application roles to backfill legacy parent and visibility-inheriting child rows in bounded resumable batches; the insert guard closes the concurrent-write tail. Readers may interpret null legacy values as direct MCP/`registry_alpha_v1` only during this explicitly measured phase, while unknown non-null kinds or contracts remain non-readable and non-dispatchable. Verify zero null/orphan/mismatched rows and exercise production-sized migration/rollback fixtures before changing access enforcement.

After that backfill succeeds, drain every old binary that could bypass the new policies and revoke its broad role credentials. In one fail-closed cutover transaction, recheck the zero-null invariant, install and force row-level security, grant only the constrained current and version-scoped compatibility roles, revoke broad base-table privileges, validate non-null/visibility/binding/contract constraints, and publish the new schema epoch. The ordinary current API role can select/mutate only `workspace` rows and their visibility-inheriting events/artifacts; the `registry_alpha_compat` role is narrower and accepts only direct workspace MCP-bound rows whose exact `execution_contract_version=registry_alpha_v1`; internal evaluation roles require an explicit campaign/stage grant. Null or unknown visibility/contract is denied after cutover except to a time-bounded audited repair role that cannot serve application traffic. The constraints require `evaluation` to use `evaluation_restricted`, a domain, a task-scoped encrypted execution binding, and `phase0_evaluation_v1` or a recognized successor; `direct` to use `workspace` without an evaluation domain; `registry_alpha_v1` to use a direct MCP binding with no API-credential, source-authority, task-execution, deployment, routing, or evaluation lineage; exactly one direct exact target or task-scoped handle whose column agrees with `execution_binding_kind=mcp_connection|task_scoped`; and every child to inherit the parent contract. Only after policy/role probes and compatibility tests pass may the `evaluation` allowed value and evaluation writers be enabled; model, CLI, and static-analysis task executions resolve an immutable `deployment_version_id` only from the active task-scoped encrypted binding and never fabricate an MCP connection.

Phase 1 uses the same expand/contract sequence to add nullable routing columns and the `routed` allowed value. Before any Phase 1 write begins, retain and revalidate the version-scoped `registry_alpha_compat` database role/policy (or security-barrier updatable compatibility views) whose run predicate permits only `run_kind=direct AND run_visibility_scope=workspace AND execution_binding_kind=mcp_connection AND execution_contract_version=registry_alpha_v1` with a non-null server-connection version and no deployment/routing/evaluation/API-credential/source-authority binding that requires newer semantics. Its event, job, attempt, lease/proof, idempotency, and artifact access is constrained through the same visible direct parent and inherited contract version; its insert trigger stamps `registry_alpha_v1`, rejects Phase 1-only fields, and its update/delete checks reject routed, evaluation, deployment-bound, API-credential-admitted, source-authority-dependent, newer/unknown-contract, or null-kind rows. The current binary stamps `phase1_v1` on every newly admitted direct or routed run and all children, even when a particular direct request uses no optional Phase 1 field, so compatibility never depends on an incomplete feature predicate. Current-version ordinary roles may access direct and routed workspace rows; evaluation roles remain grant-scoped. Compatibility probes run the actual down-level Alpha binary against this role before Phase 1 admission is enabled.

Routed creation then writes `run_kind=routed`, `run_visibility_scope=workspace`, `execution_contract_version=phase1_v1`, its routing foreign key, and its initial random task-scoped execution-binding handle into the same transaction/ledger, while every fallback attempt records its own handle and inherits visibility/contract. The task-encrypted binding maps to exactly one target only while the task is usable; erasure removes the task-to-catalog edge without changing generic execution history or financial settlement totals. Existing run IDs, attempts, events, artifacts, lifecycle projections, idempotency records, and API results are never copied or renamed. Shared maintenance repositories require an explicit visibility domain and minimum execution contract; public query/cancel/event/artifact repositories are workspace-only, and evaluation repositories are campaign/stage-grant-only rather than relying on an optional caller filter. Rollback first disables every current-binary admission path, including direct runs, then under the current binary atomically cancels every unfenced `execution_contract_version != registry_alpha_v1` attempt/job—including direct work admitted with Phase 1 API-credential, source-authority, or other semantics—while draining or reconciling fenced work. It revokes current worker and session-initialization leases/transports and proves no newer-contract claim, unredeemed proof, or unfenced row remains before any down-level process starts. Down-level binaries connect only through `registry_alpha_compat`, so all retained newer/unknown-contract direct rows as well as routed/evaluation rows and their children are invisible, immutable, and non-dispatchable until the current binary is restored; new down-level direct admissions are database-stamped `registry_alpha_v1`. Columns, policies, roles, allowed values, and compatibility views are not removed until the compatibility window and exercised rollback evidence close.
...
- Every run/attempt has one exact execution binding enforced by a database XOR constraint. A direct Alpha/Phase 1 run carries an immutable MCP `server_connection_version_id`; a task-backed evaluation/routed run carries only one random `task_execution_binding_id`, never a catalog-resolvable target ID. The task binding's encrypted, current-attested mapping contains exactly one immutable capability plus MCP `server_connection_version_id` or provider/local `deployment_version_id` and its minimum output classification. Before fencing, the worker decrypts that mapping once while the task hierarchy is active, locks/revalidates the exact resolved target and keyed mapping identity, and binds the random handle/attestation into the attempt; task erasure destroys the mapping subkey and removes every catalog edge. An MCP mapping includes the capability version whose immutable binding contains the negotiated protocol revision; protocol drift creates a fresh pending version and cannot inherit enablement.
...
- An artifact access grant admitted through an API credential retains the exact credential ID/version/revocation epoch and admitted `artifact:read` workspace/environment scope/ceilings in its grant projection, HMAC-bound bearer token, and encrypted replay envelope. Each grant or MCP read authorization snapshot for a task-derived artifact also binds the exact parent task/cascade generation. Mint replay, every redemption/chunk, and every MCP `read_artifact` response require ordinary authentication as the same subject and API credential when present, lock the inbound client-session, human, credential, artifact, and parent-task projections, and reject session invalidation/expiry, revoke/expiry/rotation/scope reduction, or parent-cascade mismatch independently of the human authorization epoch. A derived token or inbound client-session projection never outlives or bypasses its admitting credential or authentication expiry, and task-derived artifact access never outlives or bypasses its parent task; an outbound target-server proof is never read authority.
- Every routed task is encrypted before first persistence under one unique short-lived provisional external hierarchy with purpose-separated encryption/fingerprint subkeys for the original request, normalized task including the selected taxonomy literal, source-derived feature/candidate snapshot, exact selected execution mapping, task-conditioned reason, serialized dispatch arguments, and derived usage dimensions. The same transaction persists its opaque provisional handle and activation outbox with the ciphertext/task/route/idempotency rows; only idempotent post-commit activation changes it to `active`, rollback/uniqueness loss self-destructs, and committed activation failure/expiry enters attested erasure and terminally fails unfenced work without replacement. A routed Run references that hierarchy and cannot copy content or a catalog-resolvable selected implementation into run, route, attempt, job, event, audit, index, cache, training-manifest, or telemetry fields. The only durable plaintext task/routing fields are the lifecycle-scoped content-blind allowlist in Section 7.2; the taxonomy/stratum literal and every task-conditioned reason, global selected-target ID, count, bucket, identifier, URI, revision, string, hash, digest, and embedding derived from input/source belong in ciphertext. Route/replay/dispatch/analysis/training requires active keys and an exact current input/feature-policy attestation. Every derived result/artifact/key hierarchy/grant is inserted with the exact parent-task/cascade dependency before publication. Policy drift immediately suppresses every use. Retention-forbidden change advances the cascade and enters `task_erasure_pending`, cancels unfenced work, suppresses descendants, invalidates derived dataset/model lineage, destroys all task and descendant-result content/fingerprint subkeys and recovery copies, purges caches/materializations, removes task-to-catalog links, installs non-comparable/unresolvable tombstones, and reaches `task_erased` only after the complete locked descendant set is attested erased; retained generic execution/settlement metadata is neither selection nor replay authority.
- Raw usage dimensions—including prompt/context/output-token counts, tool-call counts, file/object counts, and size/structure buckets—are task-derived content, not permanent accounting metadata. Store them only encrypted under a purpose-separated subkey in the routed task hierarchy or direct run hierarchy; reconciliation may transiently decrypt them while current/authorized. Task/run erasure destroys and tombstones these dimensions across reservations, usage records, exports, WAL, replicas, and backups. Exact provider-native/normalized charged, reserved, committed, released, and reconciled monetary amounts, immutable price/FX/rounding lineage, and opaque invoice/event IDs remain append-only in the non-task financial ledger for integrity, but task erasure severs/destroys their task/run/attempt-to-provider mapping. Surviving task history may carry only a non-content settled total that cannot be joined to provider/price/capability and contains no raw dimension or reconstructable content-size surrogate beyond that amount.
...
- Every trusted source authority has one random internal ID, a unique provisional/active/destroyable external envelope hierarchy, exact-policy scan attestations over its keyed envelope identity, immutable material versions, immutable qualified refresh evidence, and an event-backed locked current material-version/generation plus freshness/key/attestation projection. Repository/source identity and signing scope exist only in that ciphertext. Authority creation follows prepare/commit/activate; no refresh, upload, grant, read, route, or fence may use `activation_pending`, stale-policy, missing-key, revalidation, or erasure state. An unchanged successful connector refresh advances only the evidence/`valid_through` pointer. Refresh failure beyond `valid_through` makes the projection stale, while a repository visibility/classification change, data-owner approval revocation, or any material generation change appends and selects a new material version. Upload-specific attestations bind the exact admitted material version/generation; artifacts without claimed authority instead bind the exact workspace unverified-upload-floor version. Completion, grant mint/replay/redemption, every artifact response/tool chunk, routing, and every initial/fallback fence lock and compare the applicable current material projection, require current freshness, active authority keys, and the exact current authority scan attestation before release/send; missing, stale, revoked, policy-mismatched, key-inactive, or materially mismatched authority or a changed unverified floor produces `source_revalidation_required`. Qualified authority-envelope reclassification can supersede its attestation only for the same keyed identity under still-current policy. Artifact revalidation can supersede the upload-specific attestation only after exact provenance/content identity verification and monotonic classification recomputation under the new authority or workspace-floor version. Authority retention denial/erasure destroys both authority subkeys before terminal tombstoning and suppresses all dependencies; artifact/source erasure removes its authority link after both per-upload hierarchies are destroyed.
- Every upload is encrypted by the controlled gateway before first object-store persistence under a unique upload-content hierarchy, and its complete upload-specific source provenance is encrypted before first database persistence under a separate unique source-identity hierarchy. Durable systems retain only ciphertext, ciphertext integrity, opaque handle/state, purpose-keyed fingerprints, safe random authority/classification/policy versions, and safe authorization-check evidence. The target and rows bind originating subject/workspace/admitted human epoch plus the exact admitting API-credential projection and exact current source-authority version/generation when present or workspace unverified-upload-floor version otherwise. Before first byte, no later than every `U` during streaming, before ciphertext commit, on target replay, and at completion, a qualified cache-bypassing lookup plus locked comparison requires current membership/upload permission, unchanged human epoch, an unchanged active credential version/revocation epoch with current upload action/workspace/environment scopes and applicable ceilings when present, an active/fresh exactly matching source authority when claimed or exact current unverified floor otherwise, both required hierarchies active, and an unfenced lease; `P + R + U + M <= 60 seconds`. Human identity/permission, API-credential, source-authority, and unverified-floor mutations fence/cancel open leases through the same lock order. Every finalized artifact stores only the immutable opaque trusted source-attestation ID/version, exact random source-authority/version/generation or unverified-floor version, and server-derived classification floor; completion decrypts and transiently verifies expected versus computed canonical identity when trusted provenance was claimed. Ordinary expected/computed identities, repository/commit/manifest fields, raw signatures, plaintext digests, and publicly verifiable candidate oracles are not retained. A claimed attestation is usable only on exact byte identity. Effective classification is the maximum of current authority or unverified-upload floor, client declaration, and detector/profile result. Missing provenance uses the workspace floor defaulting to `private_internal`; client fields cannot lower policy, and current authority/floor/attestation changes invalidate prior eligibility.
...
- Artifact-grant mint, revoke, expiry, replay, and redemption use the inherited append-only event/current-projection authority. Grant rows/tokens carry the nullable exact admitting API-credential ID/version/revocation epoch plus required artifact-read scopes/ceilings and the exact applicable source-authority-or-unverified-floor/source-attestation/scan versions. Individual revocation atomically advances to `revoked`, suppresses credential-envelope replay before asynchronous erasure, and every later replay/redemption or HTTP chunk requires the grant `active` projection, unchanged current human/API-credential authorization, and exact current equality for every source/artifact binding; a bearer token, delayed cleanup, or later revalidation cannot override or revive the grant.
- A successful HTTP artifact redemption creates no authority lasting for the response lifetime. Before every chunk of at most 256 KiB, the reader requires database server time before grant expiry, performs the cache-bypassing qualified authorization refresh, and only on success installs `download_authorization_expires_at = min(source_checked_at + D, grant_expires_at)`. It then requires current time before that deadline and locks/revalidates the grant, current human/API-credential projections, exact current source authority when bound or workspace unverified-upload floor otherwise, parent task/cascade when applicable, artifact/classification/retention/integrity, and scan/profile pointers before decrypting. The same deadline bounds the write; backpressure at expiry terminates the connection rather than permitting more writes. `P + R + D + M <= 60 seconds`; failure or expiry terminates before later bytes, and no speculative decrypted buffer crosses the check.
- An MCP artifact-read response likewise creates no authority for its transport lifetime. Its inbound client-session authorization projection carries subject/workspace/human epoch, client-session authentication expiry, and nullable exact admitting API-credential lineage/scopes/ceilings. On every call the reader performs the same qualified lookup and locks that projection plus the current human/credential/source/artifact/policy/parent-task state, then sets `artifact_read_authorization_expires_at` to the minimum of `source_checked_at + D`, client-session authentication expiry, and admitting-credential expiry when present. The at-most-256-KiB decrypt, base64/typed serialization, and complete transport write share that deadline; backpressure or expiry wipes buffers and terminates the response/transport. Human or credential mutation invalidates the inbound projection and its client transport; it does not reuse the unrelated outbound target-server session proof. `P + R + D + M <= 60 seconds` bounds stale-authority bytes.
- Every retained result/result artifact is ciphertext under a unique external result hierarchy and stores an append-only attestation over its immutable keyed content identity, exact output/artifact-scan-policy version, applicable MIME-profile version, classification, coverage, and nullable exact parent task/cascade generation. Publication atomically inserts the complete opaque task-to-result/artifact/key dependency. Release checks require an active hierarchy, an active matching parent-task projection when present, and attestations matching locked current pointers before decryption; any task-cascade or policy mismatch immediately denies logical-inline reads, grants, artifact bytes, routing, and dispatch. Output policy/profile drift may enter `rescan_required` and restore only after a same-content current-version rescan, but parent task `erasure_pending` is monotonic and cannot be restored through output rescan. It suppresses descendants and destroys their encryption/fingerprint subkeys before parent terminal erasure. Independent retention-forbidden result reclassification remains monotonic through `result_erasure_pending -> result_erased`; terminal erasure requires attested destruction and generic tombstoning, while failure remains unreadable and quarantined.
...
- Run, event, result, job, lease/proof, idempotency, and artifact visibility inherits one non-null domain, parent kind, and immutable execution-contract version. Database constraints—not merely role predicates—require every `registry_alpha_v1` parent and child to be direct, workspace-visible, MCP-bound, and free of API-credential, source-authority, routing, deployment, or evaluation lineage; Phase 0 evaluation rows use `phase0_evaluation_v1`, and Phase 1 direct/routed rows use `phase1_v1`. PostgreSQL row-level policy plus separate database roles—not an optional repository predicate—prevents ordinary API identities and workspace roles from enumerating, reading, cancelling, granting, or downloading `evaluation_restricted` data; internal evaluation access requires a compatible campaign/stage grant, and adapter-development roles cannot hold one. The version-scoped Alpha compatibility role is stricter than the current ordinary role: both read and write checks admit only direct workspace MCP-bound `registry_alpha_v1` parents and children, so every newer/unknown-contract direct row plus routed/deployment/evaluation/unknown kinds remains invisible and immutable during rollback. A worker claim requires its declared supported contract set to contain the exact row contract; application filtering alone is insufficient.
...
- Every active benchmark case/suite/attempt/report and gate-evidence version has a complete exact-artifact dependency set plus event-authoritative active retention-lease projections through its declared execution/review/reproduction deadline. The effective artifact delete time is the later of the inherited default and every active lease deadline, capped by each source's hard approved retention ceiling. Each gate-evidence current projection carries `artifact_dependency_valid_through`, the minimum effective deadline across its complete dependency set. Every task-aware route/run/attempt stores only a random gate-binding handle and purpose-keyed identity outside the task hierarchy; its encrypted mapping binds the exact evidence version, admitted projection generation, stratum, and deadline. Every initial/fallback fence decrypts the mapping once, locks the current evidence projection with its keyed binding and attempt, and requires exact generation/stratum equality, `valid` state, and database server time strictly before that deadline. Cleanup locks the artifact, its leases, and referencing benchmark/evidence projections in the shared order; it cannot erase while a permitted lease remains active. Source/policy revocation or ceiling expiry first atomically invalidates all dependent benchmark/report/evidence projections and disables task-aware flags, so every referencing queued attempt fails its mandatory fence; a durable sweep cancels those unfenced jobs/attempts and fallback eligibility without a plaintext task-to-evidence index before artifact erasure completes. Missing assets/leases, an invalidated projection, or a passed deadline fail closed and can never leave gate evidence or an already queued selection dispatch-authoritative; task erasure destroys the mapping so historical rows cannot reveal the stratum.
...
- exact-version benchmark artifact retention leases owned by case/suite/attempt/report versions, admitted only when the required execution/review/reproduction window fits the source-approved ceiling; default cleanup honors active leases, while forced policy expiry invalidates dependent evidence and task-aware activation before key erasure;
...
- artifact authorization independent of grant or signed-URL possession, with subject/workspace/current-source-authority binding, event-backed individual grant revocation, safe isolated response headers, and at-most-256-KiB HTTP chunks that reauthorize before decryption under a server deadline no later than grant expiry or `D`, with `P + R + D + M <= 60`;
...
- sealed `ArtifactAccessGrantToken` mint/replay with subject/human-epoch/artifact-version, nullable exact admitting API-credential version/revocation-epoch/`artifact:read` scope, and exact applicable source-authority-or-unverified-floor/source-attestation/scan version bindings; versioned HMAC verifier; append-only revoke/expire events plus active projection; immediate replay/stream suppression on grant, credential, source, or policy change; exact-lifetime encrypted secret-store envelope with no premature eviction; same-subject/same-credential ordinary-authentication; redacted header redemption; and no full token in application/idempotency rows, audit, telemetry, browser surfaces, or caches;
...
- random non-enumerable source-authority identities with unique provisional/active/destroyable envelope hierarchies, exact-current metadata-policy scan attestations, immutable material and refresh-evidence versions, event-backed active/revoked current projections, bounded connector freshness, and exact-version/key/attestation checks at refresh, completion, every grant/read chunk, routing, and every dispatch fence; retention-forbidden or explicit authority erasure destroys both authority subkeys before terminal tombstoning;
...
- benchmark artifact retention-lease admission, effective-deadline calculation, source-ceiling enforcement, reference release, cleanup eligibility, evidence/feature-flag invalidation, and encrypted gate-binding generation/deadline checks that deny every referencing unfenced initial/fallback attempt before cancellation fan-out or forced erasure;
...
- upload hierarchy/gateway state transitions; target subject/workspace/human-epoch plus optional exact API-credential and exact applicable source-authority-or-unverified-floor projection binding; `P + R + U + M` authorization expiry; first-byte/periodic/precommit/completion checks; human/API-credential/source-authority-or-floor-change lease fencing; ciphertext-integrity/keyed-fingerprint verification; and key-destruction-before-erasure/cleanup ordering;
- source-authority prepare/commit/activate/erase and metadata-policy attestation transitions, material-version/current-projection changes, immutable refresh evidence/deadlines, unchanged-refresh material stability, late-refresh versus revocation/change compare-and-set barriers, repository public-to-private changes, signing-approval revocation, exact-version revalidation, and floor lattice maximum prove orphaned/stale/erased authority envelopes are unusable and client declarations/detectors cannot preserve or lower trusted provenance/workspace defaults;
- task-scoped execution-binding and dependency state machines prove only an active task can resolve the random handle to exactly one target; erasure removes every catalog-resolvable selected implementation/stratum/reason edge, suppresses all descendants, and waits for every result hierarchy/grant/reference before terminal state;
- HTTP and MCP artifact-read deadlines prove the server-authored deadline is no later than the last qualified check plus `D` and applicable grant/session/API-credential expiry; credential/session/parent-task/refresh/authorization/decrypt/serialize/write ordering, bounded-write timeout under backpressure, buffer wiping, and transport termination permit no later bytes;
...
- generated Python, TypeScript, and MCP interfaces match OpenAPI semantics, including decimal/rational money and immutable FX-version fields, subject/human-epoch plus optional exact API-credential-bound sealed encryption-gateway upload/access-grant handling, exact source-authority-or-unverified-floor/source-attestation/scan bindings on artifact grants, strict outcome envelopes/artifact references, unsupported-host failure, and bounded `read_artifact` parity with session/API-credential/parent-task binding plus response deadline;
- source-authority/upload contracts expose only random authority IDs, exact immutable material/refresh/scan-policy versions, safe state/floor, opaque authorized authority-envelope/provenance/content/source hierarchy references, and a sealed subject/workspace/human-epoch target carrying optional exact API-credential plus exact applicable source-authority-or-unverified-floor projections and the exact server-owned upload-quota reservation version/ceilings. They never expose client-writable authority/quota, content keys, direct object-store/plaintext targets, raw identity responses, source payload/signature/repository/commit/manifest identity, client-writable source floor/effective class, publicly verifiable identity oracle, or ordinary plaintext digests; provisional, absent, expired/revoked/stale/policy-mismatched/key-inactive/erased/mismatched-authority-or-floor, released/exhausted/mismatched-reservation, cross-workspace, human/API-credential epoch-mismatched, permission/scope-revoked, unauthorized, or non-gateway paths fail closed, and abandoned/revoked/retention-forbidden terminal representations require the applicable authority, upload-content, and source-identity hierarchy destruction evidence;
- artifact-content contracts expose bounded HTTP streaming and bounded MCP tool results with no ambient object-store credential and a server-authored authorization deadline; generated clients tolerate termination between/during chunks and cannot request a chunk larger than 256 KiB or extend `D`/grant/session/API-credential expiry;
- task/route contracts expose only the content-blind envelope, a random non-catalog-resolvable execution-binding handle, and opaque encrypted task/feature/selection references; generated clients cannot receive original/normalized task content, source-derived fields, selected implementation mapping, keyed fingerprints, or hierarchy handles, and replay after task erasure returns the stable unavailable code rather than a partial reconstruction;
...
- benchmark-retention fixtures create source, diff, hidden-label, candidate-result, and report artifacts near the inherited 14-day boundary, then prove case/suite/attempt/report activation atomically acquires a complete exact-version lease set through its declared review/reproduction window without exceeding source ceilings. Concurrent cleanup cannot erase an actively leased asset; release permits cleanup exactly once. A task-aware initial attempt and fallback are queued with random handles mapped under task encryption to the still-valid evidence before early source-policy revocation or ceiling expiry. Invalidation atomically advances the evidence projection and disables citing flags; barriers prove it winning before either mandatory fence yields zero sends even before the durable sweep cancels both attempts/fallback eligibility. Missing/expired assets block new attempts and cannot leave historical evidence or queued selections marked reproducible, valid, or dispatch-authoritative;
- upgrades from a populated Registry Alpha ledger keep evaluation/Phase 1 writers disabled, install legacy-write defaults/guards, backfill direct MCP kind/visibility/binding plus `execution_contract_version=registry_alpha_v1` and child inheritance under existing roles, and prove zero null/orphan/mismatch rows before one atomic RLS/role/constraint cutover. Concurrent old writes cannot reopen the null tail; bypass roles are drained/revoked first. Phase 0 evaluation writes `phase0_evaluation_v1`, while the Phase 1 current binary stamps every newly admitted direct/routed row and child `phase1_v1`; database constraints reject Alpha-contract rows with any newer lineage. The actual down-level Alpha binary through `registry_alpha_compat` can read/write only direct workspace MCP `registry_alpha_v1` parents/children and cannot enumerate, mutate, cancel, grant, or dispatch newer-contract direct, routed, deployment, evaluation, or unknown rows. A rollback race fixture admits an unfenced direct `phase1_v1` MCP run through an API credential, revokes or narrows the credential, begins rollback, and proves the current binary cancels it with zero initialization/tool-call bytes; the compatibility role cannot see or claim it before or after the down-level binary starts. Rollback disables all current admission, cancels every unfenced newer-contract direct/routed job, drains/reconciles fenced work, revokes current worker/session leases and transports, and proves no newer-contract claim remains before switching binaries, without a parallel ledger or fabricated MCP connection;
- encryption-gateway upload/download authorization plus fake-clock cleanup: plaintext and decrypted provenance exist only in bounded gateway/scan memory, while object storage, database, WAL, replica, backup, queue, log, trace, and cache inspection sees ciphertext/keyed identity/opaque handle plus safe policy metadata only. Target mint/replay, first-byte, every `U`, precommit, and completion checks bind the originating subject/workspace/human epoch, current upload permission, optional exact API-credential projection, and exact applicable source-authority-or-unverified-floor projection under `P + R + U + M <= 60`; deterministic barriers remove human permission, advance either revocation epoch, rotate the credential, reduce its scope, advance/stale/revoke the source authority, or change the unverified floor before/during/after a chunk and on either side of precommit. A mutation winning before the one-shot commit fence prevents commit; a fence winning first permits only its exact ciphertext commit, accepts no more bytes, but later human/API-credential/source-authority-or-floor loss prevents artifact finalization and enters one cleanup path. Expiry/auth/authority/floor loss always returns no artifact, and a late ciphertext write cannot orphan. Cleanup destroys every upload-content and source-identity subkey, repeatedly deletes through the applicable fence/expiry plus `W + C`, survives worker/key-service failure, and reaches `deleted` within 15 minutes only after dual-hierarchy destruction and quiescent absence; retention-forbidden content reaches `upload_erased` only after both hierarchies are destroyed, with restored artifact and provenance ciphertext undecryptable and keyed identities non-enumerable even if deletion lags;
- source-authority/provenance fixtures show create rollback/uniqueness loss leaves only a self-destructing provisional authority hierarchy, no authority use precedes post-commit activation, and activation failure never remints. A malicious or mistaken client declaration cannot lower the server-derived floor: verified public repository/commit and data-owner-approved records bind their random authority plus exact current material version/generation and encrypted upload-specific attestation, arbitrary uploads default to the workspace `private_internal` floor, and detector-clean proprietary source stays private. Source-authority metadata-policy drift blocks refresh and every dependent use before envelope decryption; a clean same-keyed-identity rescan may restore, while retention-forbidden reclassification or explicit erasure destroys both authority subkeys/recovery copies, leaves restored database/WAL/replica/backup ciphertext undecryptable and keyed identities non-enumerable, and cannot terminally erase until dependent links are rebound or source-erased. A connector-observed public-to-private repository transition, missed `valid_through` refresh, signing-approval revocation, or version mismatch atomically blocks upload completion, grant mint/replay/redemption, HTTP/MCP read chunks, routing, and unfenced dispatch before fan-out; only exact-identity revalidation under the new current version can restore stricter eligible use. Reverse-order and late-refresh barriers prove the pointer change or operation wins cleanly. Low-entropy repository/commit/manifest candidates cannot be tested from database/backups; retention-forbidden completion or later artifact/source reclassification suppresses comparison immediately and cannot reach terminal erasure until the upload-specific source hierarchy and all recovery copies are attested destroyed and the artifact-to-authority link is tombstoned;
...
- Phase 0 evaluation and Phase 1 routed-task restore fixtures inspect database, WAL, replicas, backups, queues, caches, events, audit, telemetry, and training manifests and find no plaintext original/normalized request, artifact/repository/path/revision, source-derived count/bucket/string/hash/digest/embedding, exact selected implementation/stratum/task-conditioned reason, or reusable fingerprint/catalog oracle. Input/feature-policy drift blocks route replay, dispatch, analysis, training, descendant result publication, grants, and reads immediately. Retention-forbidden reclassification advances the parent cascade, destroys every task subkey—including the execution-binding mapping—and every descendant result/artifact hierarchy/recovery copy, removes task-to-catalog links and derived-dataset membership, invalidates dependent model lineage, and cannot reach `task_erased` while any dependency remains. Restored ciphertext is undecryptable and surviving random tombstones cannot reveal the selected implementation or category;
...
- API-credential barriers revoke, expire, rotate without continuity, or reduce action/workspace/environment/applicable-ceiling scope after run, upload, access-grant, or inbound MCP client-session admission while leaving actor membership unchanged. For direct and routed outbound MCP targets, interleave each mutation before/after target-server session-lease mint, redemption immediately before server-credential retrieval/first initialization byte, proof publication, and dispatch consumption. A mutation winning first emits no later initialization/tool-call bytes; one winning after redemption invalidates the outbound proof and cancels the target transport. Independently, interleave mutation before/after inbound client-session projection creation and every `read_artifact` authorization/decrypt/write; a mutation invalidates that projection and client transport, and the reader never consults the outbound proof. Every initial/fallback dispatch fence; upload target mint/replay, first-byte, periodic, precommit, and completion check; access-grant mint/replay/redemption; and MCP artifact-read call locks the current credential projection. A mutation before upload precommit permits no further bytes/commit; a mutation after precommit returns no artifact and enters dual-hierarchy cleanup; any mutation before grant replay/redemption or MCP read returns no token or bytes even when the human epoch is unchanged;
...
- private-task erasure fixtures place distinct low-entropy taxonomy literals and choose catalog-distinct implementations in otherwise identical tasks, then inspect task/route/run/attempt/result/grant/candidate/index/event/audit/analytics rows and restored WAL/replicas/backups after erasure. Only generic lifecycle/settlement facts plus random unresolvable tombstones and non-expandable schema lineage survive; no literal, taxonomy-derived feature, task-conditioned reason, joinable task-type/stratum/selected-implementation/target selector, descendant plaintext, or task-category/catalog oracle remains outside destroyed task/result hierarchies;
- OpenTelemetry propagation.

### Replay and evaluation

- golden route decisions replay exactly while their encrypted task hierarchy and current attestations remain eligible; erased decisions return `routing_replay_unavailable_erased` and expose only generic lifecycle/settlement history plus random unresolvable tombstones, never selected implementation/stratum/reason metadata;
- historical candidate sets can be scored by a new router without mutating history;
- benchmark reports reproduce from a clean database and artifact snapshot only while the report's complete exact-version retention-lease set remains active; a missing/expired/forced-erased asset produces an invalidated non-reproducible report and gate-evidence state rather than silently using mutable source or stale evidence;
...
- artifact upload through the service encryption gateway with separate unique erasable upload-content and source-identity hierarchies, exact current source-authority version/generation when bound or workspace unverified-upload-floor version otherwise, current-policy/profile-attested completion, authoritative URI issuance, authorized viewer/download grants, and rejection of underclassified, unsafe, mutable/overwritten, wrong-version, integrity-mismatched, stale/revoked-authority-or-floor, stale-scan, unfinalized, expired, direct-object-store, or cross-workspace artifacts; durable-store/restore inspection proves plaintext provenance/artifact identity is absent and every sensitive/abandoned content/source key is destroyed before terminal state;
...
- fake-clock IdP propagation and lookup races prove cached authorization plus frontend display expires within the single inherited 60-second budget, and mutations, every initial/fallback fence, grant mint/redemption, each bounded HTTP response chunk, and each `read_artifact` chunk bypass the shared cache and fail closed on source timeout/incompleteness or admitted epoch mismatch;
- individual grant revocation advances the event-backed projection, suppresses mint replay, and denies every later redemption even if envelope erasure is delayed. Human membership/role removal or authorization-epoch change; admitting API-credential revoke, expiry, rotation, environment/workspace or `artifact:read` scope reduction; source-authority revoke, staleness, or generation change; visibility/classification/retention-policy revocation; output/artifact scan-policy or MIME-profile change; quarantine; or integrity-state change likewise denies token replay and artifact bytes with an otherwise valid grant. Allowed source revalidation/rescan can restore artifact readiness but can never revive that old exact-version-bound grant; a newly authorized grant is required;
- long and backpressured HTTP-download fixtures start immediately before grant/`download_authorization_expires_at` boundaries, pause between and during 256-KiB chunks, and then revoke the grant, remove human access, revoke/reduce the admitting API credential, advance/stale the source-authority projection, or change artifact/scan policy. A check winning first emits no next chunk; a chunk winning first emits only bytes written before its bounded write deadline and no later chunk; backpressure cannot continue application writes past that deadline, no decrypted read-ahead survives, no deadline extends beyond grant expiry, and `P + R + D + M <= 60 seconds` under fake-clock provider delay;
- long and backpressured MCP `read_artifact` fixtures authenticate an inbound client session with a human and API credential, begin a 256-KiB response immediately before `artifact_read_authorization_expires_at`, then invalidate/expire the client-session projection, remove membership, advance the human epoch, revoke/expire/rotate/reduce the credential's `artifact:read` scope, change the parent-task cascade, or advance the source/artifact policy. Every call locks the exact inbound projection and current authorities; an outbound target-server proof cannot satisfy the check. Decryption/serialization/write share a deadline capped by client-session and credential expiry. A mutation winning before the call's locked authorization emits no bytes; authorization winning first may emit only bytes whose write completes before that deadline. Backpressure timeout wipes buffers and terminates the client transport within `P + R + D + M <= 60 seconds` even though the human epoch alone did not change;
...
- `read_artifact` returns authorized, integrity-checked bounded chunks only after the exact inbound MCP client-session projection, its human/API-credential bindings and expiry, parent-task cascade, source-authority or unverified-floor projection, and complete classification attestations match current state; its decrypt/serialization/write finishes within the server deadline. It rejects client-session/identity/credential/task-parent mismatch, `source_revalidation_required`, `rescan_required`, active, encrypted/uninspectable, unknown, unsupported, type-mismatched, coverage-incomplete, quarantined, expired, or cross-workspace results without exposing restricted plaintext fingerprints;
...
- deployment/capability disable, gate-evidence expiry/invalidation, policy revocation, and artifact invalidation between enqueue and every initial/fallback dispatch fence each terminate without a provider send; gate invalidation also marks the queued selection non-dispatchable before its benchmark asset is erased;
...
- sensitive/scanner-failed input leaves no persisted request, normalized task, source-derived feature, content-derived audit value, or provider send; scan/feature-policy changes before either an initial or fallback fence force a fresh decision and suppress route replay, every encrypted request/feature/selection read, dispatch, descendant result/grant access, analysis, training, and fingerprint comparison. A direct run finalizes only after both per-run subkeys and descendant result hierarchies are destroyed; a routed task finalizes only after every original/normalized/feature/candidate/execution-binding/gate-evidence-binding/task-reservation-mapping/dispatch/fingerprint subkey and every descendant result/artifact hierarchy is destroyed and task-to-catalog links are removed. Restore from database/WAL/replica/backup then proves ciphertext undecryptable and keyed values unusable for low-entropy enumeration, with only generic unresolvable no-replay tombstones and non-content settlement totals remaining;
- workspace isolation;
- rollback credential-race coverage admits an unfenced direct `phase1_v1` MCP run through an API credential, revokes or narrows that credential, and begins the down-level transition; the current binary cancels the run with zero initialization/tool-call bytes, while the compatibility role can neither see nor claim it before or after the Alpha binary starts;
- version disable and router rollback, including an actual down-level Alpha binary using only the compatibility role after Phase 1 direct and routed rows exist; every `phase1_v1`/newer/unknown-contract direct row plus routed/evaluation parents and children remains invisible, immutable, and unscheduled. Preexisting and newly down-level-admitted `registry_alpha_v1` direct MCP runs continue only after the current binary has cancelled/drained newer-contract work and revoked all current worker/session leases and transports.
...
- artifact bytes, incomplete-upload cleanup age/failures, benchmark retention-lease expiry/invalidation backlog, source-authority envelope activation/destruction failures, and retention backlog.
...
Use feature flags at workspace, immutable `evaluation_stratum_id`, task type, policy, capability version, and deployment levels. A task-aware flag must reference one passing, currently valid gate-evidence version, match the request's current stratum exactly, and satisfy database server time strictly before that evidence projection's `artifact_dependency_valid_through`; otherwise selection fails to static/shadow. Every rollout step has an owner, start/end time, comparison cohort, abort threshold, and rollback procedure.
...
| Benchmark asset expires while still authoritative | The 14-day artifact job erases an early snapshot before a three-week calibration/report finishes, or a queued task-aware selection outlives its gate evidence | Exact-version reference-owned retention leases through declared execution/review/reproduction windows, capped by source approval; every attempt has a task-encrypted exact evidence generation/deadline binding; invalidation advances the global barrier; every initial/fallback fence rechecks it and denies queued work before cancellation fan-out or forced erasure |
...
| Formerly public source remains eligible after authority drift | A private repository, revoked signing approval, or stale connector still backs public artifact reads or dispatch | Random stable source authority with immutable material versions, bounded freshness, and one locked current projection; exact-version checks on completion/grant/every read chunk/route/fence; immediate pointer invalidation and qualified revalidation under the new floor |
| Authority envelope survives revocation or failed creation | Raw repository/signing identity remains decryptable in a backup or an activated orphan after authority policy denial, deletion, rollback, or key-service failure | Unique provisional authority-envelope hierarchy; atomic ciphertext/attestation/outbox commit; post-commit activation only; current-policy gate; both-subkey destruction and recovery-copy attestation before terminal erasure |
| Long artifact response outlives authorization | A backpressured HTTP or MCP response emits bytes after grant/client-session/API-credential expiry or human/API-credential/source-authority revocation | At-most-256-KiB chunks; bind inbound MCP client-session authorization projections to exact credential lineage and keep them disjoint from outbound target-server proofs; cache-bypassing identity/credential/grant/authority/artifact/parent-task checks; server-authored decrypt/serialize/write deadline capped by `D` and applicable grant/client-session/credential expiry; wipe and terminate on timeout or mismatch |
...
| Erased task survives in routing or result lineage | Request deletion succeeds but a feature, selected implementation/stratum link, descendant output, embedding, URI, or backup still identifies or paraphrases private source | One per-task hierarchy includes features/candidates/selected-target mapping/dispatch content; random non-catalog-resolvable execution handles; atomic parent dependencies on every result/artifact/grant; cascade suppresses and destroys all task/result subkeys, removes task-to-catalog edges, and invalidates datasets/models before terminal erasure |
...
| Mixed-version migration or rollback misreads newer runs | Null visibility/contract during backfill, or an Alpha worker sees a routed/deployment row or Phase 1-authorized direct run it cannot revalidate | Legacy-write guard; complete pre-RLS contract backfill; atomic policy cutover; exact execution-contract version inherited by children; Alpha role restricted to `registry_alpha_v1`; cancel/drain every newer-contract direct/routed row and revoke current leases/transports before rollback |
...
- every benchmark input/result/report artifact has an active exact-version retention lease through its declared review/reproduction window within the source-approved ceiling, and forced expiry invalidates dependent gate evidence before erasure;
- the hidden result is reproducible by a second engineer from those lease-protected immutable assets;
...
- an authorized HTTP client, or an MCP host with a non-recording upload-target channel, can stream a repository snapshot and diff through the service encryption gateway under a subject/workspace/human-epoch-bound target that also binds the exact API-credential projection when used and the exact applicable source-authority or unverified-upload-floor projection, with separate unique erasable upload-content/source-identity hierarchies, and receive finalized workspace-scoped `artifact://` identifiers pinned to immutable ciphertext versions. A claimed source authority is itself usable only after its unique encrypted envelope hierarchy and exact current metadata-policy attestation activate; creation rollback cannot leave an active orphan, and policy-forbidden or explicit authority erasure becomes terminal only after both authority subkeys and recovery copies are destroyed. Target mint atomically owns an aggregate workspace/credential reservation for maximum ingress/storage bytes, concurrency, and worst-case spend; concurrent creation cannot overcommit, replay cannot double-reserve, the gateway cannot exceed the reservation, completion converts actual retained bytes exactly once, and cleanup does not release remaining capacity before dual-key destruction plus lease/quiescence closure. Current upload permission, any admitting API-credential upload scopes, and the exact applicable source-authority/floor version are rechecked within the inherited 60-second budget through precommit/completion; identity, source-authority, or floor loss fences the lease and returns no artifact, no durable plaintext/direct object-store or plaintext provenance path exists, abandoned/revoked/retention-forbidden uploads become terminal only after both hierarchies are destroyed, and an unsupported MCP host fails closed without receiving credentials;
- an MCP-only result client can retrieve permitted text/JSON or bounded binary chunks through `read_artifact` using an inbound client-session authorization projection bound to the human epoch, session expiry, and exact admitting API-credential projection/scopes when present, while HTTP clients use subject/API-credential/source-authority-bound short-lived grants. The inbound projection is distinct from and cannot be replaced by an outbound target-server proof. Both at-most-256-KiB paths recheck exact current identity/credential, parent-task, authority/artifact/scan state and impose a server deadline on decrypt/serialization/write capped by `D` plus applicable grant/session/credential expiry; backpressure stops within the inherited 60-second authorization budget, wipes buffers, and no path returns object-store credentials;
- an authorized HTTP or MCP client can route and execute a supported task idempotently;
- every persisted routed request, normalized task, feature/candidate snapshot, selected implementation/stratum/reason mapping, and dispatch serialization is encrypted under one per-task erasure hierarchy before first durable write; runs retain only a random non-catalog-resolvable binding handle. Every descendant result/artifact/grant binds the parent cascade before publication; policy drift suppresses replay/dispatch/descendant access/analysis/training, and retention-forbidden state becomes terminal only after all task/result content/fingerprint subkeys, task-to-catalog links, and derived-data membership are destroyed or invalidated;
...
- legacy visibility/binding/contract data is fully backfilled before RLS activation, every current-binary direct/routed child inherits `phase1_v1`, and the exercised down-level compatibility role exposes only `registry_alpha_v1` direct workspace MCP rows; rollback cancels/drains newer-contract direct/routed work and revokes current leases/transports before old workers start, while newer direct/routed/evaluation rows remain inert;
- every direct attempt preserves one exact database-enforced MCP connection; every task-backed evaluation/routed/fallback attempt preserves a random binding handle whose current-attested encrypted mapping resolves to exactly one immutable MCP connection or provider/local deployment while usable and becomes unresolvable on task erasure;
- every retained provider/MCP result is encrypted before first durable write, binds its parent task/cascade when applicable, and any parent or independent retention-forbidden reclassification completes only after attested destruction of all affected result subkeys;
- while the task is eligible, the exact decision, version, attempts, provider-native and normalized costs, immutable FX source/rate/set/factor/timestamps, artifacts, and evidence are traceable without consulting a latest rate. Feature/selection replay is exact only while its task hierarchy remains active/current; an erased task retains only generic unresolvable lifecycle and non-content settlement history, not a selected-implementation oracle;
- evidence-gated initial/fallback dispatch rechecks the exact current gate-evidence generation and dependency deadline under fault injection, while invalid evidence cancels unfenced work and uncertain fenced execution never falls back;
...
- Inventory internal repositories, assign data owners, and record provider-processing policies, hard source-retention ceilings, and the required execution/review/reproduction windows before snapshotting code; reject a proposed benchmark source when its ceiling cannot cover the declared window.
- Write ADRs 1–4.
