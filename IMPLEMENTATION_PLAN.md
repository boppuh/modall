...
- artifact-upload qualification proves targets use only the sealed encryption gateway, plaintext never reaches durable storage, each upload has its own externally erasable hierarchy, late-write cleanup fits 15 minutes, and abandoned/retention-forbidden terminal states require key-destruction evidence; unsupported backends or MCP hosts fail closed;
...
Phase 1 inherits the Registry Alpha authorization-freshness contract without resetting any timer: the conservative provider propagation bound `P`, qualified-source lookup allowance `R`, server snapshot TTL/poll interval `L`, and safety margin `M` must satisfy `P + R + L + M <= 60 seconds`. Privileged mutations, every initial/fallback dispatch fence, artifact grant mint/redemption, and every `read_artifact` chunk synchronously bypass the shared authorization cache and require `P + R + M <= 60 seconds`; timeout or incomplete source data fails closed. Each run binds its originating actor and admitted authorization epoch; a fence requires a fresh qualified lookup plus locked current membership/execution-role/unchanged-epoch check, and any change cancels unfenced work with zero provider sends and requires new confirmation. Server-authored snapshot expiry propagates to clients so browser caching cannot add another freshness interval. Phase 1 also preserves Registry Alpha's immutable MCP binding identity: negotiated protocol revision participates in every MCP capability-version digest, and a protocol switch creates a fresh `pending_review` version before routing or execution can use the changed wire contract.

Endpoints:

- `POST /v1/artifact-uploads` — authorize metadata, bind an immutable server-derived source-classification floor and provenance-attestation version whose signed content identity/manifest is expected for this exact upload, provision/activate a unique external per-upload hierarchy with purpose-separated encryption/fingerprint subkeys, and return one dedicated short-lived `EphemeralUploadTarget` for the service-controlled streaming encryption gateway scoped to a unique create-only ciphertext object/version; a client declaration may raise but never lower the floor, and no target is minted before the hierarchy is active
- `POST /v1/artifact-uploads/{id}/complete` — under the shared provenance/artifact/policy lock order, require server time strictly before `expires_at`, close the upload, verify the gateway-attested exact ciphertext version plus ephemeral client integrity proof and plaintext size/type, decrypt only into bounded scan quarantine, transiently canonicalize the plaintext and compare it with the decrypted signed expected provenance identity, then persist only the upload-hierarchy-keyed identity and comparison result. Require an attestation against the current output/artifact policy and MIME profile. Derive effective classification as the maximum of the immutable source floor, client declaration, and detector/profile result; if retention is allowed, the finalized artifact inherits the upload hierarchy and mints the authoritative `artifact://` URI without a plaintext rewrite. Provenance mismatch stays quarantined and cannot inherit the claimed floor. Sensitive/failed content whose policy forbids retention enters `upload_erasure_pending`, returns no artifact, and becomes `upload_erased` only after both subkeys are destroyed and ciphertext cleanup is attested; an expired `open` row atomically moves to `cleanup_pending` with the same key-destruction requirement even if the sweeper is late
...
| P1-01A Artifacts | Encryption-gateway ingestion, per-upload erasable hierarchy, non-overwritable ciphertext, trusted source floors, quiescent cleanup, scan/rescan/erasure, immutable URI, revocable grant | Identity, object storage | No upload plaintext reaches durable storage; client cannot lower floor; expiry/sensitive paths destroy both subkeys before terminal state; drift blocks grant/read/dispatch; revocation suppresses replay |
...
- sealed `EphemeralUploadTarget` delivery only to the service-controlled streaming encryption gateway after its unique per-upload external hierarchy activates, with five-minute maximum expiry, exact create-only non-overwritable ciphertext key/version and size/type/checksum scope, no redirects/read/list/key authority, a qualified gateway write fence plus bounded post-expiry materialization/visibility window, bounded non-swappable plaintext memory with immediate wiping and no spool, no-store/non-recording transfer, query/form/header redaction, only exact-lifetime encrypted secret-store replay with no premature eviction, and no plaintext or credential in application/object-storage metadata, database, queue, transcript, history, referrer, DOM, audit, error, log, trace, cache, backup, or analytics capture;
...
- upload hierarchy/gateway state transitions, ciphertext-integrity and keyed-fingerprint verification, plus key-destruction-before-erasure/cleanup ordering;
...
- generated Python, TypeScript, and MCP interfaces match OpenAPI semantics, including decimal/rational money and immutable FX-version fields, sealed encryption-gateway upload/access-grant handling, strict outcome envelopes/artifact references, unsupported-host failure, and bounded `read_artifact` parity;
- upload contracts expose client declaration plus opaque authorized provenance/hierarchy references and a sealed gateway target, but never content keys, direct object-store/plaintext targets, client-writable source floor/effective class, or ordinary plaintext digests; absent, cross-workspace, stale, unauthorized, or non-gateway paths fail closed, and abandoned/retention-forbidden terminal representations require hierarchy-destruction evidence;
...
- an authorized HTTP client, or an MCP host with a non-recording upload-target channel, can stream a repository snapshot and diff through the service encryption gateway under a unique erasable hierarchy and receive finalized workspace-scoped `artifact://` identifiers pinned to immutable ciphertext versions; no durable plaintext/direct object-store upload path exists, abandoned or retention-forbidden uploads become terminal only after key destruction, and an unsupported MCP host fails closed without receiving credentials;
