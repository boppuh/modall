...
- The transaction that creates `dispatch_fenced` uses steps 2–8 of the shared lock plan for connection, configuration, discovery observation, capability status, input-scan policy, global/connection policy, and attempt/job state. While holding them, revalidate the clean input-scan decision/current version, live tool name, invocable capability projection, exact current connection-configuration version and its required minimum output classification, current present discovery observation/snapshot, `last_complete_bypass_at` younger than the 15-minute dispatch-staleness bound, and current input/output policy compatibility; write the fence only if every check passes. If a control change or staleness boundary wins first, fencing fails and no call occurs; if fencing commits first, the attempt is already in at-most-once uncertain-execution semantics. There is no unlocked check-to-fence gap.
...
| E5-T2 | Implement run, request-content key/erasure, attempt, event, receipt/reconciliation, output-classification, and immutable artifact plus event-backed access-grant models | L | E1, E2-T5 | Effective output class is the lattice maximum independent of detector cleanliness; retained requests use unique external erasure keys/ciphertext-only storage; every terminal, erasure, quarantine, grant active/revoked/expired, and fingerprint-key state has complete replayable lineage |
...
| 10 | `feat(jobs): add durable jobs encrypted run ledger and erasure keys` | Leases, events, attempts, idempotency, per-run external key handles, ciphertext-only request storage, shared lock plan, output-classification fields, artifact and grant-event/projection metadata | 03, 04, 08 | Crash recovery, classification constraints, grant-status replay, encryption/key-lifecycle, and lock-order-helper tests |
| 11 | `feat(invocation): execute MCP tools with bounded inputs and results` | Input scanning, ephemeral decryption/rescanning, attested cryptographic erasure, lock-serialized dispatch, independent output classification, cancellation, and content-aware artifacts | 07, 10 | Detector-clean proprietary output honors connection minimum; input secrecy/erasure, races/faults, invalid/binary-sensitive output, malware/archive, and integrity suites pass |
...
- artifact-grant token HMAC verification, event/projection replay for `active|revoked|expired`, revoked mint-replay suppression, and TTL-matched secret-envelope expiry;
