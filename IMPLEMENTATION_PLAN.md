...
- `POST /v1/artifact-source-authorities` — create a random non-enumerable authority ID from a qualified server connector or audited data-owner approval, store raw repository/signing identity only in the external encrypted authority envelope, append the first immutable material version and immutable refresh-evidence row, and install the active/fresh current projection; caller-supplied IDs, floors without the dedicated permission, and unqualified evidence fail closed
- `POST /v1/artifact-source-authorities/{id}/refresh` — capture the exact current material version/generation and connector configuration, obtain qualified connector visibility/classification evidence without database locks, then reacquire the shared source-authority locks and require that captured projection/configuration still current before appending immutable refresh evidence and compare-and-setting its bounded `valid_through`. Append and select a new immutable material version/generation only when security-relevant state changed; an unchanged successful refresh leaves that material identity stable, while late evidence from a superseded/revoked projection and timeout/incomplete evidence cannot extend freshness or revive authority
- `POST /v1/artifact-source-authorities/{id}/revoke` — synchronously append a typed revocation event and advance the current projection before queuing dependent revalidation/erasure; all later upload, grant, read, route, and fence checks fail from the pointer without waiting for fan-out
...
| P1-01A Artifacts | Human/API-credential-bound encryption-gateway ingestion and grants, versioned current source-authority projections, atomic upload quota reservations, periodic upload/download authorization, separate per-upload content/source-identity erasable hierarchies, non-overwritable ciphertext, trusted source floors, quiescent cleanup, scan/rescan/erasure, immutable URI, revocable grant | Identity, object storage | Human/API auth loss fences upload leases and denies grant replay/redemption/continued download within the 60-second budget; repository visibility or signing-approval drift advances one current authority pointer and blocks completion/grant/read/route/dispatch until exact revalidation; concurrent target mint cannot overcommit aggregate workspace/credential byte, concurrency, or spend limits; completion/cleanup reconciles safely; neither upload plaintext nor provenance identity/signature reaches ordinary durable storage; client cannot lower floor; expired/revoked/sensitive paths destroy both hierarchies before terminal state |
...
- `artifact_source_authorities`, immutable `artifact_source_authority_versions`, immutable `artifact_source_authority_refresh_evidence`, and locked `artifact_source_authority_current_projections`. A stable authority uses a random non-enumerable ID, workspace, qualified connector/data-owner kind, encrypted external authority-envelope reference, and no content-derived ID or raw repository/signing identity. Material versions carry active/revoked status, classification floor, and connector/signing-approval generation; refresh rows carry the captured material/configuration generation, qualified safe observation lineage, observed time, and bounded `valid_through`. The current projection selects one material version/generation and latest refresh-evidence/freshness state. An unchanged successful refresh advances only the evidence pointer through an expected-prior-projection compare-and-set, while data-owner revocation or qualified connector visibility/classification change appends/selects a new material version; late evidence cannot overwrite a newer or revoked projection, and missed refresh makes it stale and unusable. Both streams are append-only, and a material projection change immediately invalidates older dependent attestations without scanning every artifact
...
- Every trusted source authority has one random internal ID, immutable material versions, immutable qualified refresh evidence, and an event-backed locked current material-version/generation plus freshness projection; repository/source identity and signing scope remain in its encrypted external authority envelope. An unchanged successful connector refresh advances only the evidence/`valid_through` pointer. Refresh failure beyond `valid_through` makes the projection stale, while a repository visibility/classification change, data-owner approval revocation, or any material generation change appends and selects a new material version. Upload-specific attestations bind the exact admitted material version/generation; artifacts without claimed authority instead bind the exact workspace unverified-upload-floor version. Completion, grant mint/replay/redemption, every artifact response/tool chunk, routing, and every initial/fallback fence lock and compare the applicable current material projection and require current freshness before release/send; missing, stale, revoked, or mismatched authority or a changed unverified floor produces `source_revalidation_required`. Revalidation can supersede the attestation only after exact provenance/content identity verification and monotonic classification recomputation under the new authority or workspace-floor version. Artifact/source erasure removes its authority link after both per-upload hierarchies are destroyed.
...
- random non-enumerable source-authority identities with immutable material versions, encrypted authority envelopes, event-backed active/revoked current projections, bounded connector-refresh freshness, and exact-version checks at completion, every grant/read chunk, routing, and every dispatch fence;
...
- source-authority material-version/current-projection transitions, immutable refresh evidence/deadlines, unchanged-refresh material stability, late-refresh versus revocation/change compare-and-set barriers, repository public-to-private changes, signing-approval revocation, exact-version revalidation, and floor lattice maximum prove stale authority and client declarations/detectors cannot preserve or lower trusted provenance/workspace defaults;
...
- source-authority/upload contracts expose only random authority IDs, exact immutable material version/generation, safe state/floor, opaque authorized provenance/content-hierarchy/source-hierarchy references, and a sealed subject/workspace/human-epoch target carrying optional exact API-credential plus exact applicable source-authority-or-unverified-floor projections and the exact server-owned upload-quota reservation version/ceilings. They never expose client-writable authority/quota, content keys, direct object-store/plaintext targets, raw identity responses, source payload/signature/repository/commit/manifest identity, client-writable source floor/effective class, publicly verifiable identity oracle, or ordinary plaintext digests; absent, expired/revoked/stale/mismatched-authority-or-floor, released/exhausted/mismatched-reservation, cross-workspace, human/API-credential epoch-mismatched, permission/scope-revoked, unauthorized, or non-gateway paths fail closed, and abandoned/revoked/retention-forbidden terminal representations require destruction evidence for both hierarchies;
...
- source-authority/provenance fixtures show a malicious or mistaken client declaration cannot lower the server-derived floor: verified public repository/commit and data-owner-approved records bind their random authority plus exact current material version/generation and encrypted upload-specific attestation, arbitrary uploads default to the workspace `private_internal` floor, and detector-clean proprietary source stays private. A connector-observed public-to-private repository transition, missed `valid_through` refresh, signing-approval revocation, or version mismatch atomically blocks upload completion, grant mint/replay/redemption, HTTP/MCP read chunks, routing, and unfenced dispatch before fan-out; only exact-identity revalidation under the new current version can restore stricter eligible use. Reverse-order barriers prove the pointer change or operation wins cleanly. Low-entropy repository/commit/manifest candidates cannot be tested from database/backups; retention-forbidden completion or later artifact/source reclassification suppresses comparison immediately and cannot reach terminal erasure until the upload-specific source hierarchy and all recovery copies are attested destroyed and the artifact-to-authority link is tombstoned;
